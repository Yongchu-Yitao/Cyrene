...
def test_browser_floating_surfaces_use_pointer_shelf_hit_testing_and_favicon_state():
    ...
    assert "image.referrerPolicy = 'no-referrer'" in main
    assert source.count('referrerPolicy="no-referrer"') >= 2
    assert source.count('onLoad={function (event) { event.currentTarget.hidden = false; }}') >= 2
