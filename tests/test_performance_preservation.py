...
def test_unrelated_cache_hit_proceeds_during_miss_and_close_waits(tmp_path):
    ...
    cache=ContextReadCache();entered=threading.Event();release=threading.Event();closed=threading.Event()
    cache.read(first,lambda _:['cached'],lambda _:None)
    def load(_):
        entered.set();assert release.wait(5);return ['slow']
    with ThreadPoolExecutor(max_workers=3) as pool:
        slow=pool.submit(cache.read,second,load,lambda _:None);assert entered.wait(2)
        ...
        release.set();assert slow.result(2)==['slow'];closing.result(2)
...
def test_same_key_load_is_coalesced_and_evicted_connection_survives_lease(tmp_path):
    first,second=database(tmp_path/'a'),database(tmp_path/'b')
    cache=ContextReadCache(max_entries=1);entered=threading.Event();release=threading.Event();calls=[]
    def load(_):
        calls.append(1);entered.set();assert release.wait(5);return {'x':[1]}
    try:
        with ThreadPoolExecutor(max_workers=3) as pool:
            slow=pool.submit(cache.read,first,load,lambda _:None);assert entered.wait(2)
            assert cache.read(second,lambda _:['other'],lambda _:None)==['other']
            release.set();assert slow.result(2)=={'x':[1]}
        assert len(cache._entries)<=1
        cache.close();entered.clear();release.clear();calls.clear()
        with ThreadPoolExecutor(max_workers=4) as pool:
            jobs=[pool.submit(cache.read,first,load,lambda _:None) for _ in range(4)]
            assert entered.wait(2);release.set();values=[j.result(2) for j in jobs]
        assert len(calls)==1
        values[0]['x'].append(2);assert values[1]['x']==[1]
    finally:release.set();cache.close()


def test_cache_replacement_write_during_load_failure_and_retry(tmp_path):
    path=database(tmp_path/'a');cache=ContextReadCache()
    def load(_):
        with sqlite3.connect(path) as db:return db.execute('select value from data').fetchone()[0]
    try:
        assert cache.read(path,load,lambda _:None)=='old'
        new=database(tmp_path/'replacement')
        with sqlite3.connect(new) as db:db.execute("update data set value='new'")
        os.replace(new,path);assert cache.read(path,load,lambda _:None)=='new'
        def changing(_):
            with sqlite3.connect(path) as db:db.execute("update data set value='changed'")
            return 'before write'
        ...
        def failure(_):raise RuntimeError('expected')
        import pytest
        with pytest.raises(RuntimeError):cache.read(path,failure,lambda _:None,variant='failure')
...
def test_disabled_logging_does_not_serialize_and_enabled_error_is_preserved(monkeypatch):
    logger=logging.getLogger('preservation');logger.setLevel(logging.INFO)
    encoded=[];original=observability._bounded_operation_payload
    def encode(value):encoded.append(value);return original(value)
...
def test_debug_cache_append_rewrite_rotate_delete_and_return_isolation(tmp_path):
    ...
    def event(n):return {'type':'llm_call','event_id':str(n),'timestamp':str(n),'context_trace':{'token_by_type':{'x':n}}}
    ...
    first=repo.context_events(120);first['events'][0]['token_by_type']['x']=99
    ...
    stat=path.stat();path.write_text(json.dumps(event(3))+'\n');os.utime(path,ns=(stat.st_atime_ns,stat.st_mtime_ns))
    assert [x['id'] for x in repo.context_events(120)['events']]==['3']
    replacement=tmp_path/'new';replacement.write_text(json.dumps(event(4))+'\n');os.replace(replacement,path)
    assert repo.context_events(120)['events'][0]['id']=='4'
    path.unlink();assert repo.context_events(120)=={'events':[]}


def test_missing_token_index_still_detects_external_null_and_backfills(tmp_path):
    path=tmp_path/'tree';tree=ContextTreeStore.create(path,tree_id='t',root_id='r',root_value={'role':'system','content':'hello'});tree.close()
    with sqlite3.connect(path) as db:
...
def test_event_store_closes_every_operation_connection(tmp_path,monkeypatch):
    store=ChatRunEventStore(str(tmp_path/'events'));opened=[];original=store._connect
    def connect():
        connection=original();opened.append(connection);return connection
    ...
    with sqlite3.connect(store.db_path) as db:assert db.execute('SELECT count(*) FROM workbench_chat_run_events').fetchone()[0]==1
...
def test_debug_cache_retries_transient_read_failures(tmp_path, monkeypatch, failure):
    ...
    repo = DebugEventRepository(tmp_path, recent_events=lambda _: [], full_event=lambda _: None,
                                subscribe_events=lambda **_: None)
    ...
    def failing_open(self, *args, **kwargs):
        if self != path: return original_open(self, *args, **kwargs)
        if failure == 'open': raise OSError(errno.EMFILE, 'temporary descriptor exhaustion')
...
def test_debug_cache_admission_cannot_break_limited_http_result(tmp_path):
    ...
    from cyrene.workbench.http.system.events import register_event_routes
    events = [{'type':'llm_call','event_id':'old','timestamp':'2020','model':'\ud800'},
              {'type':'llm_call','event_id':'new','timestamp':'2026','model':'normal'}]
    (tmp_path / 'debug_unicode.jsonl').write_text('\n'.join(json.dumps(e) for e in events) + '\n')
    repo = DebugEventRepository(tmp_path, recent_events=lambda _: [], full_event=lambda _: None,
                                subscribe_events=lambda **_: None)
    app, router = FastAPI(), APIRouter()
    register_event_routes(router, repo); app.include_router(router)
...
def test_debug_cache_measurement_failure_only_disables_caching(tmp_path, monkeypatch):
    ...
    repo = DebugEventRepository(tmp_path, recent_events=lambda _: [], full_event=lambda _: None,
                                subscribe_events=lambda **_: None)
    def failed_measurement(*args, **kwargs): raise ValueError('unmeasurable')
    with monkeypatch.context() as context:
...
def test_terminal_printable_runs_preserve_control_and_utf8_boundaries():
    ...
    for width in [1, 2, 15, 16, 17, 31, 64, len(data)]:
        parser = IncrementalPlainTextParser(); lines = []
