...
def test_search_overlay_styles_are_owned_and_loaded_by_shared_search():
    ...
    assert 'href="shared/search/overlay.css?v=0.7.0b14"' in index
    for selector in (
        ".search-overlay {",
...
def test_built_search_overlay_assets_match_the_frontend_sources():
    ...
    assert re.fullmatch(r"0\.7\.0b14-[0-9a-f]{10}", revision)
    assert built_index.replace(revision, "0.7.0b14") == source_index
