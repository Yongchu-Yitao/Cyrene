...
def test_workspace_tab_is_conditional_and_opens_the_started_surface() -> None:
    ...
    controller = (CHAT_FEATURES / "workspace-surface-controller.jsx").read_text(encoding="utf-8")
    ...
    assert 'renderer.id === "workspace-composite"' in controller
    assert '["opened", "replaced", "updated"].indexOf(result.outcome) >= 0' in controller
    assert "persistWorkspaceSurface(ownerChatId, workspaceDescriptor)" in controller
...
def test_manually_reopened_workspace_is_claimed_by_the_user() -> None:
    controller = (CHAT_FEATURES / "workspace-surface-controller.jsx").read_text(encoding="utf-8")
    reopen = controller.split("function wbcOpenStartedWorkspace(options)", 1)[1].split(
        "function useWbcResourceObservations", 1
    )[0]
...
def test_workspace_surface_is_persisted_through_the_chat_api() -> None:
    ...
    persist = controller.split("function useWbcWorkspaceSurfaceState", 1)[1].split(
