...
基准完成后，工作区出现其他并行修改；因此另补跑了 541 项涉及 subagent、remote、i18n 和前端的 Python 回归，全部通过。
...
| 场景 | 指标 | ms | 输出校验 |
|---|---|---:|---|
| chat/single_heavy_run | run_latency_p95_ms | 62.93 | 通过 |
| chat/dense_concurrency | run_latency_p95_ms | 316.96 | 通过 |
| chat/multi_tool_storm | run_latency_p95_ms | 487.42 | 通过 |
| search/simplexng_search_and_fetch | median_ms | 214.97 | 通过 |
| event_bus | 三 worker × 两轮总时间的三次中位数 | 111.79 | 通过 |
| knowledge_search | 三 worker × 两轮总时间的三次中位数 | 2100.30 | 通过 |
| knowledge_write | 三 worker × 两轮总时间的三次中位数 | 1052.27 | 通过 |
| database_init | 三 worker × 两轮总时间的三次中位数 | 8.05 | 通过 |
| scheduled_tasks | 三 worker × 两轮总时间的三次中位数 | 3359.64 | 通过 |
| file_hashing | 三 worker × 两轮总时间的三次中位数 | 55.91 | 通过 |
| terminal | 三 worker × 两轮总时间的三次中位数 | 11105.98 | 通过 |
...
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 3 | 5 | 100 | 418.65 | 137.45 | 2.83 |
| 4 | 3 | 5 | 100 | 917.28 | 301.78 | 5.56 |
| 12 | 3 | 5 | 100 | 3012.84 | 931.01 | 22.90 |
| 1 | 40 | 1 | 100 | 2400.22 | 60.71 | 6.18 |
| 1 | 1 | 5 | 5000 | 290.23 | 289.90 | 5.16 |

累计 57 个会话、276 轮、42,300 个流式分片，全部校验最终内容、模型／工具调用次数、片段顺序和 idle 终态。
所有会话关闭后重新打开，验证最后一轮持久回复一致，且不意外重新调用模型。
这属于正常关闭后恢复验证；异常中断和锁错误由现有功能测试覆盖，没有做断电或 kill -9 的真实故障实验。

40 轮对话的前五轮耗时中位数约 45.00 ms，后五轮约 72.55 ms。
...
|---:|---:|---:|---:|---:|---:|---:|
| 100 | 1 | 0.10 | 10.30 | 17.70 | 1.30 | 1.60 |
| 100 | 100 | 1.90 | 23.00 | 36.50 | 5.60 | 5.90 |
| 1000 | 1 | 0.40 | 73.20 | 143.90 | 6.80 | 7.30 |
| 1000 | 1000 | 133.40 | 304.20 | 449.70 | 171.40 | 172.90 |

最大场景包含 26,000 个后代 DOM 元素。
...
实际整体历时 1132.50 ms，定时采样最大间隔 17.10 ms；
...
- src/cyrene/workbench/webui/frontend/shared/i18n/catalog-zh.jsx
- src/cyrene/workbench/webui/frontend/shared/i18n/catalog-en.jsx
- src/cyrene/plugins/builtin/cyrene_remote/commands.py
- src/cyrene/plugins/builtin/cyrene_subagent/contracts.py
- src/cyrene/plugins/builtin/cyrene_subagent/__init__.py
- src/cyrene/plugins/builtin/cyrene_subagent/manager.py
- src/cyrene/plugins/builtin/cyrene_subagent/runtime_policy.py
