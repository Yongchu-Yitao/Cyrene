...
- `python -m cyrene --agent` 不宜在同一版本直接变成未知参数。建议至少一个发布
  周期内作为 `--workbench` 的 deprecated alias，并输出明确告警；它不再启动
  第二套 UI。
...
- 用户无法进入第二套 UI，但旧启动命令仍能安全落到 Workbench。
...
| `--agent`/旧 build config 使升级后无法启动 | 高/高 | deprecated alias 和 mode normalization |
...
- 观察期内不立即删除 deprecated `--agent` alias。
...
- [ ] 不存在 `src/workbench-webui`。
- [ ] 只有一个前端源码根和一个静态构建输出根。
- [ ] `server.py` 不再挂载 `/static/workbench-ui`。
- [ ] 不存在 `shell=legacy` 和 classic UI root 分支。
- [ ] `index.html` 不加载 classic 页面脚本。
- [ ] Workbench 源码不直接依赖未登记的 `window.*` 全局。
- [ ] 所有静态 URL 在 source、Electron 和 frozen 模式均返回 200。
- [ ] 不存在手工维护的重复 compiled 源码。
- [ ] 不存在被 Workbench 使用却归类为 legacy 的残留文件。
- [ ] `--agent` 的兼容行为已测试并有明确移除版本；或已按版本政策正式移除。
- [ ] `legacy:*` 聊天、历史数据库和 `kb_default.db` 兼容测试仍通过。
- [ ] PDF.js Electron 兼容测试仍通过。
- [ ] OpenAPI 和工具 wire snapshot 无意外变化。
- [ ] 94 个工具定义/handler 及 actor policy 保持一致。
- [ ] 架构边界、route structure、import compatibility 全部通过。
- [ ] 全量 pytest、Electron App Use、E2E、build、frozen smoke 全部通过。
- [ ] 主窗口、Quick Chat、首次启动、设置、聊天、Browser、PDF、Library 的
  人工验收有记录。
- [ ] 文档和注释不再宣称存在两套 UI。
- [ ] 临时 redirect、bridge、feature flag 和 TODO 均已删除或有明确 issue。
...
3. 产品能力上，除明确删除旧视觉 shell 外，现有所有功能均不受影响，无隐藏
   降级、无数据丢失、无权限绕过、无接口漂移。
...
在完成上述全部门禁之前，只能称为“重构进行中”，不能以目录已经合并或页面
可以打开作为完成依据。
