...
1. 配对设备长期授予的能力。
2. 被控端本次批准或无人值守策略。
3. 当前桌面会话签发的短期 capability token。
...
idle
  ...
  → selecting_mode
  → waiting_target_approval / waiting_credentials
  → gathering_ice
  → connecting_direct
...
- 用户取消批准、凭据、系统权限或麦克风提示必须是正常取消，不记录为崩溃。
- 凭据错误允许有限重试，并避免触发账户锁定风暴。
- 网络重连不重复询问仍有效的本次会话批准，但凭据句柄失效后必须重新输入。
- 第二个控制者只能发起“请求接管”，必须由当前控制者或被控端批准。
