...
The package-boundary refactor is complete and operational. Cyrene now has one
canonical implementation location for each domain while preserving historical
...
- a newly built PyInstaller application, including dynamic import aliases.
...
└── workbench-webui/         Workbench front-end source
```
...
| `python -m cyrene --agent` | foreground classic UI |
...
The PyInstaller spec enumerates all local Python modules because tools and
adapters use dynamic imports. The frozen smoke test imports critical compiled
...
| Source Electron | window/backend/static/API startup passed |
| Legacy DB migration | source retained, data copied, marker and quick-check passed |
| PyInstaller | build, 60 aliases, Web/API/migration/shutdown passed |
