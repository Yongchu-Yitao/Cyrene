...
包边界重构与 WebUI / Workbench 合并已经完成并可正常运行。Cyrene 的每个
后端领域只有一个正式实现位置，前端只有一个 Workbench 源码根和一个 Web
生成输出根，同时继续支持历史 Python Import 与用户数据。
...
- 只保留 Workbench，classic shell、双挂载和 `--agent` selector 已删除；
- 真实 `cyrene start/status/API/stop`；
- 通过物理启动垫片运行 Electron 开发模式；
- 真实 Workbench、Quick Chat、Theme/Viewport、Browser 与 PDF 验收；
- 首次启动旧数据库文件名迁移；
- 重新构建的 PyInstaller/Electron 应用、动态导入别名和基线到当前包的升级。
...
原 `--agent` UI 选择器已删除，所有 Web/Electron 启动都进入 Workbench。
历史 build mode 值 `agent` 仅为旧 Automation/Artifact 规范化到 Workbench，
不会选择第二套 UI。PyInstaller Spec 会枚举所有本地 Python 模块，因为 Tool
与 Adapter 使用动态导入。冻结 smoke test 会导入关键编译依赖并验证所有历史
别名。
...
| Web UI Build | 32 个 JSX Source，唯一 `static/app` 输出通过 |
| OpenAPI | 259 个操作，规范化 Schema 未变化 |
| Tool Registry | 94 个定义/Handler，未变化 |
| CLI 生命周期 | start/status/API/stop 通过 |
| 源码 Electron | Main Window、Backend/Static/API 与 Quick Chat 通过 |
| Browser/PDF/Theme | 真实 Browser/PDF 和 Light/Dark/System Viewport 检查通过 |
| 旧 DB 迁移 | Source 保留、数据复制、Marker/quick-check 通过 |
| Packaging/Upgrade | PyInstaller、Electron、60 个 Alias、Frozen Web/API 与隔离基线升级通过 |
...
- 不要再记录 Workbench Context Debugger 页面。Context Trace 通过 verbose
  JSONL、`/api/context-debug/events`、`cyrene flow` 和
  `cyrene.observability.context_debug` 保留。
