...
export function AuditApp(){
  ...
  React.useEffect(()=>{
    ...
    function receive(event){
      ...
      if(data.command==='split'){setShowSplit(x=>!x);
  ...
  return <><div className="workbench-shell"><div className="wbc-page"><div className="audit-thread" ref={ref}><WbcMain project={{id:"audit-project",workspacePath:"/synthetic"}} chat={chat} onSend={(...args)=>action("send",...args)} onInterrupt={()=>action("interrupt")} runtimeEngine={engine} onOpenFile={file=>action('open-file',file?.name||file?.url)} onRetryMessage={message=>action('retry',message?.id||message)} onEditMessage={(...args)=>action('edit',...args)} onAnswer={(...args)=>action('answer',...args)}/>{showSplit&&<div className="audit-split-wrapper"><WbcChatSplit chatId="audit" project={{id:'audit-project',workspacePath:'/synthetic'}} runtimeEngine={engine} onClose={()=>setShowSplit(false)} onOpenContent={(...args)=>action('split-open',...args)} menuDisabled={true}/></div>}</div></div></div><output id="input-audit" hidden></output><output id="actions">{JSON.stringify(actions)}</output></>;
}
