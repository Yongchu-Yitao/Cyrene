...
async def main(root):
    ...
    class Timed(AgentSession):
        def _prepare_model_input(self, trigger):
            ...
            try:value=super()._prepare_model_input(trigger)
            finally:
                if profiler:profiler.disable();PROFILES.append(profiler)
    ...
    results=[]
    for config in ([(1,40,1,100)] if 'profile' in sys.argv else [(1,3,5,100),(4,3,5,100),(12,3,5,100),(1,40,1,100),(1,1,5,5000)]):
...
if __name__=='__main__':
    with tempfile.TemporaryDirectory(prefix='cyrene-agent-matrix-') as temporary:
        ...
        if PROFILES:
            output=io.StringIO();stats=pstats.Stats(PROFILES[0],stream=output)
            for profile in PROFILES[1:]:stats.add(profile)
            stats.dump_stats(str(HERE/'agent-prepare.prof'))
            stats.strip_dirs().sort_stats('cumulative').print_stats(45)
            stats.sort_stats('tottime').print_stats(35)
            (HERE/'agent-prepare-profile.txt').write_text(output.getvalue())
