...
import {WbcVoice} from './frontend/features/chat/voice-playback.jsx';
...
function fixture(n=80,m=40){
  ...
  for(const message of history)if(message.role==='assistant')Object.assign(message,{usage:{prompt_tokens:100,completion_tokens:50,total_tokens:150},latestRequestUsage:{total_tokens:150},modelIdentity:{provider:'test',model:'synthetic'}});
...
export function AuditApp(){
  ...
  React.useEffect(()=>{
    ...
    const nativeFetch=window.fetch;let voiceReady=false;
    window.fetch=(url,options)=>String(url)==='/api/voice/status'?Promise.resolve(new Response(JSON.stringify({asr_ready:false,tts_ready:voiceReady,auto_read:false}),{headers:{'Content-Type':'application/json'}})):nativeFetch(url,options);
    ...
    window.audit={
      async step(command){
        ...
        }else if(command==='voice-ready'||command==='voice-off'){voiceReady=command==='voice-ready';await WbcVoice.refresh(true);
        }else if(command==='translations'){window.CyreneUI.require('i18n').registerTranslations({en:{'workbenchChat.copy':'Copy updated label'}});setChat(current=>({...current}));
        }else if(command==='scalar-mutation'){state.current.chat.messages[1].content='In-place scalar edit 文';setChat(current=>({...current}));
        }else if(command==='metadata-change'){state.current.chat.messages[1].usage.total_tokens=999;setChat(current=>({...current}));
        }else if(command==='metadata-replace'){state.current.chat.messages[1].usage={total_tokens:888};state.current.chat.messages[1].modelIdentity.model='changed';setChat(current=>({...current}));
        }else if(command==='metadata-remove'){delete state.current.chat.messages[1].usage;setChat(current=>({...current}));
