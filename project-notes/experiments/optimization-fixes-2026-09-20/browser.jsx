...
function fixture(n,m) {
  const metadata={"usage": {"prompt_tokens": 100, "completion_tokens": 50, "total_tokens": 150}, "latestRequestUsage": {"prompt_tokens": 100, "completion_tokens": 50, "total_tokens": 150}, "modelIdentity": {"provider": "test", "model": "synthetic"}};
  const text='Fixed benchmark paragraph with **bold text** and `inline code`.\n\nSecond paragraph.';
  const messages=Array.from({length:n},(_,i)=>({...metadata,id:'old'+i,role:'assistant',content:text,createdAt:'2026-09-19T00:00:00Z'}));
...
async function execute() {
  ...
  const results=[];
  try {
    for(const [n,m] of [[1000,1],[2000,1],[1000,1000]]) {
      const samples=[];
      for(let repetition=0;repetition<3;repetition++) {
        ...
        for(let i=0;i<5;i++) {
          ...
          const rendersBefore=window.__auditAssistantCalls||0;
          ...
          document.getElementById('transcript').scrollHeight;
          updateSamples.push({commitMs:updateCommitMs,layoutMs:performance.now()-t,assistantRenderCalls:(window.__auditAssistantCalls||0)-rendersBefore});
          await nextPaint();
        }
