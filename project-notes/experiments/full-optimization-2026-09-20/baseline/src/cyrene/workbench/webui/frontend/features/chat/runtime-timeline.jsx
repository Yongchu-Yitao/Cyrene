...
// Private fast path for the final projection of ordinary JSON message data.
// Preserve the legacy merge for correlation, unusual records and small inputs.
// No sorting, mutation, cross-render cache, or changes to message identity.
function wbcTryMergeOrderedMessages(messages, additions) {
  if (!Array.isArray(messages) || !Array.isArray(additions)) return null;
  if (additions.length < 8) return null;
  if (additions.length * (messages.length + additions.length) < 4096) return null;
...
function wbcMergeProjectedMessages(messages, additions) {
  ...
  return ordered === null ? wbcMergeChronologicalMessages(messages, additions) : ordered;
}
...
function wbcProjectTranscript(messages, runtime) {
  ...
  var merged = wbcMergeProjectedMessages(durable.filter(function (message) {
