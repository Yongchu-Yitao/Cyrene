...
import {WorkbenchChatRuntimes} from './frontend/features/chat/file-resources.jsx';
import {wbcProjectTranscript,wbcMergeSavedAssistantMessages} from './frontend/features/chat/runtime-timeline.jsx';
import {WbcChatSplit} from './frontend/features/chat/split-pane.jsx';
import {WorkbenchChatModel} from './frontend/features/chat/model-api.jsx';
...
const networkEvents=[];
const inputEvents=[];
for(const type of ['beforeinput','input','compositionstart','compositionupdate','compositionend','keydown'])document.addEventListener(type,event=>{if(!(event.target instanceof HTMLTextAreaElement))return;inputEvents.push({type,data:event.data??null,key:event.key??null,inputType:event.inputType??null,trusted:event.isTrusted,value:event.target.value});document.getElementById('input-audit')?.replaceChildren(JSON.stringify(inputEvents));},true);
const subscribers=new Set();
let currentRuntime=null;
let readFixtureChat=()=>null;
WorkbenchChatModel.getChat=async()=>JSON.parse(JSON.stringify(readFixtureChat()));
const engine={isRunning:()=>!!currentRuntime,reconnect:()=>{},get:()=>currentRuntime,snapshot:()=>({audit:currentRuntime}),subscribe(fn){subscribers.add(fn);return()=>subscribers.delete(fn);},subscribeSummary(fn){subscribers.add(fn);return()=>subscribers.delete(fn);}};
...
function fixture(name){
  ...
  let runtime={progress:[],activities:[],segments:[],artifacts:[],text:'',chatId:'audit',startedAt:Date.now(),timeline:{runId:'run',revision:1,status:'running',messages:live},userMessages:[]};
...
export function AuditApp(){
  ...
  readFixtureChat=()=>runState.current.chat;
  ...
  React.useEffect(()=>{
    ...
    function receive(event){
      ...
      if(data.command==='network'){
        setNetworkMode(true);
        WorkbenchChatRuntimes.setHooks({onAssistantSaved(id,messages){const next=wbcMergeSavedAssistantMessages(runState.current.chat,messages);runState.current.chat=next;setChat(next);},onError(id,error,meta){action('network-error',String(error.message),meta);if(meta?.terminal)setNetworkError(String(error.message));},onInterrupted(){action('network-interrupted');}});
        WorkbenchChatRuntimes.subscribe(snapshot=>{const runtime=snapshot.audit;const projection=wbcProjectTranscript(runState.current.chat.messages,runtime);networkEvents.push({revision:runtime?.timeline?.revision??null,cursor:runtime?.eventCursor??null,reconnecting:!!runtime?.reconnecting,messages:projection.map(x=>({id:x.id,content:x.content,status:x.status}))});document.getElementById('network-audit')?.replaceChildren(JSON.stringify(networkEvents));});
        WorkbenchChatRuntimes.reconnect('audit',WorkbenchChatModel);
      }else if(data.command==='split'){setShowSplit(x=>!x);
      }else if(data.command==='scenario'){
  ...
  React.useEffect(()=>{
    ...
    const report=()=>{
      ...
      const scroll=root.querySelector('.wbc-thread');
      const viewport=Array.from(root.querySelectorAll('.wbc-thread'),scroll=>({top:scroll.scrollTop,height:scroll.scrollHeight,client:scroll.clientHeight}));
      ...
      parent.postMessage({networkEvents,kind:'audit-result',variant:__AUDIT_VARIANT__,step,html:root.innerHTML,text:root.textContent,geometry,rows:rows.length,preserved,fastHits:window.__auditFastHits,actions,viewport,inputEvents,editValues:Array.from(root.querySelectorAll('.wbc-edit-textarea'),x=>x.value)},location.origin);
    };
  ...
  return <><div className="workbench-shell"><div className="wbc-page"><div className="audit-thread" ref={ref}><WbcMain error={networkError} errorKind="message" project={{id:"audit-project",workspacePath:"/synthetic"}} chat={chat} onSend={(...args)=>action("send",...args)} onInterrupt={()=>action("interrupt")} runtimeEngine={networkMode?WorkbenchChatRuntimes:engine} onOpenFile={file=>action('open-file',file?.name||file?.url)} onRetryMessage={message=>action('retry',message?.id||message)} onEditMessage={(...args)=>action('edit',...args)} onAnswer={(...args)=>action('answer',...args)}/>{showSplit&&<div className="audit-split-wrapper"><WbcChatSplit chatId="audit" project={{id:'audit-project',workspacePath:'/synthetic'}} runtimeEngine={networkMode?WorkbenchChatRuntimes:engine} onClose={()=>setShowSplit(false)} onOpenContent={(...args)=>action('split-open',...args)} menuDisabled={true}/></div>}</div></div></div><output id="network-audit" hidden></output><output id="input-audit" hidden></output><output id="actions">{JSON.stringify(actions)}</output></>;
}
