> **COMPLETED — 2026-07-26:** Every repository-controlled Definition of Done
> item in this plan has been satisfied. Future-tense sections are retained as
> historical implementation rationale, not pending work.
...
>
> Document role: preserves the pre-implementation plan and audit baseline;
> completion evidence is in the
> [implementation log](COMPLETED-webui-consolidation-implementation-log.md) and
> [architecture handoff](COMPLETED-refactor-handoff.md)
>
...
Implementation result (2026-07-26): the goal is achieved. During implementation
the user explicitly authorized direct removal of the `--agent` UI selector
instead of retaining a deprecated alias; historical build-mode value `agent`
still normalizes to Workbench. The final handoff review also confirmed that the
Context Debugger does not move into Workbench, while verbose JSONL, the
context-debug API, `cyrene flow`, and the canonical debug module remain. The
future-tense baseline and recommendations below are retained as historical
planning evidence, not as descriptions of the current tree.
...
to classic UI. The plan originally proposed a transitional `--agent` alias;
the user authorized direct removal during implementation. Old frozen build-mode
values still normalize to Workbench, and obsolete IPC was removed after its
consumers were audited.
...
Electron crashes. The proposed `--agent` observation period was superseded by
the user's implementation-time authorization to remove the selector directly.

## 12. Mechanical completion checklist

- [x] `src/workbench-webui` does not exist.
- [x] There is one frontend source root and one static output root.
- [x] `/static/workbench-ui`, `shell=legacy`, and the classic root branch are gone.
- [x] The final HTML loads no classic page scripts.
- [x] Workbench has no unregistered `window.*` dependencies.
- [x] Every static URL returns 200 in source, Electron, and frozen modes.
- [x] Generated output is not a second hand-maintained source tree.
- [x] No Workbench dependency is mislabeled and stranded as legacy.
- [x] `--agent` was removed with explicit user authorization; historical build-mode normalization is tested.
- [x] Historical chats/databases/default KB and PDF.js compatibility pass.
- [x] OpenAPI and tool wire snapshots have no accidental changes.
- [x] All 94 tool handlers and actor policy remain stable.
- [x] Architecture/route/import tests pass.
- [x] Full pytest, Electron App Use, E2E, build, and frozen smoke pass.
- [x] Manual main/Quick Chat/first-run/settings/chat/browser/PDF/Library evidence exists.
- [x] Documentation no longer claims two formal UIs.
- [x] Temporary redirects, bridges, flags, and TODOs are removed or tracked.
...
capabilities remain intact except the explicitly removed visual shell and the
user-approved omission of the Context Debugger page (its JSONL/API/CLI tracing
remains); data, permissions, APIs, and tools have no hidden regression;
equivalent duplicates are safely consolidated; source and generated output are
separate; new install, upgrade, Web, Electron, Quick Chat, PyInstaller, and
supported platforms pass; and an executable rollback exists.

Every gate above is complete. Commands, real-runtime evidence, environment
limitations, and rollback points are recorded in
`COMPLETED-webui-consolidation-implementation-log.md`; the historical future tense in
this document no longer indicates unfinished work.
