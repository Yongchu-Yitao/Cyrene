...
### Conversations, tool execution, and failure recovery

- When a model returns tool requests with duplicate identifiers, a missing name, malformed arguments, or another invalid shape, Cyrene asks for a corrected request before executing anything. Repeated invalid responses stop after a bounded number of attempts instead of running partial operations or retrying forever.
- If one local plugin cancels its own operation, only that tool call becomes a retryable failure; it no longer cancels the entire Conversation. An explicit user stop or task cancellation still ends the active run as before.
- If Deep Reflection is unavailable, raises an error, returns empty material, or cancels itself, Cyrene keeps the original task context and lets the Agent continue or retry. Only cancellation of the whole user task stops the run, so an auxiliary organization failure no longer discards working material.
- If required context, permission rules, or system guidance cannot be prepared, the current run ends clearly and a later user request can try again. Cyrene no longer remains indefinitely pending or calls the model without essential context.
- If a tool ran but its result could not be saved, its output could not be recorded normally, or the app exited at a critical point, Cyrene retains the honest “executed, outcome unconfirmed” state and prevents automatic re-execution. Users can check the external result before deciding whether to retry, reducing duplicate mail, file, or remote operations.
- A result-saving problem in one parallel tool no longer discards other completed results or causes the whole batch to run again. Failures in resource previews, file presentation, or timing information also cannot replace the confirmed outcome of the underlying tool.
- Once a complete answer has been produced, a failure in final processing, an automatic Hook, or the last save no longer replaces that answer. Recovery continues only work that had not started and does not repeat completed or uncertain post-processing operations.
- Even if Cyrene encounters another error while recording a run failure, the Conversation returns to a usable state and preserves as much diagnostic detail as possible instead of requiring an app restart to clear an endless run.
- When a Conversation continues working after an earlier reply segment, its activity label is shortened from “Continuing to process” to “Processing.” Context-usage details add a “Task context management” category and correctly place ordinary behavior rules that follow tool guidance under “Behavior rules,” rather than mixing different kinds of instructions in one item. All of these labels have matching English and Chinese text.
- On every turn, the Agent reads Cyrene's current Simplified Chinese or English setting and uses it for progress updates, questions, explanations, and final replies. A language change takes effect on the next turn without reopening the Conversation. An explicit request for another language still takes priority for its stated scope, while code, commands, paths, identifiers, and exact quotations remain unchanged where needed.

### Long tasks, app shutdown, and local Agent stability

- When the app closes, it first stops accepting new runs and gives an in-progress reply or result commit a grace period to finish. Unacknowledged commits and recoverable runs remain available, reducing cases where the window closes while background state is unclear.
- A browser page, HTTP request, or other waiting client disconnecting no longer releases the Conversation or closes the Agent connection that still owns the work. The same run can recover, while a conflicting new run is rejected explicitly instead of silently cancelling the original.
- Synchronous plugins without a configured timeout now run away from the Conversation's main loop, so a long operation does not freeze other run handling. If waiting times out or is cancelled, the result is shown as unknown and the same plugin cannot overlap in the same workspace until the original thread actually finishes.
- A Bash timeout becomes retryable only after its complete process tree is confirmed stopped. A synchronous plugin that cannot be confirmed stopped remains unknown, so these two cases no longer share an ambiguous result.
- If an automatic Hook worker exits unexpectedly, both the active request and queued requests receive a clear failure. Later requests can start a fresh worker instead of remaining blocked behind an abandoned wait.
- Slow, disconnected, or failing consumers of live progress, tool activity, or timing events no longer hold up the real task or turn a business success into failure. Missing transient UI progress can be recovered from the saved Conversation state.
- For local ACP Agents, input backpressure, a write timeout, or cancellation retires an untrustworthy connection and clears its waiters rather than continuing to send commands that may duplicate work. Long Agent prompts may still wait indefinitely for a response, but sending the request itself always has a finite deadline.
- If a local Agent produces more events than can be handled safely, Cyrene reports a connection-capacity failure instead of silently dropping an earlier reply delta or permission request. History replay also retains peer requests that still need a Cyrene response, reducing Agents left waiting for a lost confirmation.

### Model connections, Agent behavior, and proactive work

- Consecutive requests through the same model connection now reuse an existing network connection, reducing repeated connection setup. A change to the model address, key, proxy, or timeout uses a fresh connection without interrupting an active old request, and idle connections close automatically.
- When a model has sent a complete terminal signal but the service keeps its data stream open, Cyrene preserves the complete reply and stops waiting promptly instead of leaving a finished answer marked as processing. When the service closes normally, the necessary tail is still consumed so later requests can reuse the connection reliably.
- Tool discovery guidance now belongs to Toolbox itself. The Agent chooses a capability from concise PluginPack purposes, reads its current input requirements, and then invokes it; a malformed discovery request is corrected and retried instead of making the whole capability appear unavailable.
- Safe chunking guidance now belongs to the Write tool: the first segment overwrites, later segments append, chunks end at stable boundaries, and the assembled result is checked before completion is reported. The existing absence of a fixed character limit remains unchanged.
- Task-context guidance is consolidated so the Agent silently continues the same goal, saves current work before genuinely beginning an independent goal, and resumes by matching the requested outcome rather than the most recent record. Cross-task agreements are saved only when supported and truly shared; acknowledgments, brief topic changes, and a report alone do not cause a switch.
- Proactive background work is further limited to one explicit listed task and incremental operations; it cannot modify, overwrite, move, rename, or delete existing files. It stays quiet without a material result and sends only one concise notice for a result or concrete blocker, without greetings, guesses about the user's current state, or internal scheduling language.
- A newly created default persona now describes Cyrene as a “universal assistant,” removing the conflicting “not an assistant” wording while keeping its friendship and companionship goals. An existing custom `SOUL.md` is preserved exactly and is not rewritten during upgrade.
- The plugin authoring guide now explicitly excludes legacy activation entry points and keeps current Toolbox, Write, and verification guidance with the capabilities that own it. The Agent reads those current rules before creating and validating a plugin; existing installed-plugin behavior is unchanged.

### Cyrene Doctor investigation, repair, and interface

- Opening Doctor from a failed message now gathers the matching run's terminal state, saved events, traces, and bounded local-log metadata, adding evidence already available on the device. Reads are restricted to that run and never send full Conversations, tool arguments, results, or log bodies.
- Doctor recognizes failures saved by older versions as error-marked terminal records and brings model connectivity, general run problems, and plugin failures into the same check. Successful tools are grouped so they do not bury the failure evidence, and missing or out-of-range evidence is stated explicitly.
- A new read-only plugin investigation capability can list the real identities and current state of built-in and third-party plugins, search relevant source within strict limits, and read selected files. Doctor chooses a repair target only after obtaining source evidence and respects the user's activation setting for the investigation tool.
- When the original error does not identify a plugin, Doctor can investigate before selecting the responsible source. If the report already contains an applicable recovery action, it prepares that action for review instead of generating an unnecessary source patch.
- Plugin repair now supports existing JSON, TOML, Markdown, text, JavaScript, JSX, HTML, and CSS source or configuration files in addition to Python. It still does not create or delete files, install dependencies, or exceed the existing file-count, content-size, and scope limits.
- Packaged macOS builds can use an existing Python supplied through the environment plugin to run an isolated reproduction. When no suitable interpreter or isolation facility exists, Doctor clearly falls back to static checks; it does not install Python or execute generated code outside isolation.
- When applying a repair, Doctor resolves the actual PluginPack from its source and checks load, setup, and startup results. A change that needs a restart says so, and after reopening the app the user can verify the same repair without applying it twice; lifecycle failures still restore the original files and retain the reason.
- The Doctor page now combines progress, a user-readable conclusion, next steps, and repair status in one “Help with this problem” card. The problem description is collapsed by default, while the original report, errors, and repair controls share an “Error details” section. Primary views no longer expose raw internal errors, and connection, failure, and retry guidance has clearer English and Chinese wording.

### Plugin management, installation, and consistent runtime state

- Plugin switches in Settings and Agent tools now use the same incremental update flow: only the user's chosen items change, and runtime state plus UI notifications update after the settings are successfully saved. A save failure no longer leaves the UI or in-memory state ahead of the real setting.
- A PluginPack's main switch remains separate from the switches for its individual tools. Even when every tool in a pack is disabled, an enabled pack can still provide its application services; status views separately report pack activation, enabled-tool count, and whether its application portion is running.
- Changes to Python plugin source or the shared English and Chinese catalog now trigger the correct reload decision, reducing stale descriptions or tool names after an edit. Management operations that make no actual change no longer produce unnecessary refreshes.
- Plugin installation, authoring, Settings, and Agent tools now share the same source identification, static validation, installation, and deletion rules, reducing inconsistent load state, restart guidance, and error reporting between entry points.
- Plugin setup entry points must complete synchronously during loading. A plugin that declares asynchronous setup, or appears synchronous but returns unfinished work, is rejected clearly during validation or loading instead of appearing active with incomplete services. Ordinary tool handlers, Hooks, and application-start tasks may still be asynchronous.
- Before installing from the authoring area, Cyrene checks whether a draft changed after the user reviewed it and requires validation again when necessary. A partially failed load also cannot publish incomplete application services or claim that they are running.
- Plugin source deletion remains restricted to real top-level entries in the managed plugin directory, with consistent protection against symbolic links and escaping paths from Settings, Agent, or Doctor entry points.
- The new “Investigate plugin problems” tool has complete English and Chinese names and descriptions in activity history and tool lists. Doctor and plugin guidance also use more approachable labels such as “Extensions,” “App settings,” and “Check connection.”
...
- Built-in Windows web search fixes compatibility problems, including a process check that could unexpectedly terminate the main application after search started. ARM64 search also fixes cases where the component could load but fail to start an actual search, or close itself after incorrectly deciding that the main application had exited. Stopping search also closes its launcher and calculation processes, avoiding leftover search processes after a restart. Search startup errors in desktop packages retain logs for troubleshooting.
...
### Cyrene Doctor and diagnostic entry points

- Settings now includes a Cyrene Doctor page that can check application configuration, Plugins, Conversation failures, and project-memory state. Results clearly distinguish healthy checks, detected problems, unavailable checks, normal skips, and informational notes instead of treating an incomplete check as proof that everything is healthy.
- Help & Support also includes a prominent diagnostics entry point. Settings, Help, and the modal now use complete Chinese and English copy, a distinct rose-colored identity, and a layout that adapts to narrow windows, making it easier to start troubleshooting from the current workspace.
- Failed Conversation runs, errors on project-memory or learning pages, and ordinary error notifications now offer a Cyrene Doctor action with the relevant project, Conversation, or incident context already attached. Existing retry, copy-details, and other actions remain available.
- Basic checks do not depend on a model. Confirmed results and next-step guidance remain available when the Agent cannot run, no model is configured, or the network is unavailable. Doctor does not need to read full Conversation text, memory content, credentials, or sensitive error text to perform these checks.
- Each result can reveal its supporting evidence. Reports receive a separate identifier and can be exported as JSON so the state of a diagnosis can be retained. If the report cannot be written to disk, the current basic findings still appear with a clear warning to address the storage problem.
- Completed checks first show issue, passed, and total counts. Findings that need attention remain immediately visible, while healthy and informational results are grouped under a collapsed section; report identifiers are also moved into separate details so the main conclusion is easier to scan.
- Results for the same project or Conversation are retained briefly within the current app session. Reopening Doctor shows the existing findings immediately while refreshing them, and preparing to open it from Help begins loading early without creating duplicate checks. Different Conversations and languages remain isolated, and diagnostic evidence is not placed in persistent browser storage.
- Configuration, Plugin loading, Conversation execution, model failures, memory-processing stages, and project-learning jobs can now be related in one report instead of every problem collapsing into a generic “Agent run failed” or “memory save failed” message.

### Agent analysis, repairs, and offline recovery

- Users can optionally ask the Agent to analyze a basic report. This analysis is limited to the evidence in that report; it does not inherit the user's SOUL, memories, Skills, MCP access, or ordinary file and terminal tools, and it cannot independently perform work actions or widen the repair scope.
- Before requesting Agent analysis, users can optionally describe the symptoms, reproduction steps, and recent changes. The description is supplied to the model alongside redacted findings and remains clearly separate from verified evidence; the field warns against entering credentials and has a length limit so a clean basic check does not conceal the problem the user actually experienced.
- Agent analysis can be stopped. Interrupted, malformed, or timed-out model responses receive a limited number of automatic recovery attempts with progress shown in the interface. Existing evidence is retained and the work operations behind the original failure are never replayed; if the model remains unavailable, the basic report and manual guidance stay intact.
- “Test model” sends one minimal request without real Conversation content and warns that a small amount of model usage may be incurred, helping separate a model-connection problem from other application failures.
- Repairable findings show a concrete preview before the user decides whether to apply anything. Supported actions include restoring one built-in Plugin to its shipped files, removing one tool name, description, visibility, or deletion override, and precisely retrying one failed or conflicting project-memory learning job.
- Plugin and configuration repairs retain the previous state for rollback and automatically run a verification check afterward. If the target changes after the preview was created, the stale plan is rejected instead of overwriting newer user edits. Repairs wait until related runs are idle and do not interrupt an active Conversation or memory-learning task.
- A project-memory retry uses the exact original evidence and source Conversation saved with the failed job. Only that job is retried; newer chat content does not replace its evidence, and other jobs or already successful learning work are not replayed in bulk.
- When the Python backend fails to start, exits unexpectedly, or times out during desktop startup, Cyrene now opens an independent local recovery page instead of immediately quitting. Users can run offline checks or retry startup, and the page still provides installation, runtime-file, and local-log guidance when Python itself is unavailable.
- A new `cyrene doctor --offline` command performs read-only checks without starting the main service or editable Plugins. It can target a project or Conversation, switch between Chinese and English, and return structured output.

### Error diagnosis and project-memory reliability

- Model failures in Conversations and project memory preserve their real authentication, quota, rate-limit, network, timeout, context-size, and incomplete-response categories as they pass through the application instead of degrading into an unclassified generic error.
- Failures in Conversation runs, background Plugins, and shared application requests receive content-free incident identifiers with the stage and related object. Error cards and Doctor can use the same identifier to locate evidence, while user cancellations, external requests, and Doctor's own requests do not create repeated diagnostic prompts.
- Frontend network errors, failed application requests, and unhandled interface errors offer a persistent Doctor action, with repeated instances of the same problem combined for a short period. Records do not collect URLs, request or response bodies, exception text, credentials, or Conversation content.
- Project-memory processing now records whether a turn was triggered, obtained its material, skipped normally, completed, or failed. Normal reasons such as memory writing being disabled, ineligible content, an unmet threshold, or missing committed material are separated from actual save failures.
- Failed project-memory learning retains a more accurate model cause, and interrupted work or removed source material receives wording that better reflects what happened. A Conversation legitimately using an older frozen memory snapshot is reported as information rather than prompting the user to clear or forcibly replace memory.
- Doctor checks and repairs do not weaken existing data validation, model permissions, or tool permissions. They do not guess how to rebuild a damaged database, missing encryption key, or unavailable historical learning material; when recovery cannot be performed safely, Doctor provides guidance only.

### Long-Conversation rendering and sidebar motion

- Collapsed historical reasoning and tool activity no longer builds all of its details when a Conversation first appears. Details are created on the first expansion and then retain normal opening and closing animation, making old Conversations with extensive tool output faster and lighter to open.
- Before the sidebar opens or closes, Cyrene protects offscreen content using the stable layout from before the change, avoiding an extra full-history layout at the beginning of the animation. Restoring the right-side panel from its menu uses the same preparation.
- A single reply spanning many screens can temporarily simplify only distant paragraphs, code blocks, and tables instead of freezing the entire visible message. Streaming content, selected text, editors, retry controls, and picture-in-picture avoidance remain live and usable.
- After the sidebar animation, very large Conversations restore offscreen content in bounded batches instead of placing hundreds or thousands of messages into one frame. The current reading paragraph or live-tail position is preserved throughout restoration to prevent sudden jumps.
- Conversation navigation and location data pauses repeated measurement until the sidebar layout is fully restored, then refreshes once. Existing message order, disclosure state, browser picture-in-picture position, and user layout choices remain unchanged.
...
### The new plugin framework and Plugin Center

- Cyrene now has one extension experience for toolboxes, standalone tools, context capabilities, background work, application pages, Settings, channels, schedules, proactive work, and Knowledge.
- Except for the required Bash, Read, Write, and Toolbox kernel tools, every built-in tool is supplied as an editable plugin. Installed builds automatically prepare these implementations under the user data `plugin_impl/` directory.
- Plugin Center lists plugin packs and standalone tools together and labels kernel, editable built-in, and user-directory sources. Search, counts, enabled state, and errors update live.
- Every tool has a menu for editing its display name and Agent-facing description, choosing direct visibility or on-demand discovery, and deleting tools that are allowed to be removed.
- Tool names, descriptions, categories, states, menus, confirmations, and errors are available in Chinese and English, and plugin-contributed pages and Settings follow the active language.
- Toolboxes and standalone tools use one catalog flow: the Agent can list everything, inspect a specific tool, and invoke it without a fuzzy-search entry point.
- Visibility is configurable per tool, so frequent tools can be immediately callable while large or infrequent capabilities remain discoverable on demand.
- Disabling or removing a plugin stops its tools, context, background work, and pages together. Re-enabling restores saved settings without leaving hidden capabilities active.
- Plugins can contribute project pages, additional models, and slash commands. Pages retain split panes, fullscreen, detached windows, and restoration.
- User changes to tool names, descriptions, and visibility persist across app restarts and plugin reloads.

### Continuous Agent runs, context, and subagents

- The Agent now uses one continuous run for replies, tool work, user questions, live guidance, retries, cancellation, recovery, and final delivery instead of handing work between separate stages.
- Chat can reply directly, while tool work continues until there is an explicit result. Plans, attempts, and tool activity remain visible without being mistaken for the final answer.
- When the Agent waits for information, it resumes at the original point without redoing completed work. Cancellation, failure, recovery, and interruption all leave a clear final state.
- Context is now an inspectable tree. Personality, project, workspace, memory, tool guidance, run state, and other capabilities are mounted by their owning plugins.
- Composer choices for Personality, default workspace, selecting a directory, MCP servers, Skills, and other capabilities are owned by a dedicated composer-context plugin and directly control the context built for that chat.
- Choices persist per chat and project. Switching chat, Agent, or project cannot leak another scope's selections, and removed or disabled capabilities are cleaned up.
- `SOUL.md` is its own plugin. When enabled, personality appears immediately below the system instructions; when disabled, it is not injected. Editing, long-term memory, and first-run setup remain available.
- Long conversations retain the existing compaction experience, reducing history sent to the model while preserving the complete transcript, final artifacts, and continuation state.
- A subagent starts with the same initial context tree as the main Agent plus its assignment, then keeps independent context.
- Main Agents and subagents continue to communicate through the Inbox. Waiting, wake-up, recovery, and completion preserve already-read boundaries to reduce duplicates and omissions.
- Permission review, sensitive-data filtering, argument validation, and final-answer checks still apply to main Agents and subagents.
- Unchanged context remains reusable, while model or plugin changes update only affected parts for more stable long tasks and follow-ups.

### Workbench Chat and run information

- Workbench Chat is fully connected to the new Agent runtime, with sending, streaming, tool traces, user questions, recovery, and cancellation driven by one run state.
- Conversation Overview shows status, Agent, model, session ID, message count, creation time, input/output/total tokens, context usage, and compaction threshold.
- Context shows the active composition, Agent Inbox events, and toolboxes used during the run, with clear empty states.
- The action row under each running message again shows current-step tokens and now adds token output speed alongside duration, read-aloud, copy, and regenerate.
- Tokens and output speed update while streaming and remain visible after completion, reconnection, or reopening history.
- Active reasoning and tool traces add a one-way shimmer and respect Reduce Motion.
- Duplicate intermediate messages, repeated questions, blank recovered messages, and completed replies stuck as responding have been corrected.
- Terminal recovery, SSH reconnection, and background terminal events no longer steal composer focus.
- Stopping generation promptly restores edit, retry, resend, and navigation.
- Workbench, Quick Chat, browser, and desktop surfaces use the same run and message data.

### Knowledge, schedules, and proactive-work migration

- Knowledge and Literature are fully plugin-delivered while retaining import, attachments, extraction, OCR, vectorization, hybrid retrieval, collections, tags, notes, annotations, citations, and Zotero.
- Upgrading automatically migrates existing Knowledge content and attachments into plugin storage. Installed builds run the same migration, reruns avoid duplicates, and source data remains recoverable.
- Existing project entries, uncategorized state, favorites, recent reads, tags, attachments, and index relationships remain visible and project-isolated.
- Chat attachments, generated files, and completed task artifacts continue into the same project Knowledge service for Knowledge, global search, and Agent retrieval.
- Scheduling is plugin-owned while preserving create, edit, pause, resume, cancel, run history, project assignment, notifications, and existing schedules.
- The proactive Agent is plugin-owned and continues running in the background with selected project context, delivering results to Workbench and enabled channels.
- Disabling Schedule or proactive work stops background execution; re-enabling resumes durable state without a second legacy scheduler.
- Telegram and WeChat use the same plugin-native Agent, tools, context, and delivery flow, with configuration and status in Workbench.
- Knowledge, Schedule, Proactive, Memory, and Channels pages, search, and Settings stay aligned with plugin enablement.

### Tools, browser, code, and desktop control

- Glob, Grep, Edit, and similar filesystem capabilities are standalone catalog tools and do not need a toolbox to be listed, described, and invoked.
- Browser, code analysis, Git, terminals, files, search, attachment analysis, memory, Knowledge, entities, maps, tasks, delivery, Skills, MCP, and Cyrene self-control are plugin-delivered with existing entry points and permissions.
- The Agent can still create, read, and edit files; run shell and Git operations; analyze code; operate shared terminals; control the browser; and use desktop applications.
- Tool arguments are checked against the active plugin declaration, while different object-field ordering is accepted when all items and values map correctly.
- Browser live view retains clicking, typing, scrolling, uploading, tabs, screenshots, and login takeover.
- Cyrene self-control still acts only on visible, authorized elements, with existing permission requirements for Settings, sending, and higher-risk actions.
- MCP servers, Skills, custom plugins, and user tools remain in Settings and composer selections and leave active Agent capabilities immediately when disabled.
- Tool errors, activity traces, permission cards, and result attachments use consistent localized names.
...
- The README, built-in documentation site, application UI, web assets, channel identity, and desktop packages now present `0.9.0-beta1` consistently.
- macOS, Windows x64, Windows ARM64, and Linux continue to receive prerelease packages with the same feature set, plugin catalog, and Workbench interface.
...
- During long tasks, wake-ups, and resumed execution, sub-Agents retain the collaboration state, inbox guidance, and execution checkpoints they have already seen in order, appending only later changes to reduce forgotten context, repeated work, and inconsistent decisions. Once finalization begins, they submit the final result without continuing to use work tools.
