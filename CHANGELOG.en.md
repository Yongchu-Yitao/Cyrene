...
- Built-in Windows web search fixes compatibility problems, including a process check that could unexpectedly terminate the main application after search started. ARM64 search also fixes cases where the component could load but fail to start an actual search, or close itself after incorrectly deciding that the main application had exited. Search startup errors in desktop packages retain logs for troubleshooting.
