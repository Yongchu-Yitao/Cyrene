...
- **项目记忆保存不再因语言不匹配被拒绝** — Agent 保存记忆时,若内容语言与界面语言不一致会尽力翻译;翻译不可用(失败/超时/翻错)时按原文保存,不再返回"保存被拒"。技术密集中文内容(含代码、路径、模型名)直接通过,不再被误判为英文而触发翻译;对话自动沉淀仍按界面语言输出,同一事实与 Agent 写入可正常合并去重。
