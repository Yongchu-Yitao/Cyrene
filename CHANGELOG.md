...
- **全屏视频可以直接用键盘控制** — 视频进入全屏后，键盘输入会继续交给当前网页播放器；可使用播放器支持的空格暂停/继续、方向键快进快退或调节音量等快捷键，无需先退出全屏。
...
- 全屏视频继续复用当前网页的 `WebContentsView`，在 macOS 独立全屏窗口及 Windows/Linux 主窗口全屏模式下保留播放器的原生键盘输入与网页快捷键处理。
