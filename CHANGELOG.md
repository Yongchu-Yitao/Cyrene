...
`0.8.0-beta3` 汇总 `0.8.0-beta1` 以来的全部功能与界面变化，重点加入独立的图片、视频与音乐生成能力、参考素材和媒体消息、对话级上下文能力与斜杠命令、统一网络代理和远程终端，并进一步改善模型服务、Agent 连续执行、PowerPoint、搜索、终端性能、设置和桌面启动体验。本节只记录用户可感知的功能和界面变化。
...
- 首次设置完成后会立即创建并打开第一段对话，用户可以直接开始输入，而不会落到没有活动对话的空页面。
- 设置页进一步统一卡片、输入框、下拉菜单、状态标签、自动保存反馈、键盘焦点和窄窗口布局，使模型、代理和媒体服务的配置方式保持一致。
- README、应用界面、网页资源和桌面安装包中的当前版本展示统一更新为 `0.8.0-beta3`。
