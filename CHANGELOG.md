...
0.9.0 是 beta20 之后的首个正式版。本版重点让多个对话 Agent 可以安全协作，让中断后的对话和长时任务更可靠地继续，并改善长对话的响应速度、流式显示和桌面窗口体验。正式版覆盖 macOS、Windows x64、Windows ARM64、Linux 和 Android。

### 跨对话 Agent 协作

- 新增本地对话协作能力：Agent 可以发现正在运行的其他 Cyrene 对话，并向指定对话发送消息，便于在多个独立任务间传递结果、上下文和后续工作。
- 空闲对话收到消息后可继续工作；正在运行的对话会在安全的处理时机接收。消息按发送顺序保留，并能跨应用重启继续投递，减少协作信息丢失。
- 跨对话消息会清楚标记来源，不会被当作用户授权，也不会绕过原有权限、用户等待或安全确认。发送方在消息可靠接收后即可继续，无需等待对方回复。
- 该能力面向本机 Cyrene 中可识别的主对话、本地对话和侧边对话；不会把外部 Agent 或任意子进程暴露为可联系对象。

### 对话恢复与任务可靠性

- 遇到偶发的内部运行故障时，对话现在会进行有限次数的自动恢复，无需用户手动重发。已完成的工具操作不会重复执行，用户取消和明确不可重试的错误仍会立即停止。
- 恢复过程会保留已有回答、工具历史和对话顺序。只有新的替代内容真正到达时，才会替换本次失败的未完整回答，避免页面突然留白。
- 已结束、已取消或失败的运行会保持终态，迟到事件不会把它们误标为正在运行。用户中途补充的要求和 Agent 间消息也会保留正确来源。

### 子 Agent 长任务

- 移除了依据重复工具参数或结果判断“无进展”并强制收尾的机制。正常的工具发现、重复读取、轮询和等待不再被误停。
- 工具次数、耗时、费用、上下文限制和完成验收仍然有效。旧检查点可以正常恢复，已结束任务不会被重启。

### 长对话性能与流式显示

- 大型、持续更新的对话现在能更快合并历史消息，不必在每次新内容到达时重新处理整个时间线。切换、滚动和实时更新在长对话中更顺畅。
- 不同对话的上下文、缓存和历史读取减少相互阻塞；终端历史和调试查看也更快，同时保留原有内容、操作和显示。
- 流式文字渐现和实时活动卡片动画更平滑、更连续；长回答快速更新时不易闪烁或提前截断动画。

### 桌面窗口与侧边栏

- Windows 和 Linux 现在使用一致的原生风格标题栏，可直接最小化、最大化、还原和关闭，按钮状态会跟随实际窗口变化。
- Linux 中拆出的面板恢复了原生拖动和窗口状态同步，合并回主窗口及原有面板操作保持不变。
- 侧边栏的任务状态现在优先展示刚刚恢复的工作，不再被旧的提问或失败状态覆盖。

### 模型设置与升级

- 升级时会依据完整服务地址识别已有的模型提供方连接，避免再创建一份重复的默认连接。
- 已保存的密钥、自定义连接标识、模型配置和路由会保持不变；未使用的重复默认项会被清理，让模型列表更清晰。

### 正式版发布

- 所有支持的桌面平台和 Android 共用 0.9.0 版本与本更新说明，并通过各平台的安装、启动和基本使用验证。
