...
beta14 改进了应用更新、Windows 联网搜索和长 Conversation 的面板调整体验。更新现在可以单独选择使用代理，失败时提供更具体的原因；Windows 更新会等待应用真正退出并处理文件占用，联网搜索也修复了启动和意外退出问题。聊天状态刷新、图片能力检测和异常模型回复处理同时得到修复，现有聊天、分屏、工具、模型设置与桌面操作方式保持一致。
