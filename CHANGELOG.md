...
- Agent 现在可以在一次请求中持续进行模型思考、工具操作、结果检查和后续处理，不再需要用户为每个中间步骤手动发一条新消息。
- 新增按需发现和使用工具的工具箱。Agent 可以先查找能力、阅读说明再调用；工具名称写错或当前不可用时会获得可纠正的提示，不会因一次选择错误结束整个任务。
- 新增可持久的 Plan 模式：计划可先由用户审阅，再按依赖顺序执行和更新进度，未满足前置条件的步骤不会抢先开始。
- 新增可持久的 Goal 工作流，能保存目标、范围、限制、验收标准和执行状态，支持确认、暂停、恢复、终止和人工接受。
- Goal 完成时由独立的只读审查过程逐项对照验收标准与现有证据；未通过时可进入反思和后续修正，不会只因 Agent 声称“已完成”就收尾。
- DeepReflect 可重新整理用户真实目标、已验证事实、失败原因、应放弃的假设和更合适的后续方向，同时保留原始用户要求和已完成成果。
- 用户在 Agent 执行期间发来的补充或纠正会持久保存并进入后续处理；尚未开始的旧操作可以按新要求调整，已完成的结果不会被忽略或无端重做。
- Agent 提问和权限确认有了独立等待状态。带标签、说明或标识的选项可与普通文字选项混用；对“可以”“继续”和选项字母等简短授权回复的理解更准确，但仍不会把 Agent 自己的说明当作用户授权。
- 工具失败会保留结构化的原因和可重试性；反复失败的同一能力会在当前运行中暂停继续调用，避免无限重复同一错误。
- 偶发的内部运行故障会进行有限次数的自动恢复。已完成的工具操作不会重复执行，用户取消、权限拒绝和明确不可重试的错误仍会立即停止。

### 子 Agent、任务上下文与跨对话协作

- 子 Agent 分为独立执行、多方讨论和内部汇总等用途，各自拥有独立的对话与持久状态，并能按成功标准、证据、完成、部分完成或阻塞状态汇报结果。
- 子 Agent 受工具次数、运行时间、费用、上下文容量和完成验收约束；主 Agent 取消或失败时可协调收拢子任务，恢复后不会重复注入同一任务。
- 移除了根据重复工具参数或结果判断“无进展”并强制收尾的机制。正常的工具发现、重复读取、轮询和等待不再被误停，旧检查点仍可恢复，已结束任务不会重启。
- 长项目中可以为不同目标创建独立任务上下文，支持暂停、恢复、卸载和重新加载；切换后不会把其他任务的用户消息、回答或工具结果混入当前输入。
- 暂停任务会按原顺序恢复用户消息与 Agent 回答；历史工具只保留必要线索，不会把旧结果当成待执行操作或重新导入大段输出。
- 当当前任务接近模型上下文容量时，Cyrene 会先保存未完成进度和证据位置，再在同一任务的新分段续接，无需用户重新描述目标。
- 过大的历史任务不会被强行整体塞回当前上下文，而是提供完整的只读快照供 Agent 按需分段查看，避免因一次加载过多资料挤掉当前工作。
- 可以记录跨任务的共同约定，例如命名、风格、边界和共享资料；它们与某一个任务的私有历史分开管理。
- 新增本地对话 Agent 通信：Agent 可发现正在运行的其他 Cyrene 主对话、本地对话和侧边对话，并向指定对话传递结果、上下文和后续工作。
- 空闲对话可在来信后继续工作；忙碌对话会在安全时机接收。消息按收件人保持顺序，能跨应用重启继续投递，暂时失败会有限重试，停用功能也不会删除已接纳消息。
- Agent 来信会显示发送来源，不会被当作用户授权、用户回答或可学习的用户记忆，也不会越过权限、未解决问题或确认流程。

### 插件、扩展与 Hook

- 全面更新插件框架。模型提供方、工具、应用服务、上下文、Hook 和界面集成可以以统一扩展形式提供，内置功能也遵循同一套启停、状态和生命周期规则。
- 新的插件中心集中展示已安装和可用扩展，支持搜索、分类、启用、停用、详情、删除、运行状态、启用工具数量、应用服务状态和是否需要重启。
- 插件包和包内工具可分别启停。即使所有工具都被关闭，包本身启用时仍可运行它的应用部分；更新或重装不会悄悄打开用户原本关闭的选项。
- 桌面端可从本地文件夹或 ZIP 直接安装插件。安装前会检查内容与路径边界、拒绝覆盖已有项，安装后明确显示加载、启用、运行、错误和重启需求。
- 加载失败的插件仍可定位、检查、修复或受限删除，而不会把目录中其他插件一起当成失败对象。删除入口拒绝符号链接和目录越界。
- 插件创建区和 Agent 创建流程使用当前版本指南，明确区分工作区草稿与已安装源码；支持生成、编辑、静态检查、审阅、安装和按扩展类型验证真实能力。
- 草稿在审阅后被修改时必须重新验证；错误的启动方式或未完成初始化的扩展会被明确拒绝，不会在界面中显示为已正常运行。
- 新增自然语言 Hook 创建与管理，可围绕会话、每轮开始、模型请求、工具前后、上下文变化、用量、提交和停止等事件配置自动动作，支持测试、编辑、启停、审核和运行记录。
- 系统 Hook 页能区分正在加载、尚未初始化、确实无记录和读取失败；失败时给出重试入口，不会用“没有 Hook”掩盖问题。
- MCP 服务、CLI 工具、用户安装插件和远程授予能力均能纳入统一工具发现与权限路径，不需要为每种扩展学习一套完全不同的使用方式。

### 模型服务、路由与回答兼容

- 模型设置改为统一的提供方、连接、模型配置和路由管理，可分别选择主模型、辅助模型、视觉模型和嵌入模型，并显示连接、能力和检查状态。
- 内置服务覆盖 OpenAI 及兼容服务、Anthropic、Gemini、DeepSeek、MiniMax、Kimi、GLM、OpenRouter、阿里云、AMD、Ollama 和本地 ONNX 等路径；也支持自定义服务地址、密钥、代理和模型 ID。
- 新增 Codex OAuth 连接与配套命令行运行环境，可在应用内完成登录、模型选择和用量检查；认证、配额或模型错误会给出可操作说明，不会悄悄换用其他服务。
- Cyrene 管理的本地模型和 Qwen3 Embedding 现在直接出现在统一连接列表中；Codex 与本地连接均可删除并在需要时重新添加。
- 模型发现、手动输入、列表选择和连接测试之间的状态更一致。模型 ID 可先完整编辑再确认，Escape 可放弃未提交修改，不会因中间输入状态被自动保存打断。
- 升级时会根据完整服务地址识别旧版连接，将其安全接入新的提供方体系，清理未使用的重复默认项，但保留密钥、自定义标识、模型配置和路由。
- 流式回答、思考内容、工具调用、回答用量和模型身份会保持完整关联；空回答、输出截断、工具参数不完整和损坏文字编码会进入明确的错误与恢复流程，不会悄悄保存为正常回答。
- 模型返回不完整工具参数或截断内容时，可在保留已成功结果和文件片段的前提下有限恢复，并在连续截断时主动缩小单次输出，减少整项任务重做。
- Anthropic 兼容服务不再默认限制为较短回答，已知 Claude 模型会使用与其能力相符的输出范围，用户主动配置的限制仍然优先。
- DeepSeek 思考模型在多轮对话中会保留继续会话所需的信息，普通回答后仍可在同一会话中使用工具；明确要求必须调用工具时会选择与该要求兼容的回答模式。
- 回答缓存和连续对话会正确区分不同模型、思考设置、工具、图片和上下文，避免把不适用的旧结果复用到新请求。
- 连接检查为需要先思考再回答的模型留出更合理的时间，图片能力检查使用有效的独立测试图片，不会把真实对话内容发给检查流程。

### Conversation、流式回答与多面板工作区

- Conversation 页面重构为统一时间线，按真实顺序展示用户消息、Agent 文字、思考、工具操作、进度、提问、权限、等待、失败和完成状态，减少重复卡片和跳序。
- 回答在生成期间实时显示，连续文字、Markdown、代码、工具卡和最终回答之间的交接更稳定。流式文字渐现和活动卡动画更平滑，快速更新长回答时不易闪烁或提前截断。
- 网络短暂断开后可以从上次位置继续接收，重复事件不会重复显示；较早到达的刷新不会覆盖更新画面，切换或关闭页面后也不会继续无效刷新。
- 失败、取消、完成和中断状态会稳定保留，迟到的后台消息不会将已结束运行重新变成“正在运行”。应用退出超时时会将未结束对话保存为明确取消。
- 对中断或失败运行重试时，旧运行会安全结束，新尝试保留原历史、已生成回答和已完成工具结果；失败的未完整回答只在新替代内容到达后才会被换掉。
- 侧边栏会区分正在运行、等待用户、等待权限、失败、取消、已完成和刚恢复等状态；新恢复的工作不再被旧问题或上一次失败状态覆盖。
- 新增分屏 Agent 工作区，可同时查看和操作多个 Conversation，支持左右或上下排列、尺寸调整、主侧交换、拆出独立窗口和合并回主窗口。
- 分屏中每个对话保持自己的消息、未解决问题、运行状态、滚动位置和展开状态；拖动面板、关闭侧栏或让分屏接替主对话时会保护当前阅读位置。
- 对话输入区可显式选择工作区、附件、MCP、Skill、当前分屏和顶部固定资源作为上下文，并支持斜杠命令、文件粘贴、编辑、重新生成、复制和附件操作。
- 主 Conversation、分屏 Conversation、Side Agent 和 Quick Chat 使用同一套消息、活动、顺序和完成状态；Workbench、Quick Chat、浏览器与桌面端共用同一运行数据，切换入口不会多一条、少一条或打乱顺序。
- 对话卡片支持置顶并保持稳定排序；对话分组的拖动顺序会立即保存，看板卡片的密度、状态和操作菜单也更统一。
- 每个 Conversation 可以单独关闭或恢复记忆学习，并查看相关记忆来源；删除、清空和导出对话时会同步处理对应上下文与记忆关联。
- 用量、模型、请求阶段、活动耗时和失败位置的展示更完整，便于区分本地处理、模型等待、工具执行和连接恢复。

### 编程、项目、文件与终端

- 编程能力整合了文件读取、写入、精确编辑、搜索、符号、代码分析、Git、构建、测试和预览。完整文件不再受旧的固定 8,000 字符写入上限影响，需要分段时仍会安全组装。
- 项目可按 JavaScript/TypeScript、Python、Go、Rust、Java、TeX、Make 和 GitHub 等类型获得对应操作，通用项目动作与项目特有操作在同一入口中展示。
- 新增持久终端和后台命令，可在切换对话、关闭页面或重启后继续查看输出和发送命令，支持大量输出、完整屏幕状态和历史回放。
- 新增受管理的 SSH 远程终端，可选择远程目录和 tmux 会话，一次认证后连接到可恢复的工作现场；短暂断网会尝试重连，主动退出或分离 tmux 不会被误判为掉线。
- Terminal 可识别 Codex、Claude Code、Gemini CLI、Kimi、OpenCode、Aider、Qwen、Copilot、Goose、Amp 和 MiniMax 等命令行 Agent，独立显示工作中、等待输入、完成、失败、中断与未读输出，重启后仍保留最近状态。
- Agent 可感知对话旁已打开终端的名称、左右位置和连接状态，因此能理解“当前终端”“左边终端”等指令，包括不属于当前对话的 SSH 会话。
- 命令会遵守用户或 Agent 明确指定的更长超时时间；超时或取消时会结束整个子进程树，避免后台进程继续占用输出或资源。
- 高负载终端的输出、持久化、回放、中英文、emoji、ANSI 颜色和控制序列处理更稳定，长历史的读取速度也得到改善，不会通过丢弃字符换取速度。
- 项目文件变更、审查面板、预览、终端和工具面板可以在主工作区、分屏或独立窗口中继续使用，并在布局切换时保留状态。
- 工作区最近项目、通用项目动作、项目类型识别和项目工具界面进行了统一，项目较多时的加载、状态刷新和操作排列更清晰。

### 浏览器、联网搜索与内容

- 新的浏览器工作区支持标签、导航、前进、后退、刷新、等待、截图、上传、下载、页面内查找、登录接管和用户与 Agent 共用页面。
- Agent 可以读取当前页面可见内容与可操作元素，再按可追踪的页面引用点击、输入、滚动和等待变化；对开放的复杂组件和嵌套页面也有更深的读取路径。
- 浏览器可以画中画、最大化、拆出或回到原工作区，并保留标签所属对话、当前地址和登录状态。
- 联网搜索支持多搜索器、结果来源与证据链接、结果预览和全文读取，并能在本地 SimpleXNG 、外部实例和其他已配置搜索路径之间工作。
- SimpleXNG 设置可逐个启停内置搜索引擎、查看当前可用数量并恢复默认选择；搜索顺序、来源和用户配置代理会被保留。
- 新增自定义 SimpleXNG 搜索源，可连接 JSON 或 HTML 结果页，配置 GET、POST JSON、POST 表单、结果字段、相对链接、内容类型和鉴权请求头；已保存密钥不会在界面中回显。
- 附件、大型工具结果、网页正文和分页内容可由 Agent 按需读取，避免将整份资料一次性塞入对话；证据来源和可继续阅读位置会被保留。
- 新增本地 HTML 交互预览，Agent 可以生成可操作页面并在 Workbench 中查看，本地资源访问受路径边界保护。

### 记忆、知识库、Skill 与主动工作

- 记忆系统整合了 SOUL 人格、短期记忆、项目记忆、结构化记忆、历史对话归档与召回，用户可搜索、编辑、停用、恢复和查看来源。
- 项目记忆有修订、冲突处理、失败重试、来源撤销和学习任务状态。处理过程会区分未触发、正常跳过、完成和失败，不再把“尚不需要保存”误报为错误。
- 用户可按 Conversation 关闭学习，主动工作的合成输入、Agent 间消息和非用户来源不会被误学成用户事实，减少系统自我强化。
- 知识库支持文档导入、解析、OCR、混合检索、文献元数据、集合、标签、笔记、批注、导入导出和 Zotero 本地导入，并改善了大资料库中的搜索和写入稳定性。
- Skill 不仅可安装和调用，还可从成功工作流中提取候选、合并、参数化、审批或拒绝，并支持版本、修订、回滚、激活、废弃、运行记录和成功失败统计。
- 学习到的工具链或脚本仍会经过参数检查和正常权限路径；高风险步骤或用户纠正不会被无条件自动重放。
- 新增可持久 Entity 管理，可跟踪任务、问题、决策、项目、事件和习惯等事项的状态、截止时间、优先级、关联与来源。
- 可选的主动工作只围绕已登记且仍活动的任务或问题，执行前会检查是否已完成、取消或被替代，并有冷却、去重、阻塞说明和可选静默完成。
- 日程任务支持单次、间隔和 cron 计划、时区、历史、暂停、恢复、取消与结果投递。从现有对话创建的任务会优先把结果送回原对话，不会每次都新建一个结果对话；重复投递不会反复追加同一内容。

### 消息渠道、通知与快速入口

- Telegram 和微信渠道接入与 Workbench 相同的 Agent、工具、上下文和结果投递流程，其启停与运行状态统一在所属插件中管理。
- 通知中心会在打开时刷新最新消息，可用 Escape、点击外部或离开页面可靠关闭；通知可返回相关对话，更新失败和后台任务也会保留可追踪的状态。
- Quick Chat 与主 Workbench 使用相同的 Agent 运行、历史和消息规则，适合从快速入口提问后回到完整工作区继续处理。

### 桌面操作、远程能力与跨设备工作

- App Use 可在 macOS 和 Windows 上读取并操作本机应用的可访问界面，支持查看、深入检查、点击、输入、选择、滚动和拖动；真实鼠标或焦点操作会遵守前台与权限限制。
- Linux 保留可用的语义访问能力，不会在平台无法验证的情况下宣称已完成视觉或全局输入操作。
- 远程能力支持设备配对、共享项目、文件传输、远程任务、远程 Conversation 与 Goal 控制，以及调用远程设备上已授权的插件。
- 远程桌面支持已配对设备的画面查看与控制、多显示器、音频和文件能力，并从实验开关改为可明确启用、检查状态和处理错误的功能。

### PowerPoint、图片、视频、音乐与语音

- PowerPoint 新增实时创作和 PPTX 文件两种工作方式，支持新建、打开、检查、编辑、添加页、图形、文字、图片、表格、原生图表、排版、渲染检查、撤销和保存。
- 演示文稿的对齐、选中、元素层级、布局读取和预览更准确，大型文稿不必为每次小修改重新传输全部内容。
- 新增图片生成与编辑，支持参考图、局部修改、变体、尺寸与透明背景等常见工作流，结果作为对话附件和后续素材使用。
- 新增视频和音乐生成任务，可在后台运行、查看进度、失败重试、完成后唤醒原对话并投递成果，不会因生成时间较长阻塞当前界面。
- 媒体设置可按图片、视频和音乐分别选择服务、模型和默认参数，密钥、代理和可用性检查与模型设置保持一致。
- 语音能力整合了语音识别、语音合成、麦克风输入、播放状态和语音设置，语音指令可直接进入当前 Conversation。

### Cyrene Doctor、故障恢复与安全修复

- 新增 Cyrene Doctor，可从设置、帮助、Conversation 失败、项目记忆和普通错误入口检查配置、模型连接、插件、对话运行、项目记忆与学习状态。
- 基础检查不依赖模型，会区分正常、发现问题、无法检查、正常跳过和信息提示，并显示依据、故障编号、发生阶段和关联对象。
- 诊断报告可持久保存、缓存刷新和导出 JSON；同类前端或运行故障会合并提示，不收集凭据、对话正文、请求正文或响应正文。
- 用户可选择让隔离的 Doctor Agent 基于已脱敏报告分析，并补充现象、触发步骤和近期改动。分析不继承 SOUL、记忆、Skill、MCP 或普通文件和终端工具，不会借诊断扩大操作范围。
- 模型分析支持停止和有限恢复；模型最终不可用时，已确认的基础证据和人工处理方向仍然保留。“测试模型连接”仅发送不含真实对话的最小请求。
- Doctor 可对选定的本地插件调查、在副本中复现、生成候选修复并展示完整差异、复现内容、验证结果和实际覆盖范围；生成阶段不会直接修改已安装源码。
- 用户确认后才会应用修复，应用前会再次检查用户审阅的方案、文件版本和应用状态。生成后被其他操作改过的旧方案会失效，避免覆盖新编辑。
- 应用、重载或验证失败时会尝试恢复原文件，并保留回滚入口和部分完成状态。候选修复限制在所选插件现有文件内，不会安装依赖、修改业务数据库或重放原 Conversation。
- 项目记忆的可修复问题可精确重试某一失败任务，沿用当时保存的原始证据和来源对话，不会用新聊天替换旧依据或批量重放已成功任务。
- 桌面 Python 后端启动失败、意外退出或超时时不再直接关闭应用，而是打开本地恢复页，可运行离线检查或重试启动。命令行也新增不启动主服务的离线 Doctor。

### 更新、备份、设置与数据安全

- 应用更新支持检查新版本、查看发布信息、显示下载进度、断点续传、安装包校验和重启更新，更新流量可单独选择是否使用 Cyrene 代理。
- 更新失败会根据网络、代理、服务器、磁盘空间、权限和文件占用给出更具体的说明，Windows 下次启动时会在通知中保留上次更新未完成的阶段、原因和日志位置。
- Windows 更新会等待主应用实际退出后再替换文件，必要时关闭终端服务并等待临时文件释放；最终失败时保留已下载安装包供重试。
- 新增应用数据备份与恢复，可在操作前检查包内版本、内容和可用空间，并在不覆盖新数据的前提下安全恢复；数据导入导出和清理入口统一到设置。
- 设置页统一管理语言、主题、字号、模型、媒体、浏览器、联网搜索、代理、通知、语音、地图、插件、工具、快捷键、数据、更新和 Doctor，各项保存、测试和错误状态更一致。
- 密钥、鉴权请求头和其他敏感设置采用只写或保留原值的交互，不会将已保存明文重新显示在界面、诊断报告或 Agent 上下文中。
- 通知中心、帮助中心、个人信息和版本页重新整理，可查看系统通知、更新错误、消费与请求、Token、连续活跃和当前/最新版本信息。

### Android 本地工作空间

- 新增单 APK 的 ARM64 Android 版 Cyrene，版本号与桌面端统一。安装后可在手机本地启动完整工作空间，无需另装运行时应用或连接电脑后端。
- Android 复用桌面 Workbench 的项目、模型配置、Conversation、Agent、工具、记忆、知识库、日程和设置，不再维护一套功能不同的旧移动界面。
- 启动页按准备资源、校验、解压、创建工作空间、启动环境和服务、准备工具、连接和加载界面等真实阶段展示说明与可计算进度，失败时提供重试。
- 首次启动会准备完整环境，之后会复用已验证资源与健康连接。短暂检查失败不会立即拆掉页面，同时进入应用的请求会共用同一次启动或恢复。
- 常驻通知新增用户主动触发的“休眠”。它会保存项目文件、本地服务、终端程序和未结束状态后停止后台环境，下次打开时恢复到保存工作现场。
- 恢复状态只使用一次，并与当前版本和资源绑定。升级、资料缺失或不匹配时会改用普通启动；休眠后又强制结束应用也不会回到旧时点覆盖后来文件。
- 新增 Android 原生应用内浏览器，用户和 Agent 共用标签、登录状态和 Cookie。用户可直接触控、用系统键盘输入和选择文件；Agent 可导航、读取、点击、输入、滚动、等待和截取当前视口。
- Android 浏览器在手机上始终使用全屏形式，保留返回、切换标签、刷新、静音和页面操作，不显示桌面画中画和无法在手机正常缩放的浮动窗口。
- 手机顶栏收敛为项目、居中当前对话和更多菜单；标签中心统一展示当前与其他对话及运行、等待、失败和完成状态。
- 手机默认只显示中间页面，左右卡片可从边缘或页面中间水平滑入，并保留文本输入、编辑器、终端、代码块和横向滚动内容自身手势。无右侧内容的页面不会划出空白卡片。
- 日程、看板、记忆、知识库、项目工具、终端、设置、Doctor、个人信息和版本信息均有响应式布局，输入框、浮动菜单、触控区域、深色侧栏和系统键盘适配进一步统一。
- Android 工作空间与手机之间的上传、表单、大请求、分段传输和实时双向连接更稳定；用户明确停止工作空间时，进行中的启动不会在稍后自行复活。
- Android 当前仍为实验平台：首次启动需要较长准备时间和较大可用存储，旧双应用实验版与不同镜像之间不自动迁移，桌面上的所有外部工具也不代表已在手机完整验证。

### 桌面窗口、界面与可用性

- Windows 和 Linux 使用一致的原生风格标题栏，可直接最小化、最大化、还原和关闭，按钮状态会跟随实际窗口变化；macOS 继续保留自身窗口风格。
- Linux 拆出面板恢复原生拖动、最大化/还原状态同步和合并回主窗口，浏览器、终端、预览和对话面板保留原有操作。
- 整体界面统一了浮动卡片、标题、页签、图标、按钮、表单、空状态、错误状态、深浅色和中英文排版，插件中心、设置、Doctor、日程、记忆、知识库、看板和个人信息的布局更一致。
- 通知、错误卡、帮助、模型检查、插件状态和后台任务的用语更具体，原有重试、复制详情、打开 Doctor、停止和导出等操作保留。
- 键盘焦点、Escape 收起、触控区域、窄窗口换行、长标题省略、文本选择和减少动画设置得到统一处理，不会为触屏优化牺牲键盘用户。

### 性能、并发与可靠性

- 长 Conversation 的历史与实时消息使用按顺序的快速合并，对重复 ID、关联消息、异常日期和乱序数据仍保留安全路径，不会为提速改变消息顺序或去重结果。
- 实时更新只刷新可能变化的运行记录，简单静态历史消息可复用已有显示；附件、跟踪、用量、模型信息和特殊卡片变化时仍会正常刷新。
- 展开思考或工具卡、拖动侧栏与分隔条、切换对话和连续接收回答时，远处历史减少重复排版，同时保留滚动锚点、选区、搜索、DOM 内容和动画。
- 不同对话的上下文和缓存读取不再因某一个慢加载相互阻塞；同一内容仍会共用一次加载，过期、关闭和失败重试的边界保持不变。
- 终端历史、调试事件和日志摘要查询更快，日志扫描不再长时占住其他对话处理；文件覆盖、轮转、删除和临时读取失败仍会正确使缓存失效。
- 对话、日程、统计、项目、记忆、知识库、事件和后台结果并发保存时更稳定，对读取、写入、关闭和迁移的协调减少了长时使用中的数据库占用与刷新失败。
- 应用启动、插件扫描、会话列表、应用状态、最近项目和未变化资源会减少重复读取与请求，较早返回的旧状态不会覆盖新状态。
- 上述性能优化不删除消息、不丢弃终端字节、不减少传入模型的信息、不降低数据持久性，也不通过隐藏历史节点、关闭动画或改变布局换取速度。

### 正式版平台与发布范围

- 0.9.0 统一发布 macOS、Windows x64、Windows ARM64、Linux 和 Android，共用版本号与本更新说明。
- 桌面发布包含 macOS DMG、Windows x64 安装包与便携包、Windows ARM64 及兼容组件、Linux AppImage、Debian 包和 RPM 包；Android 提供 ARM64 APK 与 SHA-256 校验文件。
- 发布只在版本、标签、主分支、完整 CI、各平台构建、安装、启动、基本使用验证和资产上传全部通过后才算完成；任一平台失败不会被标记为全部成功。

## [0.9.0-beta20] - 2026-09-11

beta20 为 Android 本地工作空间加入可由用户主动触发的完整休眠与恢复，并进一步收紧手机浏览器和 Workbench 的移动端呈现。重新打开应用时可以继续此前的本地环境、文件和终端状态；异常退出、升级或无效恢复状态则安全使用当前数据重新启动。DeepSeek 的思考模型与工具调用兼容性同步改善，输入区、深色侧栏、看板和手机消息操作也得到一轮视觉修正。

### Android 工作空间休眠与恢复

- Android 的常驻通知新增“休眠”操作。用户在当前任务完成后可以主动保存整个本地工作空间并停止后台运行环境；下次打开 Cyrene 时会恢复到保存时的工作状态，而不是重新经历一次完整冷启动。
- 休眠会保留本地项目文件、已经打开的本地服务，以及终端中正在运行的程序和未结束状态。恢复后，原有终端可以继续接收新命令，保存前已经写入的数据仍然可用。
- 恢复状态只使用一次，并与当前安装版本和工作空间资源绑定。应用升级、恢复资料缺失或不匹配时会自动改用正常启动，不会强行加载不兼容的旧状态。
- 如果应用在正常休眠后又被强制结束，Cyrene 会从当前磁盘数据重新启动，不会回到更早的休眠点覆盖后来保存的文件。无效或已消费的恢复状态也不会重复使用。
- 休眠完成后，本地环境会保持停止，不会在保存结束后悄悄继续执行任务；重新打开时会重新连接页面、校正时间并确认环境可用。
- 对不会变化的大型应用资源会复用已经验证过的结果，减少正常恢复时重复检查带来的等待；完整性和安全检查仍会在每次进入时执行。
- 切到后台不会自动休眠，原有的后台任务保持不变。休眠只由用户明确触发，进行中的联网请求仍建议先等待完成。
- 休眠、保存、恢复、回退到正常启动及失败状态均提供中英文可见提示，用户可以分辨当前是在保存环境、恢复环境还是重新启动。

### Android 浏览器与移动界面

- Android 原生浏览器现在始终使用全屏页面，不再显示桌面版的画中画窗口和“恢复窗口大小”按钮；返回、标签切换、刷新、静音和页面操作仍保留，避免手机上出现无法正确缩放的悬浮浏览器状态。
- 对话输入区上方的引导和操作浮层现在与输入框分别取景，半透明背景不会互相叠加或把下层文字映入输入框，亮色与深色主题的玻璃质感更一致。
- 深色模式中的对话、日程、资料库和记忆侧栏统一使用相同的不透明底色，切换模块时不再因下层页面颜色不同而出现侧栏色差。
- 看板左侧卡片为固定顶栏保留了正确间距，此前可能被顶栏遮住的搜索和工具控件可以完整显示，同时继续保持与其他浮动卡片一致的边距。
- 开启性能模式时，“滚动到底部”按钮使用清晰的实体背景，不再因关闭模糊效果而让对话文字透过按钮。
- 手机消息底部的时间、状态与操作按钮现在可以稳定换行；操作按钮保持一致的 32 像素触控区域，长状态或窄屏下不再互相挤压。

### DeepSeek 对话与工具调用

- 改善 DeepSeek 思考模型在连续对话中的兼容性。Cyrene 会在后续请求中保留 DeepSeek 继续当前对话所需的信息，减少多轮对话被服务拒绝的情况。
- DeepSeek 在使用工具时会按服务要求整理思考内容；普通回答后的下一轮工具调用也可以继续使用同一段对话，而不必重新开始。
- 当用户或 Agent 明确要求必须调用某个工具时，Cyrene 会自动使用与该要求兼容的回答模式，避免“强制工具调用”与思考模式组合导致请求失败。工具权限、选择结果和已有功能含义均未改变。

## [0.9.0-beta19] - 2026-09-11

beta19 让 Android 版从“可以打开桌面 Workbench”进一步变成可持续使用的移动工作空间：启动过程现在清楚展示当前阶段和可计算的进度，应用内新增与 Agent 共用页面的原生浏览器，并集中改善恢复、重连和长时间运行的稳定性。联网搜索新增按引擎选择和自定义搜索源；手机上的顶栏、侧栏、日程、看板、项目工具、输入区与诊断助手继续精修。桌面与 Android 仍使用同一套项目、对话、功能和设置，本轮修复不改变已有功能的入口与含义。

### Android 启动、工作空间与运行稳定性

- Android 启动页现在按“准备应用资源、校验资源、解压资源、创建工作空间、启动环境、启动服务、准备工具、连接服务、加载界面”等真实阶段更新说明；可以计算进度时同时显示百分比，不再让较长的首次准备过程只停留在笼统等待状态。
- 首次启动仍会准备完整的本地工作空间，后续进入会复用已经准备好的资源和运行中的健康连接，避免重复启动同一工作空间。资源校验、解压、磁盘创建和服务准备过程经过优化，减少不必要的重复读取与等待。
- 短暂的健康检查失败不会立即清空当前页面或拆掉仍可恢复的连接；后台真正失效时会自动执行一次协调后的重连，同时进入应用的多个请求会共用同一次启动或恢复过程，避免互相抢占。
- 用户明确停止工作空间时，仍在准备的启动过程会被取消且不会在稍后自行复活；重新进入后可以按正常流程再次启动。前后台切换、恢复和连续请求中的连接也更稳定。
- 改善手机与本地工作空间之间的上传、表单提交、连续请求和实时双向连接，较大的请求内容、分段传输及连续操作不再容易被截断或串到下一次请求。
- Android 默认项目统一显示为“Cyrene”，并自动修正早期实验版显示的内部目录名称；只修正默认项目的标题，不移动工作区、不改变项目身份，也不会重命名用户自己创建的项目。

### Android 原生浏览器与 Agent 共用网页

- Android Workbench 新增应用内原生浏览器。浏览器标签、地址栏、前进、返回、刷新、关闭、切换、最大化和页面预览沿用现有 Workbench 入口，无需另外启动桌面浏览器或依赖电脑上的浏览器服务。
- 用户可以直接触控网页、使用系统软键盘输入，并通过 Android 文件选择器选择网页要求的文件；用户与 Agent 查看和操作的是同一组页面，并共享这些页面的登录状态与 Cookie。
- Agent 可以在当前对话所属的浏览器标签中打开网页、读取页面文字与可操作元素、点击、输入、滚动、等待页面变化和截取当前视口。Agent 导航后页面会直接出现在 Workbench 中，用户手动操作后的内容也能继续交给 Agent 读取和处理。
- 浏览器标签会记录所属对话、地址和当前选择；Activity 重新创建后可按需恢复标签，返回仍在运行的应用时会继续复用现有页面。不同对话的标签保持分组，最多可保留 12 个标签。
- 网页导航限于正常的 HTTP 和 HTTPS 地址，并阻止本机地址、应用内部文件和其他高权限链接从普通网页中打开；页面内容不能直接调用 Workbench 的浏览器控制能力。
- 当前原生浏览器覆盖日常顶层网页操作。跨域内嵌页、复杂网页组件、Agent 自动上传与下载管理、整页长截图等桌面专用能力仍有边界；遇到不支持的操作会明确说明，不会假装成功。部分网站可能需要在 Android 中重新登录。

### 联网搜索与自定义搜索源

- “设置 → 联网搜索 → SimpleXNG”新增“搜索源”展开区，可以查看当前可用的搜索引擎数量，并逐个启用或停用内置引擎；也可以一键恢复默认选择，修改会从下一次搜索开始生效。
- 新增自定义 SimpleXNG 搜索源，可接入返回 JSON 的搜索接口或用 XPath 读取 HTML 结果页。用户可以填写名称、搜索地址、结果列表、标题、链接、摘要及相对链接前缀，并随时编辑、删除或单独停用。
- 自定义源支持 GET、POST JSON 和 POST 表单，可配置请求内容类型及搜索词位置；需要鉴权时可以添加 `Authorization`、`X-API-Key` 等请求头。已保存的密钥不会重新显示在界面中，可保留原值、替换或明确清除。
- 新增、编辑或删除自定义源后，本地搜索服务会自动重新载入；若重载未成功，界面会给出可见提示而不会把保存动作误报为完全可用。外部 SimpleXNG 实例仍需在对应服务器上管理搜索源。
- 搜索结果继续保留来源顺序和证据链接，并明确区分结果预览与完整正文读取；使用代理时保持用户配置的代理路径，不会被系统环境中的其他代理设置意外替换。

### 手机界面、设置与诊断助手

- 手机顶栏重新分配为固定宽度的项目图标、居中的当前对话和更多按钮；项目名称不再挤压对话标题，使窄屏上的中心位置和左右操作更稳定，项目切换入口仍保持原有图标和菜单。
- 左右卡片、底部导航和详情卡统一使用桌面浮动卡片的背景、圆角与阴影。右侧详情不再叠加两层边框或背景，底部导航也不再被额外容器裁切，展开后的层次更干净。
- 在手机设置左侧卡片中选择模块后，导航会自动收起并显示所选页面，减少每次切换后再手动关闭侧栏的步骤；页面切换或卡片内容频繁更新时，滑动和布局同步也更平稳。
- 日程页重新排列日期标题、今天、前后日期、日／周／月和新建按钮，按钮高度与间距更紧凑，长日期在窄屏上保持完整可读。看板搜索独占一行，工具与新建按钮分列下一行，不再互相挤压。
- 展开的项目工具在手机上使用更紧凑的标题栏和一致的返回、标题及操作按钮排列；终端等带额外操作的工具也保留完整按钮。按钮不会因浮动卡片移动而出现重复背景或裁切。
- 手机对话输入框的默认可视高度增加，空白输入时也保留足够的多行空间；输入区中的说明与操作行更加紧凑，方便查看和编辑稍长的请求。
- 诊断助手弹窗收窄并采用新的图标化标题、检查、停止、连接、导出、说明与详情入口；连接状态和失败进度更容易辨认，手机上的操作按钮保持足够触控高度。分析、修复、停止、导出和技术详情等原有能力全部保留。
- 模型设置中的模型 ID 可以先完整编辑，再通过 Enter 或离开输入框确认；清空、替换或输入中间状态不会被自动保存打断。按 Escape 可放弃尚未确认的修改，从列表选择模型仍会立即生效。

### 对话、后台任务与整体可靠性

- 对话实时更新改为按顺序合并处理；回复、活动状态和会话摘要连续变化时，较早的刷新不会覆盖较新的画面，切换或关闭页面后也不会继续执行无效刷新。
- 应用关闭超过等待时间时，仍未结束的对话会保存为明确的已取消状态；重新打开后不会显示成仍在运行。对中断运行执行“重试”时，会在保留原历史的前提下安全结束旧状态并开始新的尝试，避免旧运行在后台恢复后与新回复重叠。
- 已失败、取消或结束的运行继续保持终态；迟到的后台消息和排队结果不会重新激活它们，也不会阻止用户开始下一轮。结果保存发生局部异常时，已经完成的并行结果和已生成答案仍会保留，避免重复执行已有操作。
- 对话列表、上下文摘要和详情读取现在会复用未变化的数据，项目包含较多对话或较长上下文时，打开、刷新和实时更新更轻量；清除或导出对话时仍会包含对应的上下文内容。
- 日程、统计、项目和后台结果并发保存时更加稳定，降低长时间使用或 Android 本地服务繁忙时出现数据库占用、结果延迟或页面刷新失败的概率。
- 插件启动与检查减少对未变化文件和重复规则的读取；已禁用的选择、自定义设置和失败来源识别仍按原状态保留，不会因为启动优化而重新启用、覆盖或隐藏。

### 下载与预发布

- Android APK 现在与 macOS、Windows x64、Windows ARM64 和 Linux 一起进入同一个 GitHub 预发布流程，版本标签和发布说明完全一致，不再需要在独立发布位置查找 Android 构建。
- Android 下载同时提供对应的 SHA-256 校验文件；只有安装包、版本、来源和校验均匹配当前标签后才会发布。任一目标平台构建或上传失败，整次发布不会被标记为全部成功。

## [0.9.0-beta18] - 2026-09-10

beta18 将 Cyrene 的桌面 Workbench 带到 Android，并集中完善手机、小窗口和桌面之间的响应式体验。移动端继续使用熟悉的项目、对话、日程、看板、知识库、记忆和设置页面，通过手势展开左右卡片；同时统一了诊断助手与设置页的视觉规范，改善定时任务结果的回传、模型连接检查和系统 Hook 状态提示。

### Android 版 Cyrene

- 新增 ARM64 Android 实验版，应用名称统一为 Cyrene，版本号与桌面版一致。安装一个 APK 即可启动应用，不再需要另外安装运行时应用或连接电脑上的 Cyrene 后端。
- Android 直接复用现有 Workbench 与本地工作区体验，项目管理、模型配置、会话、Agent 和工具入口保持与桌面一致，不再提供另一套独立的旧版聊天界面、模型设置或更新页面。
- 启动时使用统一的 Cyrene 欢迎与准备页面；启动失败时提供重试入口，不再把启动、停止后端等实验控制面板作为应用主界面展示。
- 支持在 Workbench 中使用系统键盘输入，改善软键盘出现、收起时的可视区域和输入位置；文件打开、保存及外部链接继续通过 Android 系统入口处理。
- 安装包不携带开发者的会话或模型凭据，模型服务需自行配置。首次启动需要数分钟，并建议至少预留 12 GB 可用存储；旧双应用版本和不同实验镜像中的工作区数据暂不自动迁移，也不会主动删除。
- Android 当前仍为实验构建，桌面浏览器及部分外部工具的兼容性仍有待完善，不能将桌面上的所有工具可用性或运行速度视为已经在手机上完成验证。

### 响应式顶栏、项目菜单与标签中心

- 手机顶栏收敛为项目切换、当前对话和更多菜单三个入口，桌面仍保留完整标签栏。当前对话按钮以整个顶栏水平居中，不再随项目名称长短向一侧偏移；长项目名和长对话标题会自动省略。
- 点击项目名称打开项目列表，点击当前对话打开标签中心。手机上的标签中心包含当前可见及其他对话，继续区分异常状态和普通标签，方便切换正在运行、等待输入、失败或已完成的任务。
- 更多菜单集中提供搜索、帮助、通知和个人信息，统一图标、头像及文字对齐，去掉不必要的主题切换占位。点击功能后菜单会自动收起，帮助和通知的内容仍可继续操作。
- 项目编辑、记忆编辑和删除菜单改为真正的浮动菜单，展开时不再撑大项目列表，也不会被列表边缘裁切；弹出位置会适应可用屏幕空间。
- 修复移动端按钮按下时突兀的蓝色矩形高亮，统一顶栏、输入区按钮和浮动菜单的轻量点击反馈，保留键盘操作时的焦点提示。

### 左右卡片与滑动操作

- 手机默认只显示中间页面，左右卡片通过水平滑动展开，不再显示占据页面空间的“左侧卡片”“右侧卡片”“滑动收起”等专门操作栏。
- 除屏幕边缘外，也可以从页面中间水平滑动呼出卡片。设置页可以展开左侧导航；只有实际存在右侧内容的页面才能展开右侧卡片，看板或空知识库等页面不会再划出一层没有内容的阴影。
- 左右卡片展开后，上方顶栏和 Android 状态栏、底部系统区域会一起变暗；收起后恢复，避免遮罩只覆盖中间内容而显得割裂。
- 展开后的卡片边缘与主对话卡片对齐，收起与展开沿同一路径反向移动。快速改变方向时位置保持连续，底部导航也与左侧卡片同步移动，不再跳动或错位。
- 左侧工具区、记忆来源信息和底部模块导航之间不再保留重复占位；右侧对话面板根据实际内容呈现，减少没有内容却占满整屏的空白。
- 保留文本输入、编辑器、终端、代码块和可横向滚动内容自身的操作，减少与呼出侧栏手势的冲突；同时改善触控板、鼠标拖动和滚轮操作，避免滑动时反复出现无法阻止默认滚动的错误。
- 展开的卡片继续支持按 Escape 收起和键盘焦点切换，关闭后恢复原先操作位置。

### 日程、记忆、知识库与个人信息页面

- 日程工具栏在窄屏上分行排列日期导航、日／周／月切换和新建按钮，今天、前后日期按钮不再被挤掉或遮住。
- 记忆页保留完整的记忆搜索、类型与来源筛选、排序、分类统计、来源信息和技能学习入口；不再把诊断助手按钮固定插入正常工具栏。
- 记忆搜索框与筛选按钮适应手机宽度，避免横向溢出和文字拥挤；需要查看详情时保留对应右侧卡片，发生错误时仍可进入诊断助手。
- 个人信息页的消费、请求、Token、连续活跃及活动洞察卡片改为适应屏幕宽度的布局，不再靠横向滚动查看被挤出屏幕的卡片，长标签和数据也更容易阅读。
- 改善设置页面在手机上的整体宽度与滚动行为，减少内容超出屏幕边缘，同时保留左侧模块导航的滑动入口。

### 设置与诊断助手的统一体验

- 语言、主题、字号、地图来源和导出格式等分段切换控件改为随选项内容收紧，去掉右侧被拉长的空轨道；窄屏放不下时允许换行。
- “导入到当前项目”等独立操作按钮以及测试连接、保存按钮不再被拉满整行，按文字内容保持紧凑尺寸。
- 联网搜索设置去掉每个搜索器名称、说明和操作按钮之间的大块空白；排序、启用开关和需要填写的凭据紧跟所属搜索器显示。
- 版本信息在手机上排列为整齐的两列信息格，当前版本、最新版本、发布分支和发布时间之间使用细分隔线，移除突兀的粗灰色横带；长版本号允许换行。
- 移动端和桌面端诊断助手统一采用其他设置页的标题、表单、卡片和按钮规范，设置内页面与单独打开的诊断弹窗同时调整。
- 诊断助手移除额外的大图标标题区、重复内边距和特殊按钮配色，操作按钮按内容排列并可换行，检查结果和说明区域更加紧凑。
- “问题描述（可选）”只显示一次，文本框使用统一的输入与焦点样式；保留描述折叠、字数统计、连接检查、分析、停止、报告导出、修复及错误详情等原有能力。

### 后台任务、模型检查与系统 Hook

- 从现有对话创建的定时任务完成后，结果优先回到原对话，不再为每次完成结果另开一个新对话；原对话不可用时仍能创建结果对话，避免结果丢失。
- 定时结果与原对话正在进行的工作协调后再写入，保留当前会话状态；重复投递同一次结果不会反复追加相同消息，通知仍可定位到对应对话。
- 模型连接和能力检查为需要先思考再回答的模型留出更合理的响应空间，减少模型实际可用却因为检查过早截断而被误判失败的情况。
- 系统 Hook 页面区分正在加载、尚未初始化、确实没有记录和读取失败。遇到读取问题时给出明确提示和重新加载入口，不再把失败误显示为“没有 Hook”。
- Hook 列表加载失败时不再显示具有误导性的正常数量；查看已有系统 Hook 状态不会为了展示列表而额外启动一次 Agent 会话。

## [0.9.0-beta17] - 2026-09-09

beta17 重点改进了长任务和并行工作流的上下文管理：不同目标或重要子任务现在拥有彼此隔离、可独立暂停和恢复的对话记录，接近容量上限时也能保存进度并在新分段继续，而不会丢失原始资料或把其他任务内容混入当前工作。同时增强了模型临时故障、用户中途补充要求、失败任务与子 Agent 收尾时的恢复可靠性，并修复模型设置、通知中心、Conversation 侧栏和分屏 Agent 中多处会影响实际操作的界面问题。

### 长任务、上下文隔离与容量保护

- Agent 会把同一请求中具备独立进度、证据和决策的重要工作流分别管理，例如前端、后端或安全检查；紧密相关的说明、修正、测试和结果汇报仍留在同一工作流，不会把每个清单项都机械拆开。
- 切换工作流后，其他任务的用户消息、回复和工具结果不会继续混入当前模型输入；恢复时只还原对应任务的对话与资料，减少长项目中不同目标互相污染、错误沿用旧要求或泄露无关结果的情况。
- 当前用户请求在切换和恢复过程中始终保持原样可用。暂停过的任务会按原顺序恢复用户消息与 Agent 回复；历史工具调用只保留工具名称和截断后的参数提示，不会把旧结果重新当作待执行操作，也不会重复导入大段输出。
- 应用关闭再打开后仍能恢复正确的活动任务。较早版本留下的上下文记录也会按新的隔离规则读取，无需用户手工迁移；遗失的旧工具附件不再阻止恢复，因为这些结果不会被重新注入任务。
- 当当前任务接近模型可用上下文上限时，Cyrene 会在下一次模型处理前提醒 Agent 保存未完成进度和证据路径，并在同一任务的新分段继续。此过程是续接而不是完成任务，不会要求用户重新描述目标。
- 过大的历史任务不会被强行整体载入并挤占当前工作空间；加载会保持现有状态不变，并提供完整的只读快照供 Agent 按需分段读取。快照包含任务文档和必要的历史对话线索，但不会重新带入已丢弃的工具结果或完整大参数。
- 过大的跨任务共享资料会自动保存到可校验的只读快照，当前输入只保留其位置和使用提示。共享内容更新后会生成新的完整快照并保留旧证据；快照缺失或损坏时会从仍然有效的原始内容修复，不会静默隐藏约束。
- 容量检查、任务切换和深度反思只处理相关工作流；其他任务与共享约定保持不变。普通短对话或没有明确容量上限的模型仍按原方式运行，不会产生额外切换。

### 模型回复、重试与运行恢复

- 模型服务在尚未返回任何内容前遇到临时服务器故障时，会像网络瞬断一样自动进行有限次数的连接重试，并继续在 Conversation 中显示重试状态；明确的配置、权限或请求错误不会被无意义重试。
- 一旦模型已经开始输出内容，Cyrene 不会因后续连接错误自动重放整次请求，避免重复回复、工具调用或外部操作。所有重试仍有明确上限，失败后会正常结束并允许新一轮操作。
- 用户在模型思考或生成过程中补充新要求时，旧尝试的部分思考或回复会立即标记为已取消，界面不会残留一直闪动的“思考中”状态；已在运行的工具不会被误取消，新要求会继续生成修订后的答案。
- 已失败或已取消的运行现在是明确的终点。迟到的子 Agent 消息、排队结果或后台事件会保留在记录中，但不会覆盖失败结论、重新唤醒原任务或让 Conversation 再次进入处理中；用户发起的新运行仍可正常开始。
- 主 Agent 失败时会停止该轮仍在工作的子 Agent，不会等到迟到结果后把失败误显示成中间状态。重新打开 Conversation 时也会恢复到真实的失败或取消节点，而不是选择其后无效的后台消息。
- 失败后的“重试”在原运行尚未完全结束时会显示“等待运行结束”，运行中或已在准备另一项重试时会给出清楚提示，防止连续点击启动重叠请求；收尾完成后原有重试能力保持不变。

### 设置、通知与 Conversation 界面修复

- 模型设置中的模型 ID 下拉框现在会正确使用当前界面语言，搜索提示、加载状态、空结果和选择说明不再在中文界面中意外显示英文。
- 修复通知中心因缺少界面依赖而无法打开的问题。打开通知中心会刷新最新通知，按 Esc、点击外部或离开页面都能可靠关闭，并正确恢复被通知面板暂时遮挡的浏览器窗口。
- 修复部分 Conversation 页面、侧栏、分屏和顶部栏在特定入口下因缺少依赖而崩溃或按钮无响应的问题；未分类错误也会重新使用统一的中英文错误说明。
- 拖动 Conversation 分组后会立即保存新的顺序，并向辅助功能读屏清楚说明已移动的分组；重新进入项目后仍保持该顺序。
- 选中文本打开的分屏 Agent 现在始终绑定自己的 Conversation，文件、消息和实时状态不会错误引用主 Conversation 或另一个分屏 Agent。
- 新建 Conversation 时选择的 Agent 会正确保存到草稿；通知、分组移动、分屏提问和错误重试等既有入口在打包应用中均可正常使用。

## [0.9.0-beta16] - 2026-09-08

beta16 重点提高了长时间运行、工具执行、应用退出和异常恢复的可靠性：连接中断、插件局部失败或结果保存异常不再轻易让 Conversation 卡住，也不会在结果不确定时盲目重复操作。Agent 现在会在每轮自动遵循应用当前语言。Cyrene Doctor 能从本地运行记录中主动补齐诊断依据、调查实际插件并在重启后继续验证修复；诊断页面也改成更易理解的问题处理卡片。插件安装、启停、应用服务状态和源码校验进一步统一，同时保留原有功能、权限与操作方式。

### Conversation、工具执行与失败恢复

- 模型返回重复编号、缺少名称、参数格式错误等无效工具请求时，Cyrene 会先要求模型修正，确认请求完整后才执行；连续无效的返回会在有限次数后明确失败，不会执行半成品操作或无限重试。
- 单个本地插件主动取消自己的处理时，只会把该次工具调用标为可重试失败，不再连带取消整轮 Conversation。用户真正点击停止或取消任务时仍会立即终止当前运行，原有取消方式不变。
- 深度反思服务不可用、报错、返回空内容或自行取消时，会保留原来的任务上下文并让 Agent 继续处理或重试；只有用户取消整轮任务时才停止，不会因一次辅助整理失败而丢失工作材料。
- 必需的上下文、权限规则或系统指令准备失败时，本次运行会清楚结束并允许下一条用户请求重新尝试，不再停留在一直等待的状态，也不会在关键上下文缺失时继续调用模型。
- 工具已经运行但结果保存失败、输出无法正常保存或应用在关键时刻退出时，会保留“已执行但结果未确认”的真实状态，并阻止自动重复执行。用户可以先核对外部结果，再决定是否重试，降低邮件、文件、远程操作等被执行两次的风险。
- 并行工具中的单个结果保存异常不会丢掉其他已完成结果，也不会让整个批次重新执行。展示文件、资源预览或耗时统计失败时，也不会覆盖工具本身已经确认的成功或失败结论。
- Conversation 已经生成完整答案后，即使收尾处理、自动 Hook 或最终保存步骤失败，答案仍会保留并正常显示；重新恢复时只继续尚未开始的收尾工作，不会重复运行已经完成或结果未知的后处理操作。
- 运行层自身记录错误失败时也会让 Conversation 回到可继续状态；失败详情仍尽量保留，用户无需通过重启应用来解除一个永远不结束的运行。
- Conversation 在上一段回复后继续执行时，活动提示从“继续处理中”简化为“处理中”。上下文用量详情新增“任务上下文管理”分类，并把工具说明之后的普通行为规则正确归入“行为准则”，不再将不同用途的提示混在同一项里；这些界面文字均有对应英文显示。
- Agent 每一轮都会读取 Cyrene 当前的简体中文或英文设置，并将进度、提问、解释和最终回复保持为该语言；在设置中切换语言后下一轮即可生效，无需重新打开 Conversation。用户明确要求其他语言时仍以该次要求为准，代码、命令、路径、标识符和需要原样保留的引文不会被改写。

### 长任务、应用退出与本地 Agent 稳定性

- 关闭应用时会先停止接收新运行，并给正在生成的回复和正在提交的结果一个完成窗口；尚未确认的提交与可恢复运行会保留，避免关闭窗口后出现“界面结束了但后台状态不清楚”的情况。
- 浏览器页面、HTTP 请求或其他等待方中途断开，不再自动释放正在工作的 Conversation 或关闭它使用的 Agent 连接。再次使用相同运行可以恢复；新的冲突运行会被明确拒绝，而不是暗中取消原运行。
- 未设置超时的同步插件也会在独立工作线程中运行，避免长操作卡住 Conversation 主循环。等待超时或取消时会显示结果未知，并在原线程真正结束前阻止同一工作区中的同名插件再次执行，避免重叠操作。
- Bash 命令确认其进程树已经停止后，超时会作为可重试的运行失败返回；不能确认已经停止的同步插件仍保持结果未知，两类情况不再使用同一个含糊状态。
- 自动 Hook 的工作线程异常退出时，当前请求和排队请求都会收到明确失败；后续请求可以启用新的工作线程继续执行，不会被遗留的等待状态永久阻塞。
- 实时进度、工具活动或耗时信息的接收方变慢、断开或报错时，不再拖住实际任务，也不会把业务成功改写成失败。界面遗漏的临时进度会从已保存的 Conversation 状态恢复。
- 本地 ACP Agent 在输入管道拥塞、写入超时或请求取消时会停用已不可信的连接并清理等待项，不会在同一连接上继续发送可能重复的命令。长时间 Agent 请求仍可不设回复期限，但发送请求本身始终有明确等待上限。
- 本地 Agent 事件过多时会明确报告连接容量问题，不再悄悄丢弃较早的回复片段或权限请求；恢复历史时也会保留仍需要 Cyrene 回答的请求，减少 Agent 永久等待已丢失确认的情况。

### 模型连接、Agent 行为与主动工作

- 连续使用同一个模型连接时会复用已有网络连接，减少每次请求重复建立连接的等待；模型地址、密钥、代理或超时设置变化后会使用新的连接，正在进行的旧请求不会被中途打断，闲置连接也会自动关闭。
- 模型已经发出完整结束信号、但服务端迟迟不关闭数据流时，Cyrene 会保留完整回复并及时结束等待，不再让已经完成的回答一直显示为处理中；服务端正常结束时仍会读取必要的尾部数据，以便后续请求稳定复用连接。
- 工具发现步骤现在由 Toolbox 自己提供完整说明：Agent 会先按插件包的简短用途选择能力、读取当前输入要求，再进行调用；请求写错时会修正后重新发现，而不是把一次名称或格式错误当成整个功能不可用。
- 长文件的分段写入规则由写入工具直接提供，明确要求第一段覆盖、后续追加、在稳定边界分段并在完成前检查组合结果，降低中途覆盖前文或把不完整文件报告为已完成的情况；原有无固定字符上限的能力不变。
- 任务上下文指引经过统一，Agent 会静默地延续同一目标、在真正开始独立目标前保存当前工作，并按具体目标而非最近记录恢复任务。跨任务约定只保存有依据且确实共享的内容，普通致谢、临时话题和一次报告不会误触发切换。
- 主动后台工作进一步限制为清单中一个明确任务，只能做增量操作，不会修改、覆盖、移动、重命名或删除已有文件。没有实质结果时保持安静；有结果或明确阻塞时只发送一次简短通知，不会寒暄、猜测用户当前状态或暴露内部调度说明。
- 新创建的默认人格将 Cyrene 描述为“通用助手”，不再使用“不是助手”的互相冲突表述，同时仍保留朋友与陪伴者的目标。已有自定义 `SOUL.md` 会原样保留，不会因升级被改写。
- 插件创建指南进一步明确不再使用旧式激活入口，并把当前 Toolbox、写入和插件验收要求放在各自能力说明中，让 Agent 读取当前规则后再创建和验证插件；现有已安装插件的功能不会因此改变。

### Cyrene Doctor 调查、修复与界面

- 从失败消息打开 Doctor 后，会主动读取该次运行的终止状态、已保存事件、运行轨迹和限定范围内的本地日志元数据，补充用户已经拥有但此前没有展示的诊断依据。读取严格限定到对应运行，不发送完整 Conversation、工具参数、结果或日志正文。
- Doctor 能识别旧版本中仅以错误标记保存的失败节点，并把模型连接、普通运行故障和插件故障放入同一次检查。成功工具会合并显示，避免大量正常记录淹没真正的失败线索；缺失或超出读取范围的证据会明确说明。
- 新增只读的插件调查能力，可以列出内置与第三方插件的真实身份和当前状态，在限定范围内搜索相关源码并读取文件。Doctor 只有取得对应源码依据后才会选择修改目标，调查能力被停用时也会遵守用户的插件设置。
- 无法从最初错误直接定位插件时，Doctor 可以先调查再选择责任插件；若报告中已经有适用的恢复动作，会直接准备该动作供用户审阅，不会为了同一问题再生成不必要的源码补丁。
- 插件修复范围从 Python 扩展到所选插件中已有的 JSON、TOML、Markdown、文本、JavaScript、JSX、HTML 和 CSS 源码或配置文件，仍不创建或删除文件、不安装依赖，并继续限制文件数、内容大小和修改范围。
- macOS 安装版可以使用环境插件提供的现有 Python 运行隔离复现；找不到可用解释器或隔离执行器时会明确退回静态检查，不会自动安装 Python，也不会在未隔离环境中运行生成代码。
- 应用修复时会按源码位置识别实际 PluginPack，检查加载、初始化和启动结果。需要重启的修改会显示“重新打开应用后检查”，重启后可继续验证同一份修复而不重复写入；生命周期检查失败时仍会恢复原文件并保留原因。
- Doctor 页面把进度、用户可读结论、下一步和修复状态集中在同一张“问题处理”卡片中；问题描述默认收起，原始报告、错误记录和修复控制统一放入“错误详情（供排查使用）”。主要界面不再直接展示生硬的内部错误，连接、失败和重试说明也改为更清楚的中英文用户指引。

### 插件管理、安装与运行状态一致性

- 设置页面和 Agent 启停插件现在使用同一套增量更新流程：只修改用户选择的项目，并在保存成功后同步运行状态和通知界面。保存失败时不会让界面或内存先进入一个实际未保存的状态。
- PluginPack 总开关与其中各工具的开关继续分别生效。即使包内所有工具都被关闭，只要包本身仍启用，它提供的应用服务仍可运行；状态页会分别显示包是否启用、启用工具数量和应用部分是否正在运行。
- Python 插件源码或共享中英文目录内容变化都会触发正确的重新加载判断，减少修改说明或工具名称后界面仍显示旧内容的情况；没有实际变化的管理操作不会产生多余刷新。
- 插件安装、创建、设置页面和 Agent 工具共用相同的来源识别、静态校验、安装与删除规则，因入口不同而出现加载状态、重启提示或错误说明不一致的情况进一步减少。
- 插件的初始化入口必须在加载阶段同步完成；错误声明为异步、或表面同步却返回未完成任务的插件会在安装或加载时被明确拒绝，不再显示为已运行后留下不完整的服务。普通工具处理、Hook 和应用启动任务仍可异步运行。
- 从创建区安装插件前，会重新检查草稿是否在用户审阅后又被修改；发生变化时要求重新验证，避免把未经确认的新内容直接安装。加载部分失败时也不会发布不完整的应用服务或错误显示“正在运行”。
- 删除插件源码继续只允许实际管理目录中的顶层项目，并新增对符号链接和越界路径的一致保护，避免通过设置页、Agent 或 Doctor 的任一入口误删插件目录之外的内容。
- “调查插件问题”工具补齐中英文名称和说明，在活动记录与工具列表中会显示可理解的名称；Doctor 和插件相关提示也统一使用更贴近日常操作的“扩展功能”“应用设置”“检查连接”等文字。

## [0.9.0-beta15] - 2026-09-08

beta15 让 Cyrene Doctor 从“发现和说明问题”扩展到可审阅、可验证、可回滚的插件修复流程，并让失败消息能够携带准确的运行线索直接进入诊断。插件中心新增本地文件安装入口，Agent 创建插件的引导、安装状态、失败恢复和真实宿主验收也更加完整。与此同时，长回复、较大文件写入和长时间命令不再受隐含上限影响，异常模型输出会进行更稳健的分步恢复；长 Conversation 展开活动详情以及插件中心的布局和品牌图标也得到改进。

### Cyrene Doctor 诊断、生成修复与回滚

- Doctor 现在可以针对一个选定的本地插件调查问题并生成候选修复。用户可以填写现象和触发步骤，生成过程会读取所选插件及相关宿主规则，在副本中构造复现、尝试修复并验证，不会在生成阶段直接修改已安装源码。
- 修复提交前会展示完整文件差异、复现内容、验证结果和实际覆盖范围。界面明确区分“已应用”“已验证”“仅静态检查通过”“需要检查”“已回滚”和“部分完成”，避免把写入成功误报成原问题已经解决。
- 在 macOS 源码运行环境中，生成的复现会在限制文件访问、网络、运行时间和输出的隔离环境中执行，并使用同一份冻结复现对比修复前后结果。其他平台或无法执行隔离验证的安装环境会明确降级为静态验证，不会把未运行的检查显示为通过。
- 用户确认执行后，Doctor 会再次核对所审阅的方案、插件文件版本和应用运行状态，保留原始文件后再应用修复。插件在生成方案后被其他操作修改时，旧方案会失效，避免覆盖较新的编辑。
- 应用、重载或验证失败时会尝试恢复原文件；无法安全自动恢复时会保留清楚的部分完成状态和回滚入口。修复计划、原文件和执行记录会独立保存，关闭再打开 Doctor 后仍可查看，应用在关键阶段中断后也可以在文件仍匹配时继续回滚。
- 生成过程支持停止，并限制候选数量、持续时间、模型调用、读取文件数量和可修改范围。当前只会修改所选插件中已有的 Python 文件，不会创建或删除文件、安装依赖、修改业务数据库或重放原 Conversation。
- Conversation 的失败消息通过“Cyrene 诊断助手”进入 Doctor 时，会携带对应运行和故障线索，自动读取该次运行的有限结果信息，定位最后失败阶段；旧记录中以“已结束但结果为错误”保存的失败运行也能被识别。
- 直接诊断失败消息时，已成功执行的工具不会被重新运行。若能唯一确认责任插件并验证修复前失败、修复后通过，流程可以继续应用候选；只有静态检查结果时会停在人工审阅阶段，不会自动修改插件。
- 模型连接故障会转入连接检查，待回答问题或权限确认会提示用户处理而不会自动代答，无法确定责任插件时会保留诊断和证据缺失说明，不会尝试修改无关插件。重新打开同一故障会连接已有流程，“重新检查”才会启动新流程。

### 插件安装、创建与运行状态

- 桌面端插件管理页新增“从文件安装”，可以直接选择一个本地插件文件夹或 ZIP 压缩包。安装会检查压缩包路径和内容边界、拒绝覆盖已有插件，完成后刷新插件状态，并明确提示是否需要重启应用。
- 插件管理页打开时会自动刷新当前状态；安装按钮、加载提示、成功提示和需要重启提示提供完整中英文显示。原有插件中心、创建入口、启停、删除和详情操作继续保留。
- Agent 创建插件时会先读取当前版本的创建指南，不再从旧工作区示例推断协议。旧版 `plugin.json`、旧激活入口和旧工具索引会被明确识别为已废弃，并引导迁移到当前插件格式。
- 创建流程现在明确区分工作区草稿和已安装源码：草稿使用普通文件工具编辑，安装后才使用插件源码管理；完成静态检查和安装后，还会按插件类型验证实际工具调用、Context、应用服务、界面连接或模型能力，不再把模拟环境中的调用当成已完成验收。
- 输入不存在、不可用或缺失的工具名称时，工具发现入口会返回可纠正的提示，并允许 Agent 重新列出和选择当前工具；一次名称错误不再关闭同一轮中的整个工具发现流程。
- 插件安装结果会分别显示是否已加载、是否启用、应用部分是否正在运行、是否需要重启，以及设置或启动错误。既有的禁用选择会在重新安装时保留，不会因为安装成功而被意外开启。
- 一个插件安装失败时，会将目标插件的问题与目录中其他插件的问题分开显示。失败源码仍可被定位和修复，也可以通过受限的失败项删除入口清理后重新安装；该入口只接受实际加载失败的顶层插件，不会删除其他文件。
- 已安装插件的源码管理可以正确定位可编辑的核心工具，并在普通应用插件改动需要重启时返回真实状态。插件管理列表也会持续显示应用运行、重启需求和启动失败信息，减少“已启用但实际不可用”的误判。
- 插件创建指南补充了工具输入、持久数据、调用已配置模型、并发与超时、权限审核、源码变更确认和不同贡献类型的验收方式，让 Agent 能更可靠地完成创建、安装、调用和失败恢复的完整闭环。

### 长任务、模型回复与命令可靠性

- 文件写入不再有固定的 8,000 字符工具上限；能够放入一次模型输出的完整内容可以直接写入，确实需要分段时仍会按首段覆盖、后续追加的方式安全组装。
- 通过 Anthropic 协议连接模型时，不再对兼容服务默认施加 4,096 token 的回复上限；已知 Claude 模型会使用与其能力匹配的输出范围，用户主动配置的上限仍然优先，减少长回复和大型工具参数被过早截断。
- 模型返回不完整工具参数、空响应或输出截断时，恢复尝试由一次增加到最多三次。恢复会要求重新生成完整调用，并在连续截断时逐步缩小单次内容、保留已经成功的工具结果和文件片段，降低重复写入或整项任务中断的概率。
- 命令工具会遵守用户或 Agent 明确指定的更长超时时间，不再被另一层固定时限提前终止。命令超时或任务取消时会结束其整个子进程树，避免后台子进程继续运行或占住输出管道。
- 权限审核现在可以结合用户对上一条方案的简短回复理解“可以”“继续”或选项字母等上下文，同时仍把助手自己的说明与真实用户授权区分开，减少已获同意操作被误拒或助手自称授权被误放行的情况。
- 失败运行和权限审核在活动时间线中的状态更加一致：拒绝的权限审核会显示为失败，旧式“状态为结束、结果为错误”的运行会按错误处理，失败诊断也能关联到实际发送的用户消息。

### Conversation 性能、插件中心与界面细节

- 长 Conversation 展开或收起思考、工具调用和活动分组时，消息列表减少对完整历史布局的重复计算；主聊天和分屏聊天都会受益，消息宽度、间距、空白页和加载状态保持原样。
- 插件中心页面采用更适合阅读的内容宽度，页签、标题、返回按钮、卡片、图标、文字和操作区更紧凑；宽屏能容纳更多内容，窄窗口与触控设备仍保留易操作的按钮尺寸和自适应排列。
- Python、Node.js 和 uv 使用经过校正的品牌图标与颜色，避免图形被统一填色后出现缺失或形状错误；长插件标识可以正常换行，空操作区不再占据额外空间。
- 本地模型服务名称、核心工具名称、插件安装与 Doctor 修复流程补齐并统一中英文显示，模型设置、插件中心和故障处理界面的文字在切换语言后更完整一致。

## [0.9.0-beta14] - 2026-09-07

beta14 改进了应用更新、Windows 联网搜索，以及长 Conversation 的面板调整和活动详情展开体验。更新现在可以单独选择使用代理，失败时提供更具体的原因；Windows 更新会等待应用真正退出并处理文件占用，联网搜索也修复了启动和意外退出问题。Agent 提问选项兼容性、聊天状态刷新、图片能力检测和异常模型回复处理同时得到修复，现有聊天、分屏、工具、模型设置与桌面操作方式保持一致。

### 应用更新与代理设置

- 设置的代理区域新增“Cyrene 更新”开关，可以单独决定检查版本和下载更新是否使用已配置的代理，适用于无法直接连接更新服务的网络环境。搜索、浏览器和插件集成仍可分别选择自己的代理范围。
- 更新代理设置从下一次请求开始生效，不会中断正在进行的下载；原有更新检查、下载进度、断点续传、安装包校验和“重启更新”操作继续保留。
- 检查更新失败时，关于页面会显示实际失败原因；连接超时、网络连接失败、服务器错误以及更新包权限或文件操作失败，会提供对应说明，方便判断应检查网络、代理、磁盘空间还是目录权限。
- Windows 更新安装失败后，下次启动会在通知中心提示“上次更新未完成”，并保留失败阶段、原因和日志位置。同一次失败不会反复通知，查看失败提示也不依赖重新连接更新服务器。

### Windows 更新、启动与联网搜索

- Windows 的“重启更新”会等待应用实际退出后再开始安装，避免关闭确认尚未完成时安装器就开始替换文件。应用未能及时退出时会停止本次安装，而不是继续覆盖正在使用的程序。
- 更新时会在主窗口退出后关闭 Cyrene 的终端服务，减少安装器仍提示应用正在运行的情况。更新会结束终端中运行的命令；普通关闭应用时保留终端的原有行为不变。
- Windows 便携版更新遇到程序退出清理或临时文件占用时，会等待并重试替换；最终失败时保留下载的更新包，便于重试。
- 调整 Windows 启动和更新时后台程序的窗口处理，减少不必要的命令行窗口闪现，正常终端和需要用户确认的安装授权仍然保留。
- 修复 Windows 内置联网搜索的兼容问题，避免搜索启动后因检查主程序是否仍在运行而导致主程序意外退出；修复 ARM64 版本搜索组件能够加载却无法实际启动搜索，以及启动后误判主程序已退出而自行关闭的问题。停止搜索服务时会一并结束其启动器与计算子进程，避免重启后遗留后台搜索进程。桌面安装包中的搜索启动错误也能保留日志供排查。
- Windows 搜索继续提供计算器答案，包括算术、比较和数学常量；计算耗时过长时仍会停止该次计算，不会为了恢复网页搜索而关闭这项能力。

### Conversation、分屏与界面细节

- 长 Conversation 展开或收起思考、工具调用和活动分组时，只刷新相关卡片及关联分组，减少大量历史卡片一起刷新的卡顿；同一内容在多个分屏中的展开状态仍保持同步。
- 思考和工具活动展开、收起的动画期间，Conversation 导航暂缓重复测量，动画完成或取消后恢复更新，让长聊天中的活动详情切换更平稳。
- 修复 Agent 提问时带有标签、说明或标识的选项可能被拒绝的问题，支持普通文字选项与这类选项混合使用，界面继续显示清晰的选项文字。
- 长 Conversation 拖动右侧面板宽度、调整分屏列宽或上下分屏比例时，减少远处历史内容的重复排版。主聊天和分屏聊天都会保护当前阅读位置，正在跟随最新回复时继续保持跟随。
- 关闭面板并让原来的分屏 Conversation 接替主聊天时，会沿用布局调整前的历史内容尺寸，减少长聊天切换过程中的集中排版与跳动。
- 持续拖住分隔条、拖动过程中切出窗口或取消操作时，聊天内容会按对应操作恢复正常显示；滚动、选择文字、键盘操作和编辑内容仍可正常使用。
- 调整收起侧栏后底部快捷区域的间距和对齐，使其与侧栏边缘及底部留白更协调。
- 修复聊天页面和命令行查询应用状态时遇到状态接口不存在的问题，模型、会话消息数等状态能够正常读取与刷新。连接检查也能正确区分 Cyrene 服务、其他本地服务和已经失效的登录凭据。
- 会话列表与应用状态的实时刷新减少重复请求；较早返回的数据不会覆盖更新后的状态，状态读取失败时仍保留已经成功取得的会话列表。

### 模型连接与回复可靠性

- 修复图片能力检测使用的测试图片无效，导致部分实际支持图片的模型检测失败的问题。检测继续使用独立测试内容，不会发送真实 Conversation 中的图片或正文。
- 模型流式回复包含损坏的文字编码时，会明确识别为响应格式异常，并交给现有错误提示和恢复流程处理，避免将损坏文字悄悄当成正常回复保存；正常中文、跨段传输的文字和不同换行方式仍然支持。
- 修复聊天耗时信息提交被拒绝的问题，使性能诊断能够正常记录对应阶段，便于区分本地处理和模型等待耗时。

## [0.9.0-beta13] - 2026-09-07

beta13 新增了 Cyrene Doctor，让配置、插件、Conversation 和项目记忆出现问题时，有一个统一的检查、说明和安全恢复入口。即使 Agent 或模型暂时不可用，基础检查仍然可以运行；桌面端后端无法启动时，也会打开独立恢复页，而不是只显示错误后退出。错误提示现在能带着对应证据直接进入诊断，项目记忆失败可以按原始资料精确重试。与此同时，超长 Conversation 的首次显示、活动详情展开和侧栏动画进一步减少卡顿，并继续保持原来的阅读位置和操作状态。

### Cyrene Doctor 与诊断入口

- 设置中新增“Cyrene Doctor”页面，可以检查当前应用的配置、插件、Conversation 故障和项目记忆状态。检查结果明确区分正常、发现问题、无法检查、正常跳过和信息提示，不会把尚未完成的检查误报为一切正常。
- 帮助与支持页面也新增了醒目的诊断助手入口；在设置、帮助中心和弹窗中使用完整的中英文文案、独立的玫红色标识及适配窄窗口的布局，更容易从当前工作位置直接开始排查。
- Conversation 运行失败、项目记忆或学习页面出错，以及普通错误提示出现时，会提供 Cyrene Doctor 入口并自动带入相关项目、Conversation 或故障线索；原有的重试、复制详情和其他操作仍然保留。
- 基础检查不依赖模型，Agent 无法调用、模型未配置或网络不可用时仍能给出已经确认的结果和下一步方向。Doctor 不会为了诊断而读取完整 Conversation 正文、记忆正文、凭据或错误中的敏感内容。
- 每项结果可以展开查看对应依据；报告会保存独立编号，也可以导出为 JSON，方便保留当时的检查状态。报告无法写入磁盘时，当前基础结果仍会显示，并明确提醒先处理存储问题。
- 检查完成后会先显示需要处理的问题数量、通过数量和总数，待排查项目直接展开，其余正常或提示性结果集中收起；报告编号等信息也放在独立详情中，主要结论更容易浏览。
- 同一项目或 Conversation 的诊断结果会在当前应用会话中短暂保留。再次打开时可以立即看到已有结果并在后台刷新；从帮助中心准备打开 Doctor 时会提前加载，重复点击也不会创建多份相同检查。不同 Conversation 和中英文结果仍彼此分开，诊断依据不会写入浏览器长期存储。
- 配置、插件加载、对话运行、模型错误、记忆处理阶段和项目学习任务现在可以在同一份报告中关联查看，避免所有问题都只显示成笼统的“Agent 运行失败”或“记忆保存失败”。

### Agent 分析、修复与离线恢复

- 用户可以在基础报告上选择“让 Agent 分析”。分析只使用当前报告中的诊断依据，不继承用户的 SOUL、记忆、Skills、MCP 或普通文件和终端工具，也不能自行执行工作操作或扩大修复范围。
- 在让 Agent 分析前，可以选择填写问题现象、触发步骤和近期改动。说明会与已脱敏的检查结果一起提供给模型，并与已经确认的依据明确区分；输入区提示不要填写密钥，并限制长度，避免一份正常基础检查掩盖用户实际遇到的问题。
- Agent 分析支持停止；模型响应中断、格式异常或超时时会在有限次数内自动恢复，并在界面显示恢复进度。已有依据会保留，不会重放导致原问题的工具操作；模型最终仍不可用时，基础报告和人工处理方向不会丢失。
- “测试模型连接”会发送一次不含真实 Conversation 内容的最小请求，并明确提示可能产生少量用量，便于区分模型连接问题和其他应用故障。
- 可修复的问题会先显示具体修复预览，再由用户决定是否执行。当前支持恢复单个内置插件的发行版文件、清除单项工具名称或可见性等覆盖，以及精确重试一项失败或冲突的项目记忆学习任务。
- 插件和配置修复会保留原状态用于回滚，执行后自动复检；目标在预览后又被修改时会拒绝使用过期方案，避免覆盖新的用户改动。修复只在相关运行空闲时进行，活跃 Conversation 或记忆学习不会被中途打断。
- 项目记忆重试沿用失败任务当时保存的原始证据和来源 Conversation，只重试该任务，不会用新的聊天内容替换旧依据，也不会批量重放其他任务或已经成功的学习。
- 桌面端 Python 后端启动失败、意外停止或启动超时时，不再立即退出应用，而是打开独立的本地恢复页。用户可以运行离线检查或重试启动；即使 Python 本身不可用，页面也会保留安装完整性、运行文件和本地日志的排查方向。
- 新增 `cyrene doctor --offline` 命令，可在主服务和自定义插件未启动时执行只读检查，并支持指定项目或 Conversation、切换中英文以及导出结构化结果。

### 错误定位与项目记忆可靠性

- Conversation 和项目记忆中的模型失败会继续保留鉴权、额度、限流、网络、超时、上下文过大、响应不完整等真实分类，不再因为经过其他运行层而退化成无法判断的普通错误。
- Conversation 运行、后台插件和公共接口发生故障时，会记录不含内容的故障编号、发生阶段和关联对象，错误卡和 Doctor 可以用同一个编号定位证据；主动取消、外部请求和 Doctor 自身请求不会反复生成诊断提醒。
- 前端网络错误、接口失败和未处理的界面异常会提供持久的 Doctor 操作，同类问题会在短时间内合并。记录不会收集网址、请求或响应正文、异常正文、凭据和 Conversation 内容。
- 项目记忆现在会记录一次处理处于已触发、取得资料、正常跳过、完成或失败中的哪一阶段，并区分记忆写入关闭、内容不适用、阈值未达到和缺少已提交资料等正常原因，减少把“尚未触发”误判成“保存失败”。
- 项目记忆学习失败会保留更准确的模型原因；任务被中断或来源已移除时也会使用更符合实际的说明。已有 Conversation 正常使用较早的冻结记忆快照时只作为信息提示，不会建议清空或强制替换记忆。
- Doctor 的检查和修复不会降低现有数据校验、模型权限或工具权限，也不会猜测重建损坏数据库、丢失的密钥或已经不存在的历史学习资料；无法安全恢复时只给出明确处理方向。

### 超长 Conversation 的显示与侧栏动画

- 已收起的历史思考和工具活动不会在 Conversation 首次显示时立即构建全部详情；用户第一次展开后再加载，并在之后保留正常的展开与收起动画。大量工具输出的旧 Conversation 打开更快、占用更少。
- 展开或收起侧栏前，会先依据变化前的稳定布局保护屏幕外内容，减少动画开始时对整段历史进行一次额外排版；从菜单重新显示右侧面板时也使用相同处理。
- 单条跨越很多屏幕的长回复现在可以只暂时简化远离视口的段落、代码块和表格，而不必冻结整条可见消息；正在生成的内容、选中文字、编辑区、重试入口和画中画避让仍保持实时可用。
- 超大 Conversation 在侧栏动画结束后会分批恢复屏幕外内容，不再把成百上千条消息集中到同一帧处理。恢复期间会持续保持当前阅读段落或最新消息跟随位置，避免突然跳动。
- Conversation 导航和定位信息会在侧栏布局尚未完全恢复时暂停重复测量，恢复完成后再一次更新；现有消息顺序、折叠状态、浏览器画中画位置和用户布局选择保持不变。

## [0.9.0-beta12] - 2026-09-06

beta12 主要改进了图片与截图处理、视觉模型自动切换，以及长 Conversation 调整侧栏时的流畅度。图片请求不再因为传输时的文件体积被误判为超出模型容量；专用视觉模型不可用时，可以在安全范围内自动尝试当前 Conversation 已选择的兼容模型。长聊天展开或收起侧栏时会减少无关历史内容的重复排版，同时保持阅读位置和浏览器画中画布局。任务上下文的内部保存与切换也会更安静、更准确。

### 图片、截图与视觉模型

- 发送图片或截图时，Cyrene 会根据图片的实际尺寸和模型能力判断所需容量，不再把图片在传输过程中的编码体积直接当成上下文占用，减少明明可以处理却被提前拒绝的情况。
- 已配置的专用视觉模型不可用、容量不足、没有返回有效内容或处理失败时，Cyrene 可以自动尝试当前 Conversation 中兼容且支持图片的模型，避免一次视觉模型故障直接中断整项任务。
- 自动切换会优先使用当前 Conversation 已选择的主模型，并只在同一模型服务中选择确实支持图片的候选；纯文字模型不会收到图片，现有模型选择、服务设置和权限不会被改写。
- 尝试视觉模型前会同时考虑图片、文字和回答所需空间。容量不足的候选会被跳过，但不会阻止容量更大的兼容模型继续处理，降低长提示词、多张图片或高分辨率截图造成的无效重试。
- 如果没有任何兼容模型能够安全处理请求，Conversation 仍会明确显示失败原因，而不是悄悄更换到不符合当前设置的模型或给出看似成功但缺少图片理解的回答。

### 长 Conversation、侧栏与浏览器画中画

- 在很长的 Conversation 中展开或收起侧栏时，暂时离开屏幕很远的历史消息不会参与每一帧的重复排版，侧栏动画、消息宽度变化和输入区移动更加顺滑。
- 侧栏变化结束、被取消、超时，或用户开始滚动和操作消息时，暂时简化的历史内容会立即恢复；恢复后仍保持原来的消息高度和阅读位置，不会突然跳到别处。
- 正在查看最新回复的用户会继续跟随最新内容；停留在较早消息的用户会保持当前阅读锚点。文字选择、正在编辑的内容、重试入口以及需要避让画中画的消息在侧栏移动期间仍然可以正常使用。
- 使用浏览器画中画时，展开或收起侧栏会保留用户设定的画中画高度和垂直位置，不再因为 Conversation 重新排版而改变窗口大小或上下漂移。
- 浏览器侧卡片隐藏后不会继续把画中画推离原位；历史消息恢复完整布局后，Cyrene 会重新计算必要的避让范围，避免画中画遮住消息或留下多余空白。

### 任务上下文与进度沟通

- Agent 保存、加载或暂停任务上下文时会保持安静，不再主动展示内部编号、状态和整理过程；用户要求查看时仍会提供这些信息，正常的工作进度更新也会继续显示。
- 完成工作、汇报结果、等待下一步或只是保存当前进展时，不会为了内部整理而卸载正在使用的任务上下文，减少同一任务被无意义地暂停和恢复。
- 恢复任务时会按用户要完成的具体目标寻找对应资料，而不是默认采用最近保存的内容；无关任务和仅用于占位的空资料不会被误加载。
- 调研或说明结束后，如果用户原本的实现任务仍需继续，Agent 会回到该任务完成后续工作；用户明确要求拆成多个工作方向时，各自的进度和资料会分开保存，只有真正共同的约定才会共享。

## [0.9.0-beta11] - 2026-09-06

beta11 主要让长 Conversation、后台主动工作和模型异常恢复更稳定、更容易理解。回复生成时界面不再带动无关区域反复刷新；思考、工具活动和最终回答在保存、恢复或重新连接后不会重复出现。后台主动工作只会处理用户明确记录的任务，不会从记忆或旧聊天中擅自猜测要做什么。模型输出被截断、没有完整收到或由服务商明确拒绝时，也会显示更准确的原因和下一步建议。

### Conversation 实时内容与界面流畅度

- 长回答逐字生成时，只有实际变化的消息区域会更新；输入框、浏览器、侧栏和其他相邻面板不会被每一小段文字带着重新刷新。快速输出、长时间思考和分屏使用时的闪动、卡顿及布局抖动更少。
- Conversation 侧栏展开或收起时，消息区域、输入区和相关面板使用一致的移动时长与节奏，不再出现一部分已经停下、另一部分仍在移动的脱节感。
- 拖动分屏的横向或纵向分隔线时，界面会直接跟随指针预览尺寸，并在松开后一次保存最终布局。连续拖动不再反复刷新整条 Conversation 或频繁改写设置，长聊天中的分屏调整更加顺滑。
- 没有使用浏览器画中画时，侧栏尺寸变化不会反复检查整段聊天记录或改写消息布局；使用画中画时仍会只移动需要避让的消息，关闭画中画后正确恢复阅读位置和最新消息跟随状态。
- 正在运行的思考与工具活动从实时显示到保存、刷新和恢复都会沿用同一个身份。界面不会因为一项活动同时存在实时版本和已保存版本，就显示两张内容相同的活动卡片。
- 回答等待用户确认后继续、页面中途重新加载，或 Conversation 从运行记录恢复时，会使用同一条完整时间线。已经完成的工具、最终回复和等待确认前后的内容不会被再次追加一份。
- 历史数据中已经存在的重复活动或重复回答会在展示时合并为一条，并优先保留信息更完整、归属更准确的版本；用户确实连续执行的不同工具调用、重复发送的独立回答和不同用户轮次仍会分别保留。
- 实时活动、保存后的历史、重新连接的页面和公开 Conversation 数据现在使用一致的排序与去重结果，消息数量也与用户实际看到的内容保持一致。

### 模型响应、错误提示与自动恢复

- 模型达到输出上限、模型响应没有完整传完，以及模型返回无效内容现在会被区分为不同问题。Conversation 会显示对应的中英文标题、原因和处理建议，不再把这些情况统一归为模糊的“无效响应”。
- 模型服务明确返回失败时，会尽可能区分服务繁忙、请求过快、额度不足、密钥无效、请求内容过长、请求格式不被接受或模型不存在，并显示与真实原因一致的提示。
- 模型已经完整结束回答、但连接在最后关闭得不够正常时，Cyrene 会保留已经确认完成的正文、工具操作和用量信息，不再把成功回答误报为网络中断。
- 如果连接在模型完成前中断，或服务明确表示响应仍不完整，Cyrene 仍会按失败处理并提供重试，不会把半段回复或不完整操作当成成功结果。
- 模型刚好在输出上限处给出了一项完整、有效的工具操作时，该操作可以正常继续执行；只有参数确实不完整时才会停止，并进行一次有边界的修正或重试，避免丢弃有效工作或执行被截断的内容。
- 因输出过长而需要重试时，Agent 会获得把内容拆成更小步骤的明确提示；同一问题不会无限重试，现有工具权限、确认要求和实际功能保持不变。
- 写入较长文本时，不再因为内容超过建议的单次长度而在开始前直接拒绝。完整内容可以原样保存；Agent 仍会优先采用更稳妥的分段方式处理特别长的文件。
- 模型连接结束时的清理异常不会覆盖已经确认的成功结果、真正的模型错误或用户取消操作，最终显示的状态更接近实际发生的情况。

### 主动工作、提醒与任务来源

- 后台主动工作现在只处理当前项目中由用户或 Agent 明确记录、仍处于活动状态的“任务”或“问题”。记忆、旧 Conversation、知识资料、项目文件和推测出的兴趣只能作为核实资料，不能自行变成新的后台任务。
- 用户在界面中创建或通过工具明确追踪的任务会被标记为可处理的正式事项；自动整理出的候选信息不会在未经明确确认时触发主动执行或通知。
- 有多个正式事项时，会优先考虑临近截止日期和较高优先级的内容，并始终限制在当前项目范围内，避免一个项目的任务被带到另一个项目中处理。
- 开始主动工作前，Cyrene 会结合最近的公开聊天和当前项目状态核实所选事项。如果任务已经完成、被替代、取消或不再适用，会更新其状态并保持安静，不会继续执行或发送过时提醒。
- 主动工作完成后，对应事项会标记为完成；确实需要用户补充信息时会暂停该事项，并只发送一次具体的阻塞说明，而不是反复发出笼统问候或催促。
- 同一个没有变化的任务版本只会评估一次。用户没有回复并不会让旧任务在每次后台检查时重新出现；任务内容、状态、截止日期或优先级发生变化后，才会作为新的状态重新判断。
- 后台检查使用的系统请求和自身输出不会写入短期记忆、项目记忆或对话归档，避免一次主动检查成为下一次检查的依据，形成重复提醒或自我强化。

### 任务上下文与共同约定

- 任务上下文现在更明确地跟随“用户想完成的目标”，而不是跟随单条消息、关键词、工具或文件。解释、修正、测试、进度询问和实现同一目标的后续步骤会继续使用当前任务，不会产生无意义的保存与切换。
- 仅仅换了话题、说“另外”或“回到刚才”不会自动触发切换；只有开始一个可以独立完成的新目标，或确实恢复目录中较早保存的目标时，才会保存当前进度并切换到准确的任务资料。
- 首次任务直接使用准备好的上下文；恢复旧任务时仍会核对准确的上下文和可能已变化的资料。用户不需要了解或管理上下文编号，也不会看到只为确认“无需切换”而产生的额外操作。
- Conversation 的共同约定会更严格地保留来源、适用任务以及约束的是 Agent 还是用户。针对 Agent 回答方式的要求不会被错误写成对用户的限制，也不会擅自补充没有说过的日期、版本、义务或适用范围。
- 新的跨任务约定会追加到共同资料，过时或错误的约定会在保留其他有效信息的前提下完整修正；已经准确保存的内容不会重复写入，局部结论和不确定推断仍留在各自任务中。
- 上下文工具在中英文设置界面中的说明已同步更新，更清楚地说明何时继续当前任务、何时暂停或恢复，以及如何维护共同约定。
