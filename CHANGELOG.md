...
- 新增由 Cyrene 管理的本地模型连接，首次使用时自动出现，仍可由用户删除并在需要时重新添加。
- 新增本地 Qwen3 Embedding 0.6B 嵌入模型，无需填写服务地址或密钥，可用于知识库检索和内容向量化。
...
- 大型工作区、长对话、持续终端输出、大规模索引、文件同步和多窗口同时使用时的响应速度与稳定性全面改善。
