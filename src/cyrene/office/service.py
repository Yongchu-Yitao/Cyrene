...
from cyrene.office.protocol import READ_ONLY_METHODS, expected_handshake
...
class OfficeBridgeService:
    ...
    async def register(self, websocket: Any, hello: dict[str, Any]) -> OfficeSession:
        ...
        if host not in {"powerpoint", "word"}:
            raise OfficeBridgeError("unsupported_host", "Only PowerPoint and Word hosts are accepted.")
    ...
    async def unregister(self, session_id: str, websocket: Any) -> None:
        ...
        for future in list(session.pending.values()):
            if not future.done():
                future.set_exception(OfficeBridgeError("session_disconnected", "The Office add-in disconnected."))
    ...
    def get_session(self, session_id: str | None, *, host: str = "powerpoint") -> OfficeSession:
        ...
        if requested:
            ...
            if session is None:
                raise OfficeBridgeError("session_not_found", f"Office session {requested!r} is not connected.")
            if host and session.host != host:
                raise OfficeBridgeError("wrong_host", f"Session {requested!r} is connected to {session.host}, not {host}.")
            return session
        ...
        *,
        ...
        async with session.lock:
            ...
            try:
                ...
                raise OfficeBridgeError("send_failed", "Could not send the request to PowerPoint.") from exc
            try:
                payload = await asyncio.wait_for(future, timeout=timeout)
            except TimeoutError as exc:
                session.pending.pop(request_id, None)
                raise OfficeBridgeError("office_timeout", f"PowerPoint did not finish {method} within {timeout:g} seconds.") from exc
        ...
        if payload.get("ok") is not True:
            ...
            raise OfficeBridgeError(
                ...
                str(error.get("message") or f"PowerPoint failed {method}."),
