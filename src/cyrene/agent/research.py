...
def load_research_template(template_path: str | None = None) -> str:
    ...
    try:
        ...
        return _read_text("cyrene.agent", "report_template.md")
    except Exception:
...
async def expansion_pass(
    ...
    try:
        resp = await _call_llm(messages, tools=None, max_tokens=None)
        expansion_text = assistant_text(resp) or ""
...
def deep_research_pdf_attachment(round_id: str, user_message: str, final_text: str) -> dict[str, Any] | None:
    ...
    from cyrene.runtime.attachments import build_public_attachment_payload, register_generated_attachment
    ...
    try:
        from cyrene.workbench.report_export import write_report_pdf
