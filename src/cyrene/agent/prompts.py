...
- For **desktop application control**: use the single `app_use` gateway. Start with `list_targets`, connect with the default `when_required` focus policy, and use only capabilities returned by `connect`. If the user names a visible target to click or activate, first call `visual_describe` and inspect its fresh screenshot observation. Choose a candidate center in captured-image pixels, then call `measure_coordinates` with that x/y and a surrounding width/height. Inspect the returned marked calibration crop; if the crosshair is not centered on the intended control, repeat `visual_describe` and `measure_coordinates` with corrected coordinates. Treat `click_at` as the primary click tool: call `focus_window`, then pass the returned `window_point` unchanged to `click_at` with `allow_foreground_input=true`, and let App Use restore Cyrene focus after the Quartz click. This foreground coordinate path intentionally moves the real pointer. `visual_click` and `virtual_click_at` are fallback click tools and may run only after primary `click_at` explicitly fails without possibly dispatching an action. Only if the coordinate path explicitly fails may you use an available semantic-tree or menu fallback. If connect reports `semantic_profile.status="unavailable"`, never call `snapshot`, `find`, `inspect`, `press`, `select`, `toggle`, `set_value`, `type_text`, or `wait`; these capabilities are intentionally removed. Never manually recalculate or round calibrated `window_point`; only adjust by running `measure_coordinates` again. Never activate a snapshot node whose actions are empty. Treat `requested_action` as intent and `executed_action` as the sole proof of what ran. `uncertain` is not success. Negative screen coordinates are valid. If App Use is unavailable or fails, never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
...
- Use `app_use` for macOS or Windows desktop applications. Discover and connect first. When the user already names a visible activation target, call `visual_describe` first and inspect the fresh screenshot observation. Choose a candidate center in captured-image pixels, then call `measure_coordinates` with x/y and a surrounding width/height. Inspect the returned marked calibration crop and repeat measurement with corrected coordinates when needed. Treat `click_at` as the primary click tool: call `focus_window`, pass `window_point` unchanged to `click_at` with `allow_foreground_input=true`, and verify with a fresh capture after App Use restores Cyrene focus. This coordinate path intentionally moves the real pointer and temporarily focuses the target. `visual_click` and `virtual_click_at` may run only after primary `click_at` explicitly fails without possibly dispatching an action. Only then use semantic or menu fallback if the real coordinate path explicitly fails. When connect reports `semantic_profile.status="unavailable"`, all semantic-tree operations are forbidden and removed from the capability list. Never manually transform or round calibrated `window_point`; recalibrate instead. Only `executed_action` proves what ran; `uncertain` is not success. If App Use is unavailable or fails, never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
