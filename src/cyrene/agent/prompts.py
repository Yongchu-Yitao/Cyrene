...
_MAIN_DESKTOP_PROMPT = _tool_pack_prompt_block(
    "desktop_tools",
    """- Desktop control has two explicit schemes. Use `desktop.use` for the existing visual scheme. Use `desktop.semantic.snapshot` plus `desktop.semantic.inspect|click|double_click|type|scroll|drag` for the accessibility-tree scheme. Never silently fall back between schemes. Linux supports only the semantic scheme.
- In the semantic scheme, start with `AppUISnapshot` list_targets → connect → snapshot. Every mutation must echo the exact `session_id`, `snapshot_id`, `revision`, `node_id`, and `action_id` returned by the current tree, plus a reason and a fresh idempotency key. Never invent selectors, native references, scripts, coordinates, screenshots, or focus changes. Treat `initializing`, `partial`, and timeouts as recoverable: reprobe or take a broader fresh snapshot. A stale lease or revision conflict requires a fresh snapshot, not a guessed retry. Only report an action as successful when post-action verification supports it; `uncertain` is not success.
- For **visual desktop application control**, invoke `desktop.use` through `desktop_tools`. Start its internal App Use request with `list_targets`, connect with mode=`visual` and the default `when_required` focus policy, and use only capabilities returned by `connect`. If the user names a visible target, first use `visual_describe`, then `measure_coordinates` with the same target description and inspect the marked calibration crop. Choose a candidate center in captured-image pixels and let App Use perform the coordinate mapping. Before a coordinate gesture, pass the latest measured `window_point` unchanged; never guess, round, or manually transform it. For primary clicking, call `focus_window`, then `click_at` with that unchanged point and `allow_foreground_input=true`; verify the result with a fresh capture. `visual_click` and `virtual_click_at` are fallbacks only after primary `click_at` explicitly fails without possibly dispatching an action. Treat `requested_action` as intent and `executed_action` as the sole proof of what ran; `uncertain` is not success. Negative screen coordinates are valid. If App Use is unavailable or fails, never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
...
- Use either explicit desktop scheme: `desktop.use` with mode=`visual` for screenshot/coordinate control, or the seven `desktop.semantic.*` capabilities for accessibility-tree control. Never fall back implicitly between them. Linux is semantic-only. Semantic writes require current session/snapshot/revision/node/action leases, reason, idempotency key, and post-action verification. If App Use is unavailable or fails, never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
