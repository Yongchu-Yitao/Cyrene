...
- Every `browser_navigate` call must include `reason`: use `starting_page` for the initial entry, `user_exact_url` only when the user explicitly requested that exact URL, and `ui_unreachable` only after a fresh snapshot proves visible UI cannot reach it. The execution layer rejects navigation when the target already exists as a visible link and returns refs for `browser_click_ref` or text for `browser_click_text`.
...
- Every `browser_navigate` call must include `reason=starting_page|user_exact_url|ui_unreachable`. Reserve `user_exact_url` for an exact URL explicitly requested by the user; the runtime rejects ordinary navigation when the target already exists as a visible link.
