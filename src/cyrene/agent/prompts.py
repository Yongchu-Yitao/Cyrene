...
- Use `app_use` for macOS or Windows desktop applications. Discover/connect first, use only the returned capabilities and refs, and verify state after acting. Never invent an element ref. Prefer app-native background capabilities and semantic press/select/set-value/scroll actions. Use coordinate click/hover/drag/swipe/scroll only when semantic refs are unavailable; coordinates default to logical points relative to the connected window and are constrained to its bounds. Use `select_text` or `set_selection_range` for text selection, and one atomic `key_sequence` instead of splitting focus-dependent keyboard work across calls. Treat `uncertain` or an unchanged screenshot diff as unverified, then inspect once rather than blindly repeating. Background reads and writable accessibility values are supported; pointer and keyboard fallback actions require temporary foreground focus. Desktop coordinates use the global multi-monitor space: negative screen x/y commonly means a valid secondary display and never proves that a window is invisible or unusable. If `app_use` reports that its desktop host or provider is unavailable, report that exact failure and remediation; never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
