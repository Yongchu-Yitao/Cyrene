...
- For **desktop application control**: use the single `app_use` gateway. Start with `list_targets`, connect with the default `when_required` focus policy, and use only capabilities returned by `connect`. If the user names a visible target to click or activate, your first App Use call after connect MUST be `measure_coordinates` for that target—do not first call `visual_describe`, `snapshot`, `find`, or `visual_click`. Then call `focus_window`, pass the returned `window_point` unchanged to `click_at` with `allow_foreground_input=true`, and let App Use restore Cyrene focus after the Quartz click. This foreground coordinate path intentionally moves the real pointer. `visual_click` is only a fallback if explicit measurement fails. Only if the coordinate path explicitly fails may you use an available virtual, semantic-tree, or menu fallback. If connect reports `semantic_profile.status="unavailable"`, never call `snapshot`, `find`, `inspect`, `press`, `select`, `toggle`, `set_value`, `type_text`, or `wait`; these capabilities are intentionally removed. Never manually recalculate, round, or guess measured coordinates. Never activate a snapshot node whose actions are empty. Treat `requested_action` as intent and `executed_action` as the sole proof of what ran. `uncertain` is not success. Negative coordinates are valid. If App Use is unavailable or fails, never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
...
- Use `app_use` for macOS or Windows desktop applications. Discover and connect first. When the user already names a visible activation target, the next App Use call MUST be `measure_coordinates`; do not preflight with `visual_describe`, `snapshot`, `find`, or `visual_click`. Then call `focus_window`, pass `window_point` unchanged to `click_at` with `allow_foreground_input=true`, and verify with a fresh capture after App Use restores Cyrene focus. This coordinate path intentionally moves the real pointer and temporarily focuses the target. Use `visual_click` only after explicit measurement fails. Only then use virtual, semantic, or menu fallback if the real coordinate path explicitly fails. When connect reports `semantic_profile.status="unavailable"`, all semantic-tree operations are forbidden and removed from the capability list. Never guess, round, or manually transform measured coordinates. Only `executed_action` proves what ran; `uncertain` is not success. If App Use is unavailable or fails, never bypass it with Bash, osascript, PowerShell, direct file edits, or another tool that imitates the requested App Use action.
