...
def _bytecode_cache_only_pack(path: Path) -> bool:
    """Return true when an unmanaged pack contains generated bytecode only.

    An interrupted development refresh can remove a new pack's sources after
    Python has imported it, leaving only ``__pycache__`` behind.  Treating that
    generated shell as a user PluginPack permanently blocks canonical seeding
    on the next startup.  Keep the exception deliberately narrow: any source,
    metadata, symlink, or other user-owned entry still preserves the collision.
    """
    ...
    for entry in entries:
        if entry.name != "__pycache__" or not entry.is_dir() or entry.is_symlink():
            return False
        try:
            cached_entries = tuple(entry.iterdir())
        except OSError:
            return False
        if any(
            cached.is_symlink()
            or not cached.is_file()
            or cached.suffix not in {".pyc", ".pyo"}
...
def _classify_canonical_packs(
    root: Path,
    canonical_files: dict[str, bytes],
    previous_hashes: dict[str, str],
    deleted_contributions: set[str],
    existing: list[Path],
    ...
    for pack_name in sorted(canonical_packs):
        ...
        if not _path_present(target_pack):
            ...
            continue
        if _bytecode_cache_only_pack(target_pack):
            try:
                ...
                new_packs.add(pack_name)
                diagnostics.append(f"repaired bytecode-only canonical pack shell {target_pack}")
                continue
        prefix = f"{pack_name}/"
        baseline = {
            relative: digest
        ...
        next_reason = "is no longer a regular directory" if baseline else "has no Cyrene upstream-hash baseline"
        diagnostics.append(f"preserved canonical pack collision {target_pack}: {next_reason}; no files were merged")
    return skipped_packs, new_packs, diagnostics
...
def _seed_canonical_directory(root: Path) -> BuiltinPluginSeedResult:
    ...
    for pack_name in skipped_packs - deleted_contributions:
        prefix = f"{pack_name}/"
        next_hashes.update(
            (relative, digest)
            for relative, digest in previous_hashes.items()
