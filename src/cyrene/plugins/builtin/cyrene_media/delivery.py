...
class MediaDelivery:
    ...
    async def _project(self, job: dict[str, Any]) -> dict[str, Any] | None:
        ...
        language = app_language(request.get("language"))
        reference_ids = [
            str(value or "").strip()
        ...
        if status == "succeeded":
            ...
            content = localized(
                "Media generation completed · {kind}",
                "媒体生成完成 · {kind}",
                language=language,
                kind=kind,
            ...
            content = localized(
                "Media generation cancelled · {kind}",
                "媒体生成已取消 · {kind}",
                language=language,
                kind=kind,
            )
            ...
            content = localized(
                "Media generation failed · {kind} · {error_code}",
                "媒体生成失败 · {kind} · {error_code}",
                language=language,
                kind=kind,
