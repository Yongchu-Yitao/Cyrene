...
class WeChatClient:
    ...
    def _base_info(self) -> dict:
        return {"channel_version": "0.9.0-beta12", "bot_agent": "Cyrene/0.9.0-beta12"}
    ...
    async def send_file(self, chat_id: str, filepath: str, filename: str = "") -> bool:
        ...
        if len(raw) > MAX_FILE_SIZE:
            await self.send_message(chat_id, localized(
                "The file is too large (50 MB limit).",
                "文件过大（超过 50MB 限制）。",
            ))
            return False
        ...
        try:
            ...
            await self.send_message(chat_id, localized(
                "File upload failed: could not obtain an upload URL.",
                "文件上传失败，无法获取上传地址。",
            ))
            return False
        ...
        try:
            ...
            await self.send_message(chat_id, localized(
                "File upload failed because of a CDN error.",
                "文件上传失败，CDN 上传出错。",
            ))
            return False
        except Exception:
        ...
        try:
            ...
            await self.send_message(chat_id, localized(
                "Could not send the file.", "文件发送失败。"
            ))
            return False
