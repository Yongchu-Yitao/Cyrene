"""Restricted LAN listener for Remote Plugin IP + short-key pairing."""
...
from .control import DIRECT_PAIRING_PORT, RemoteControlStore
...
class DirectPairingServer:
    ...
    async def _handle_connection(
        ...
        try:
            ...
            if method != "POST":
                status, payload = 405, _pairing_error(
                    "method_not_allowed",
                ...
                if path == "/v1/pairing/claim":
                    ...
                    if self.store.get_peer(sender) is None:
                        status, payload = 403, _pairing_error(
                            "peer_not_trusted",
                            "The remote device is not trusted.",
                            "远程设备不受信任。",
                        )
                    elif self._inline_request_receiver is None:
                        status, payload = 503, _pairing_error(
                            "control_gateway_offline",
                            "The control gateway is offline.",
                            "控制网关处于离线状态。",
                        )
                    else:
                        ...
                        if saved_address:
                            ...
                            if saved_host != source:
                                status, payload = 403, _pairing_error(
                                    ...
                                    "The remote device address does not match.",
                                    "远程设备地址不匹配。",
                                )
                            else:
                    ...
                    status, payload = 404, _pairing_error(
                        "endpoint_not_found",
                        "Endpoint not found.",
                        "未找到接口。",
                    )
        except RuntimeError as exc:
            logger.warning("Pairing request failed (429): %s", exc, exc_info=True)
            status, payload = 429, _pairing_error(
            ...
            status, payload = 500, _pairing_error(
                "pairing_request_failed",
                "The pairing request failed.",
                "配对请求失败。",
            )
