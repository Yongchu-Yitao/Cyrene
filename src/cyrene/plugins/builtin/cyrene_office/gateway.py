...
from .protocol import expected_handshake
...
'''
    ...
    def public_info(self, *, running: bool) -> dict[str, Any]:
        return {
            ...
            "install_command": (
                "uv run python -m "
                "cyrene.plugins.builtin.cyrene_office.install --trust"
            ),
...
def _register_document_routes(app: FastAPI, material: OfficeGatewayFiles) -> None:
    ...
    async def benchmark_invoke(request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=localized("Invalid JSON payload.", "JSON 请求内容无效。"),
            ) from exc
        ...
        if plugin_name is None:
            raise HTTPException(
                status_code=400,
                detail=localized(
                    "This method is not part of the PowerPoint benchmark workload.",
                    "此方法不属于 PowerPoint 基准工作负载。",
                ),
            )
        ...
        if not isinstance(args, dict):
            raise HTTPException(
                status_code=400,
                detail=localized("Arguments must be an object.", "参数必须是对象。"),
            )
        ...
        # The live host exposes the canonical dispatch entrypoint.  Keeping
        # the small structural fallback here also lets lightweight gateway
        # embedders provide the same PluginRuntime contract without having to
        # manufacture a host-specific adapter.
        dispatch = getattr(host.runtime, "call_canonical", None)
        if not callable(dispatch):
            dispatch = host.runtime.call
        call = await dispatch(
            plugin_name,
            args,
            PluginContext(
        ...
        if not call.success:
            raise HTTPException(
                ...
                detail=str(
                    call.error
                    or localized(f"{plugin_name} failed", f"{plugin_name} 执行失败")
                ),
            )
        result = call.value
        if isinstance(result, str):
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=localized(
                        "The benchmark plugin returned invalid JSON.",
                        "基准插件返回了无效的 JSON。",
                    ),
                ) from exc
        if not isinstance(result, dict):
            raise HTTPException(
                status_code=500,
                detail=localized(
                    "The benchmark plugin returned a non-object result.",
                    "基准插件返回的结果不是对象。",
                ),
            )
        return result
    ...
    async def taskpane(request: Request) -> Response:
        if not _authorized(request, material):
            raise HTTPException(
                status_code=401,
                detail=localized("Invalid bridge token.", "桥接令牌无效。"),
            )
...
def _webui_static_path(*parts: str) -> Path:
    import cyrene.workbench.webui as webui
...
def _register_asset_routes(app: FastAPI, material: OfficeGatewayFiles) -> None:
    ...
    async def icon(size: int, request: Request) -> Response:
        ...
        if size not in {16, 32, 80}:
            raise HTTPException(
                status_code=404,
                detail=localized("Icon unavailable.", "图标不可用。"),
            )
    ...
    async def cyrene_theme(request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(
                status_code=404,
                detail=localized("Theme unavailable.", "主题不可用。"),
            ) from exc
    ...
    async def cyrene_fonts(request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(
                status_code=404,
                detail=localized("Font styles unavailable.", "字体样式不可用。"),
            ) from exc
        for font_name in {
    ...
    async def appearance(request: Request) -> dict[str, Any]:
        ...
        public_values["language"] = app_language(
            (runtime_snapshot.get("values") or {}).get("app_language")
        )
        return {
    ...
    async def font(font_name: str, request: Request) -> Response:
        ...
        if font_name not in {
            ...
            raise HTTPException(
                status_code=404,
                detail=localized("Font unavailable.", "字体不可用。"),
            )
        try:
        ...
        try:
            ...
            await websocket.send_json({
                ...
                "error": None if session.compatible else {
                    "code": "addin_outdated",
                    "message": localized(
                        "The PowerPoint task pane build does not match the running Cyrene gateway.",
                        "PowerPoint 任务窗格版本与当前运行的 Cyrene 网关不匹配。",
                    ),
                },
            })
            while True:
