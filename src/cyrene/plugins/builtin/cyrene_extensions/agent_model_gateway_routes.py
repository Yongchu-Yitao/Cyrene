...
async def _gateway_request_body(
    request: Request,
) -> tuple[dict[str, Any] | None, JSONResponse | None]:
    try:
        body = await request.json()
    except ValueError:
        return None, _gateway_error(
...
def register_agent_model_gateway_routes(router: APIRouter) -> None:
    ...
    async def api_agent_model_gateway_chat(request: Request):
        ...
        body, body_error = await _gateway_request_body(request)
        ...
        try:
            ...
            if result == "":
                return _gateway_error(
                    "Cyrene model is not configured",
                    "尚未配置 Cyrene 模型。",
