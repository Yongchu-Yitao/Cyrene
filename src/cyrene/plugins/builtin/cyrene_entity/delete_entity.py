"""Safely archive or permanently delete one durable entity."""

from __future__ import annotations

from typing import Any

from cyrene.core.plugin import PluginContext
from cyrene.plugins.native_runtime import plugin_localized

from ._plugin import create_tool_plugin
from ._service import current_project_id, entity_service, resolve_entity

TOOL_NAME = "entity.delete"


async def delete_entity(
    arguments: dict[str, Any],
    context: PluginContext,
) -> dict[str, Any]:
    if not arguments.get("id") and not arguments.get("title"):
        return {
            "ok": False,
            "error": "invalid_locator",
            "message": plugin_localized(
                context,
                "Provide a full UUID, a unique UUID prefix, or an exact title.",
                "请提供完整 UUID、唯一 UUID 前缀或精确标题。",
            ),
        }
    service = entity_service(context)
    resolution = await resolve_entity(
        service,
        entity_id=arguments.get("id"),
        title=arguments.get("title"),
        type=arguments.get("type"),
        project_id=current_project_id(context),
    )
    if resolution["matches"]:
        return {
            "ok": False,
            "error": "ambiguous",
            "matched_by": resolution["matched_by"],
            "matches": resolution["matches"],
            "message": plugin_localized(
                context,
                "Multiple items matched; nothing was deleted to avoid a mistake.",
                "匹配到多条事务，为避免误删未执行。",
            ),
        }
    entity = resolution["entity"]
    if entity is None:
        return {"ok": False, "error": "not_found", "message": plugin_localized(context, "Item not found.", "未找到事务。")}

    permanent = bool(arguments.get("permanent", False))
    if not await service.delete(entity["id"], permanent=permanent):
        return {"ok": False, "error": "not_found", "message": plugin_localized(context, "Item not found.", "未找到事务。")}
    action = "permanently_deleted" if permanent else "archived"
    return {
        "ok": True,
        "action": action,
        "entity": entity,
        "message": plugin_localized(
            context,
            (
                "Permanently deleted item: {title} (ID: {entity_id})"
                if permanent
                else "Archived item: {title} (ID: {entity_id})"
            ),
            (
                "已永久删除事务：{title}（ID: {entity_id}）"
                if permanent
                else "已归档事务：{title}（ID: {entity_id}）"
            ),
            title=entity["title"],
            entity_id=entity["id"],
        ),
    }


plugin = create_tool_plugin(TOOL_NAME, delete_entity)
handler = delete_entity

__all__ = ["TOOL_NAME", "delete_entity", "handler", "plugin"]
