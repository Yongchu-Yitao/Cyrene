...
def _build_simplexng_launch_cmd(port: int, host: str, *, settings_path: Path | None = None) -> list[str]:
    ...
    if importlib.util.find_spec("simplexng.simplexng") is not None:
        # Preserve the historical process command because external launchers
        # and parent-process supervisors may identify this stable module name.
        # The entry point itself aliases the focused backend implementation.
