...
def _workbench_new_session(project_id: str, title: str, goal: str='', now: str | None=None, *, kind: str='task', status: str='idle') -> dict[str, Any]:
    ...
    return {'id': session_id, 'projectId': project_id, 'kind': kind, 'title': str(title or '新任务').strip()[:80] or '新任务', 'goal': str(goal or '').strip(), 'constraints': [], 'status': status, 'priority': 'medium', 'createdAt': now, 'updatedAt': now, 'agentReply': '', 'plan': [], 'planRevision': 0, 'planDefinitionRevision': 0, 'approvedPlanDefinitionRevision': None, 'events': [], 'runs': [], 'artifacts': [], 'acceptanceCriteria': [], 'summary': None, 'titleLocked': False}
...
def _workbench_mark_completed_if_acceptance_passed(session: dict[str, Any], *, now: str | None=None, event_body: str='所有验收标准均已通过，任务自动标记为已完成。') -> bool:
...
def _workbench_new_init_session(project_id: str, project: dict[str, Any], now: str | None=None) -> dict[str, Any]:
    now = now or _utc_now_iso()
    session = _workbench_new_session(project_id, '初始化项目', '完成项目的基础设置与初始规划。', now, kind='init', status='initializing')
