...
CONTEXT_PLUGINS = (
    tool("load_context", "Resume an earlier goal by restoring its saved task context before answering or using its evidence. Use the catalog's exact ID; unload a different active context first. Do not call for the already active task, a new goal, or shared. Call alone and wait for success.", {"context_id": ID}, ["context_id"]),
    tool("unload_context", "Save a checkpoint and pause the active task before starting an independent goal or loading an earlier one, including text-only work. Do not call for the first task, acknowledgments, or explanations, corrections and tests serving the current goal. Subsequent task work creates a context unless you load one. Call alone and wait for success.",
