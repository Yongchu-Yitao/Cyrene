...
CONTEXT_PLUGINS = (
    tool("load_context", "Resume a goal or subtask using its exact catalog ID, restoring its messages and truncated historical calls in order without old tool results. Unload a different active context first. Never load shared, an unrelated placeholder or the already active context. Oversized loads fail without changing state: read the returned snapshot in small sections instead. Call alone and wait for success.", {"context_id": ID}, ["context_id"]),
    tool("unload_context", "Pause before another workstream, or checkpoint a continuation segment when warned about capacity. Save task messages in order; retain tool names and truncated arguments, discard results from restored context. Do not unload merely to save, finish or wait. Then load an existing context or start work to create one; after a capacity warning, continue in a new segment and read old evidence by path. Call alone and wait for success.",
