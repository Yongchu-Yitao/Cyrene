...
from cyrene.platform.bootstrap import create_runtime_context
...
def _get_default_ui_mode() -> str:
    ...
    try:
        from cyrene.platform.buildinfo import DEFAULT_UI_MODE
...
async def _handle_menu():
    while True:
        ...
        if choice == "0":
            ...
            from cyrene.platform.setup import init_setup_flag, run_setup
            init_setup_flag()
            print(localized(
...
async def _run_cli_host() -> None:
    ...
    try:
        from cyrene.platform.setup import init_setup_flag, is_setup_done, run_setup
...
def _dump_error(message: str) -> None:
    ...
    _paths = []
    try:
        from cyrene.platform.paths import TEMP_DIR as _CYRENE_TEMP_DIR
