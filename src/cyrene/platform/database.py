...
from cyrene.platform.persistence.analytics import (
    activity_column,
    backfill_analytics,
    bump_activity_sync,
    count_stat_days,
...
from cyrene.platform.persistence.migrations import initialize_runtime_database
from cyrene.platform.persistence.schema import RUNTIME_SCHEMA
...
async def get_execution_time_totals(db_path: str) -> dict:
    ...
    if callable(time_totals):
        ...
        schedule_total_ms = int(getattr(schedule_totals, "total_ms", 0) or 0)
        schedule_longest_ms = int(getattr(schedule_totals, "longest_ms", 0) or 0)
        schedule_runs = int(getattr(schedule_totals, "runs", 0) or 0)
    return {
        "total_ms": goal_total_ms + schedule_total_ms,
        "longest_ms": max(goal_longest_ms, schedule_longest_ms),
        "runs": int(goal_runs or 0) + schedule_runs,
    }
