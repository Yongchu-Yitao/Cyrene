"""Typed capture models for behavior-learning telemetry."""
...
class CaptureService:
    ...
    async def record_browser_user_event(self, *, session_id: str='', round_id: str='', event_kind: str, payload: dict[str, Any] | None=None, browser_url: str='', browser_title: str='', target: dict[str, Any] | None=None) -> None:
        ...
        if not turn_id:
            ctx = await self.begin_turn(session_id=sid, round_id=rid or self.ports.new_id('browser_round'), user_message='用户接管内置浏览器并执行操作。', history=[], session_title='Browser user operation')
