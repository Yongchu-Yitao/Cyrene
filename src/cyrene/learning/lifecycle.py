"""Pure learned-skill lifecycle policy."""
...
class LifecycleService:
    ...
    async def manual_activate_skill(self, skill_id: str) -> bool:
        ...
        async with self.ports.connect() as conn:
            ...
            await self._save_skill_version(conn=conn, skill_id=skill_id, version=next_version, parent_version=int(row['current_version']), definition=current, change_type='activate', change_summary='Manually activated from evolution UI.')
            await conn.commit()
        return True
    ...
    async def manual_deprecate_skill(self, skill_id: str) -> bool:
        async with self.ports.connect() as conn:
            ...
            await self._save_skill_version(conn=conn, skill_id=skill_id, version=next_version, parent_version=int(row['current_version']), definition=current, change_type='deprecate', change_summary='Manually deprecated from evolution UI.')
            await conn.commit()
        return True
    ...
    async def update_learned_skill(self, skill_id: str, updates: dict[str, Any], *, reason: str='Manual skill edit.') -> dict[str, Any] | None:
        ...
        async with self.ports.connect() as conn:
            ...
            persisted = await self._persist_skill_version(conn, skill_id=skill_id, current_row=row, definition=sanitized, change_type='manual_edit', change_summary=reason)
            await conn.commit()
        return persisted
    ...
    async def apply_skill_patch(self, skill_id: str, patch_id: str) -> dict[str, Any]:
        async with self.ports.connect() as conn:
            ...
            definition['run_statistics'] = current['run_statistics']
            sanitized = await self._sanitize_skill_definition(definition)
            persisted = await self._persist_skill_version(conn, skill_id=skill_id, current_row=skill_row, definition=sanitized, change_type='apply_patch', change_summary=str(patch_row['reason'] or 'Applied skill patch.'), patch_list=applied_changes)
    ...
    async def rollback_learned_skill(self, skill_id: str, rollback_version: int) -> dict[str, Any]:
        async with self.ports.connect() as conn:
            ...
            sanitized = await self._sanitize_skill_definition(definition)
            persisted = await self._persist_skill_version(conn, skill_id=skill_id, current_row=current_row, definition=sanitized, change_type='rollback', change_summary=f'Rolled back skill to version {rollback_version}.', rollback_target=int(rollback_version))
            await conn.commit()
