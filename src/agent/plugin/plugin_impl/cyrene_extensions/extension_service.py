...
from agent.plugin.plugin_impl.cyrene_extensions.extension_catalog import (
...
def application_extension_service(context: Any) -> "ExtensionService | None":
    """Resolve the application-owned service without reviving a disabled pack.

    Other application packs may contribute their own routes before this pack is
    attached.  They can use this bridge while still respecting the Extensions
    pack switch; a disabled or missing pack must not construct its singleton.
    """
    services = getattr(context, "services", None)
    if isinstance(services, Mapping):
        service = services.get("extensions")
        if service is not None:
            return service
    registry = getattr(context, "registry", None)
    if registry is None:
        # Standalone callers (for example a pack-level route test) do not have
        # an application registry. They are not an attached host and therefore
        # cannot represent a disabled pack; preserve the direct service API in
        # that narrow case.
        return get_extension_service()
    try:
        if not registry.pack_enabled("cyrene_extensions"):
            return None
    except Exception:
        return None
    return get_extension_service()
...
def _localized_catalog_spec(spec: Mapping[str, Any]) -> dict[str, Any]:
    result = dict(spec)
    description_zh = str(result.pop("description_zh", "") or "").strip()
    if description_zh:
...
def _is_fixed_registry_package_version(registry_type: str, value: Any) -> bool:
    version = str(value or "").strip()
    kind = str(registry_type or "").strip().lower()
    if not version:
        return False
    if kind == "npm":
        return _NPM_EXACT_VERSION.fullmatch(version) is not None
    if kind == "pypi":
        try:
            Version(version)
...
def _validated_skill_repository_url(value: Any) -> str:
    ...
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError(
            "Skill repository URL must not contain credentials, query parameters, or fragments"
        )
    return url


def _validate_persisted_mcp_url(value: Any) -> None:
    url = str(value or "").strip()
    if not url:
        return
    parsed = urllib.parse.urlparse(url)
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError(
            "MCP URL must not contain credentials, query parameters, or fragments; use headers for authentication"
        )


def _active_mcp_service(*, required: bool = False) -> Any | None:
    from agent.plugin import active_plugin_service

    service = active_plugin_service("mcp")
    if service is None and required:
        raise RuntimeError("MCP Plugin is disabled or unavailable")
    return service


def _active_skills_service(*, required: bool = False) -> Any | None:
    from agent.plugin import active_plugin_service

    service = active_plugin_service("skills")
    if service is None and required:
        raise RuntimeError("Skills Plugin is disabled or unavailable")
    return service
...
def _extension_error_message(reason_code: str) -> str:
    messages = {
        "dependency_conflict": (
            "The extension's dependencies conflict with this environment.",
            "扩展依赖与当前环境冲突。",
        ),
        ...
        "executable_not_found": (
            "The installed extension did not provide its expected executable.",
            "已安装的扩展未提供预期的可执行文件。",
        ),
        "fixed_version_required": (
            "A fixed extension version is required.",
            "必须指定固定的扩展版本。",
        ),
        ...
        "uv_runtime_missing": (
            "The bundled uv runtime is unavailable. Repair or reinstall Cyrene.",
            "内置 uv 运行时不可用，请修复或重新安装 Cyrene。",
        ),
    }
    en, zh = messages.get(
        reason_code,
...
def agent_extension_paths(
    *,
    ...
    if extensions_enabled and bundled_uv and extension_is_enabled("toolchain", "uv"):
        candidates.append(bundled_uv.parent)
    managed_toolchains = _managed_extension_records("extension_toolchains")
    if extensions_enabled and any(str(record.get("id") or "") == "python" for record in managed_toolchains) and extension_is_enabled("toolchain", "python"):
        candidates.append(_UV_BIN_DIR)
    managed_mise_enabled = any(
        (cli_enabled if kind == "cli" else extensions_enabled)
        and extension_is_enabled(kind, str(record.get("id") or ""))
    ...
    if extensions_enabled and _AGENT_BIN_DIR.is_dir():
    ...
    if extensions_enabled and managed_tex and extension_is_enabled("toolchain", "tex") and _TEX_DIR.is_dir():
...
def _agent_environment_pack_state() -> tuple[bool, bool]:
    """Return independent CLI and environment Plugin activation state."""

    # Import lazily: the extension service is also used while the Plugin
    # registry is being assembled.
    from agent.plugin.application import active_plugin_application_host

    host = active_plugin_application_host()
    if host is None:
        # Standalone Agent/terminal processes do not own the application host.
        # Preserve the legacy environment behavior there; when a host exists,
        # its pack services remain the authoritative activation gates.
        return True, True
    try:
        return host.service("cli") is not None, host.service("extensions") is not None
    except Exception:
        # Deleted, disabled, or failed-to-reload packs must fail closed.  The
        # caller will preserve the original process environment.
        return False, False
...
def agent_process_environment(base: dict[str, str] | None = None) -> dict[str, str]:
    ...
    try:
        ...
        for kind, setting_key in (("cli", "extension_clis"), ("toolchain", "extension_toolchains")):
            ...
            for record in _managed_extension_records(setting_key):
                ...
                if (
                    ...
                    and (not owner_enabled or not extension_is_enabled(kind, extension_id))
                ):
        ...
        fallbacks = [
            path for path in agent_extension_paths(
...
class InstallTaskStore:
        ...
        for task_id, task in (data if isinstance(data, dict) else {}).items():
            if task.get("status") in {"queued", "running", "cancelling"}:
                ...
                task["error"] = localized(
                    "Cyrene stopped before the installation completed. Retry the installation.",
                    "Cyrene 在安装完成前已停止，请重试安装。",
                )
    ...
    def create(self, *, kind: str, extension_id: str, action: str, actor: str, request: dict[str, Any]) -> dict[str, Any]:
        ...
        task = {
            ...
            "message": localized("Waiting to start", "等待开始"), "created_at": _now(), "started_at": "", "finished_at": "", "error": "", "result": None,
        }
        with self._lock:
    ...
    def start(self, task: dict[str, Any], manager: str, worker: Any) -> None:
        async def run() -> None:
            ...
            self.update(
                ...
                started_at=_now(),
                progress=2,
                message=localized("Preparing installation", "正在准备安装"),
            )
            try:
                ...
                if current.get("status") == "cancelling":
                    self.update(task_id, status="cancelled", finished_at=_now(), message=localized("Cancelled", "已取消"))
                else:
                    self.update(task_id, status="completed", finished_at=_now(), progress=100, message=localized("Installed", "已安装"), result=result)
            except asyncio.CancelledError:
                self.update(task_id, status="cancelled", finished_at=_now(), message=localized("Cancelled", "已取消"))
            except Exception as exc:
                logger.exception("Extension install task failed: %s", task_id)
                reason_code = _extension_error_reason(exc)
                self.update(
                    task_id,
                    status="failed",
                    finished_at=_now(),
                    error=_extension_error_message(reason_code),
                    reason_code=reason_code,
                    message=localized("Installation failed", "安装失败"),
                )
                _audit(
                    task.get("actor", "user"),
                    f"{task.get('action')}.finish",
                    task.get("extension_id", ""),
                    {"reason_code": reason_code},
                    "failed",
                )
            finally:
    ...
    def cancel(self, task_id: str) -> bool:
        ...
        self.update(
            task_id,
            status="cancelling",
            message=localized("Cancelling", "正在取消"),
        )
...
class ExtensionService:
    ...
    def process_environment(
        ...
        """Return the credentials-free PATH contribution for Agent children."""

        return agent_process_environment(base)

    def managed_runtime_bin_directory(self) -> str:
        """Return the managed Node runtime bin directory when installed."""

        executable = _managed_npm_executable()
        return str(Path(executable).resolve().parent) if executable else ""

    @staticmethod
    def get_agent_installation(installation_id: str) -> dict[str, Any] | None:
        from .extension_agent_runtime import get_agent_installation

        return get_agent_installation(str(installation_id or ""))

    @staticmethod
    def storage_paths() -> dict[str, tuple[Path, ...]]:
        return {"extensions": (_ROOT, _MISE_CACHE, _NPM_CACHE_DIR)}
    ...
    async def settings_changed(
        ...
        """Apply process-scoped extension settings owned by this pack."""
        if not {
        ...
        try:
            ...
            logger.debug("Unable to recycle external Agent transports after settings change", exc_info=True)
    ...
    def agent_listing(self) -> dict[str, Any]:
        ...
        agent_runtime = importlib.import_module("agent.plugin.plugin_impl.cyrene_extensions.extension_agent_runtime")
    ...
    def list_extensions(self) -> dict[str, Any]:
        ...
        mcp_service = _active_mcp_service()
        statuses = (
        ...
        mcp_cards = []
        safe_mcp_configs = (
            mcp_service.configs(redacted=True)
            if mcp_service is not None
            else []
        )
        for config in safe_mcp_configs:
            ...
            if live_error:
                logger.info(
                    "MCP connection status failed [server=%s]: %s",
                    name,
                    live_error,
                )
            mcp_cards.append({
                "key": f"mcp:{name}", "id": name, "kind": "mcp", "name": name, "description": str(config.get("description") or localized("MCP server", "MCP 服务器")),
                ...
                "connection_status": status, "tool_count": int(live.get("tool_count") or 0), "tools": list(live.get("tools") or []),
                "pack_id": str(live.get("pack_id") or f"mcp.{name}"),
                "error": localized(
                    "The MCP server could not be connected.",
                    "无法连接 MCP 服务器。",
                ) if live_error else "",
                "error_code": "mcp_connection_failed" if live_error else "",
                "config": config,
            })
    ...
    def install_local_skill(
        ...
        *,
        ...
        result = _active_skills_service(required=True).install_skill(source)
    ...
    async def set_mcp_enabled(self, extension_id: str, enabled: bool, *, actor: str = "user") -> dict[str, Any]:
        mcp_service = _active_mcp_service(required=True)
        status = await mcp_service.set_enabled(extension_id, enabled)
    ...
    async def set_extension_enabled(self, kind: str, extension_id: str, enabled: bool, *, actor: str = "user") -> dict[str, Any]:
        ...
        extension_id = str(extension_id or "").strip()
        if kind == "skill":
            if not _active_skills_service(required=True).set_skill_enabled(
                extension_id,
                enabled,
            ):
    ...
    async def search(self, kind: str, query: str, *, advanced: bool = False, cursor: str = "") -> dict[str, Any]:
        ...
        if kind == "skill":
            _active_skills_service(required=True)
            return await self._search_skill_repositories(query)
        if kind == "toolchain":
            results: list[dict[str, Any]] = []
            for tool_id, spec in TOOLCHAINS.items():
                search_text = (
                    f"{tool_id} {spec.get('name')} {spec.get('description')} "
                    f"{spec.get('description_zh')}"
                )
                if query.casefold() in search_text.casefold():
                    item = {"id": tool_id, **_localized_catalog_spec(spec), "source": "cyrene-catalog", "verified": True}
    ...
    async def _search_cli(self, query: str, *, advanced: bool) -> dict[str, Any]:
        ...
        for cli_id, spec in CURATED_CLIS.items():
            search_text = (
                f"{cli_id} {spec.get('name')} {spec.get('description')} "
                f"{spec.get('description_zh')}"
            )
            if query.casefold() in search_text.casefold():
                results.append({
                    "id": cli_id,
                    **_localized_catalog_spec(spec),
                    "source": spec.get("ref"),
                    "verified": True,
                })
        mise = _bundled_binary("mise")
    ...
    async def _search_aqua_registry(self, query: str) -> list[dict[str, Any]]:
        ...
        return [{
            ...
            "description": localized(
                "Aqua Standard Registry package: {package}",
                "Aqua 标准注册表软件包：{package}",
                package=package,
            ),
            "manager": "mise",
    ...
    async def _search_mcp(self, query: str, *, cursor: str = "") -> dict[str, Any]:
        ...
        for wrapper in wrappers:
            ...
            installable_packages = [
                package
                for package in packages
                if _is_fixed_registry_package_version(
                    str(package.get("registryType") or ""),
                    package.get("version"),
                )
                and not any(
                    variable.get("isRequired")
    ...
    async def inspect_skill_source(self, url: str) -> dict[str, Any]:
        skills_service = _active_skills_service(required=True)
        with tempfile.TemporaryDirectory(prefix="cyrene-skill-inspect-", dir=TEMP_DIR) as tmp:
            ...
            for entrypoint in sorted(root.rglob("SKILL.md")):
                if any(part == ".git" for part in entrypoint.parts):
                    continue
                inspected = skills_service.inspect_skill_directory(entrypoint.parent)
                if not inspected.get("ok"):
                    continue
                candidates.append({
                    "path": entrypoint.parent.relative_to(root).as_posix(),
    ...
    async def _checkout_skill_source(self, url: str, destination: Path) -> tuple[Path, dict[str, Any]]:
        url = _validated_skill_repository_url(url)
        ...
        revision, revision_error, revision_code = await self._run_subprocess(
        ...
        if revision_code != 0:
            raise RuntimeError(
                revision_error.decode("utf-8", errors="replace")[-1000:]
                or "Unable to resolve the inspected Skill commit"
            )
    ...
    def start_install(self, kind: str, extension_id: str, request: dict[str, Any], *, actor: str = "user") -> dict[str, Any]:
        ...
        if kind == "skill":
            _active_skills_service(required=True)
            _validated_skill_repository_url(request.get("url"))
        elif kind == "mcp":
            config = request.get("config")
            remote = request.get("remote")
            if isinstance(config, Mapping):
                _validate_persisted_mcp_url(config.get("url"))
            if isinstance(remote, Mapping):
                _validate_persisted_mcp_url(remote.get("url"))
    ...
    async def _install_worker(self, task_id: str, kind: str, extension_id: str, request: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        self.tasks.update(
            ...
            progress=8,
            message=localized(
                "Resolving exact version and source",
                "正在解析精确版本和来源",
            ),
        )
        if kind == "agent":
    ...
    async def _install_agent(self, task_id: str, extension_id: str, request: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        self.tasks.update(
            task_id,
            progress=12,
            message=localized("Validating Agent manifest", "正在验证智能体清单"),
        )
        ...
        self.tasks.update(
            task_id,
            progress=100,
            message=localized("Agent installed", "智能体已安装"),
        )
        ...
        return {
            ...
            "runtime_state": record["runtime_state"],
            "blocked_reason": "",
            "note_code": "agent_starts_on_demand",
            "note": localized(
                "Manifest validated; ACP stdio starts on demand when the agent is selected.",
                "清单已验证；选择该智能体后，ACP stdio 将按需启动。",
            ),
        }
    ...
    async def _run_manager(self, task_id: str, command: list[str], *, env: dict[str, str], timeout: float = 1800) -> tuple[str, str]:
        if task_id:
            self.tasks.update(
                task_id,
                progress=35,
                message=localized("Downloading and installing", "正在下载并安装"),
            )
    ...
    async def _install_mise(self, task_id: str, kind: str, extension_id: str, request: dict[str, Any], actor: str) -> dict[str, Any]:
        ...
        self.tasks.update(
            task_id,
            progress=18,
            message=localized(f"Resolved {exact}", f"已解析 {exact}"),
        )
        env = extension_environment()
        ...
        self.tasks.update(
            ...
            progress=78,
            message=localized(
                "Activating Cyrene-managed version",
                "正在激活 Cyrene 托管版本",
            ),
        )
        ...
        if kind == "cli":
            # CLI installation remains identical to the previous installer.
            # Only the follow-up integration owner changed: the active
            # ``cyrene_cli`` pack now assesses and registers tree-local Hooks.
            from agent.plugin import active_plugin_service

            cli_service = active_plugin_service("cli")
            schedule = getattr(cli_service, "schedule_hook_configuration", None)
            if callable(schedule):
                schedule({**record, "key": f"cli:{extension_id}"}, trigger="install")
        return record
                        ...
                        with destination.open("wb") as handle:
                            async for chunk in response.aiter_bytes():
                                ...
                                if total:
                                    self.tasks.update(
                                        task_id,
                                        progress=min(70, 20 + int(50 * written / total)),
                                        message=localized(
                                            f"Downloading {written // 1048576} / {total // 1048576} MB",
                                            f"正在下载 {written // 1048576} / {total // 1048576} MB",
                                        ),
                                    )
                last_error = None
                break
    ...
    async def _install_skill(self, task_id: str, extension_id: str, request: dict[str, Any], staging: Path, actor: str) -> dict[str, Any]:
        ...
        reviewed_commit = str(request.get("source_commit") or "").strip().lower()
        ...
        if not re.fullmatch(r"[0-9a-f]{40}|[0-9a-f]{64}", reviewed_commit):
            raise ValueError("Inspect the Skill repository before installing its reviewed commit")
        root, metadata = await self._checkout_skill_source(url, staging)
        installed_commit = str(metadata.get("source_commit") or "").strip().lower()
        if installed_commit != reviewed_commit:
            raise ValueError(
                "The Skill repository changed since inspection; inspect it again before installing"
            )
        if not selected:
        ...
        self.tasks.update(
            ...
            progress=55,
            message=localized(
                f"Installing {len(selected)} Skill snapshot(s)",
                f"正在安装 {len(selected)} 个技能快照",
            ),
        )
        for relative in selected:
            ...
            result = skills_service.install_skill(
                candidate,
                source_metadata={**metadata, "source_subdir": relative},
            )
            if not result.get("ok"):
    ...
    async def _rollback_mcp_mise_activation(self, target: str) -> None:
        if not target:
            return
        mise = _bundled_binary("mise")
        if not mise:
        ...
        try:
            await self._run_manager(
                "",
                [str(mise), "unuse", "--global", target],
                env=extension_environment(),
                timeout=120,
            )
        except BaseException:
            logger.exception("Failed to roll back MCP mise activation %s", target)

    async def _rollback_mcp_managed_package(
        self,
        *,
        pypi_identifier: str,
        mise_target: str,
        ...
        if pypi_identifier:
            ...
            if uv:
                try:
                    await self._run_manager(
                        "",
                        [str(uv), "tool", "uninstall", pypi_identifier],
                        env=extension_environment(),
                        timeout=120,
                    )
                except BaseException:
                    logger.exception(
                        "Failed to uninstall PyPI MCP package after rollback"
                    )
        await self._rollback_mcp_mise_activation(mise_target)

    def _mcp_install_config(
        self,
        extension_id: str,
        ...
        if any(header.get("isRequired") for header in remote.get("headers") or []):
            raise ValueError(
                "This MCP remote requires credentials; add it through Manual MCP after configuring its authentication"
            )
        remote_type = (
            str(remote.get("type") or remote.get("transport") or "streamable_http")
            .lower()
            .replace("-", "_")
        )
        return {
    ...
    async def _install_mcp_pypi_package(
        self,
        task_id: str,
        identifier: str,
        version: str,
        ...
        try:
            before = (
                {path.resolve() for path in _UV_BIN_DIR.iterdir()}
                if _UV_BIN_DIR.is_dir()
                else set()
            )
            await self._run_manager(
                task_id,
                [str(uv), "tool", "install", "--force", f"{identifier}=={version}"],
                env=env,
            )
            after = [
                path.resolve()
                for path in _UV_BIN_DIR.iterdir()
            ...
            executable = next(
                (path for path in after if path.stem.casefold() in expected_names),
                None,
            )
            new_executables = [path for path in after if path not in before]
            executable = executable or (
                new_executables[0] if len(new_executables) == 1 else None
            )
            if executable is None:
                raise RuntimeError(
                    "The installed PyPI MCP package did not expose a deterministic executable"
                )
            ...
            try:
                await self._run_manager(
                    "",
                    [str(uv), "tool", "uninstall", identifier],
                    env=env,
                    timeout=120,
                )
            except BaseException:
    ...
    def _mcp_npm_binary_name(self, install_root: Path, identifier: str) -> str:
        manifests = sorted(
            install_root.rglob("package.json"),
        ...
        for manifest in manifests:
            ...
            if isinstance(bin_value, str):
                return str(package_json.get("name") or identifier).rsplit("/", 1)[-1]
            if isinstance(bin_value, dict) and bin_value:
                return str(next(iter(bin_value)))
        return ""

    async def _install_mcp_npm_package(
        self,
        task_id: str,
        identifier: str,
        version: str,
        ...
        if not mise:
            raise RuntimeError("Bundled mise is missing")
        target = f"npm:{identifier}@{version}"
        activated_target = ""
        try:
            await self._run_manager(task_id, [str(mise), "install", target], env=env)
            await self._run_manager(
                task_id,
                [str(mise), "use", "--global", "--pin", target],
                env=env,
                timeout=120,
            )
            activated_target = target
            where_out, _ = await self._run_manager(
                task_id,
                [str(mise), "where", target],
                env=env,
                timeout=120,
            )
            binary_name = self._mcp_npm_binary_name(
                Path(where_out.strip()),
                identifier,
            )
            if not binary_name:
            ...
            if proc.returncode != 0 or not stdout.decode().strip():
                raise RuntimeError(
                    ...
                    or "Unable to locate the installed MCP executable"
                )
            return stdout.decode().strip(), target
        except BaseException:
            await self._rollback_mcp_mise_activation(activated_target)
            raise

    async def _install_mcp_registry_package(
        self,
        task_id: str,
        extension_id: str,
        ...
        if (
            registry_type not in {"npm", "pypi"}
            or not identifier
            or not _is_fixed_registry_package_version(registry_type, version)
        ):
            raise ValueError(
                "Only fixed-version npm or PyPI MCP packages can be installed directly"
            )
        if any(
            variable.get("isRequired")
            for variable in package.get("environmentVariables") or []
            ...
            raise ValueError(
                "This MCP package requires environment configuration; use Manual MCP after installing its executable"
            )
        env = extension_environment()
        pypi_identifier = ""
        mise_target = ""
        if registry_type == "pypi":
            command = await self._install_mcp_pypi_package(
                task_id,
                identifier,
                version,
                env,
            )
            pypi_identifier = identifier
            managed_ref = f"pypi:{identifier}"
        else:
            command, mise_target = await self._install_mcp_npm_package(
                task_id,
                identifier,
                version,
                env,
            )
            managed_ref = f"npm:{identifier}"
        try:
            package_arguments = [
                str(item.get("value"))
            ...
            return config, source, pypi_identifier, mise_target
        except BaseException:
            await self._rollback_mcp_managed_package(
                pypi_identifier=pypi_identifier,
    ...
    def _prepare_mcp_config(
        ...
        if not config:
            raise ValueError(
                "Select a registry remote transport or provide a fixed local MCP configuration"
            )
        ...
        if config.get("transport", "stdio") == "stdio":
            ...
            if "@latest" in dynamic or (
                Path(command).name in {"npx", "uvx"}
                and ("-y" in args or not config.get("version"))
            ):
                raise ValueError(
                    "Runtime dynamic downloads and @latest are not allowed for MCP servers"
                )
            if not Path(command).is_absolute():
                resolved = shutil.which(command)
                if not resolved:
                    raise ValueError(
                        "Local MCP command must resolve to an installed deterministic executable"
                    )
        ...
        servers = [
            ...
            if str(server.get("name")) != config["name"]
        ]
        self.tasks.update(
            task_id,
            progress=75,
            message=localized("Connecting MCP server", "正在连接 MCP 服务器"),
        )
        return mcp_service, previous_servers, servers

    async def _connect_mcp_config(
        self,
        mcp_service: Any,
        ...
        if (
            not status
            or status.get("status") != "connected"
            or str(status.get("error") or "").strip()
        ):
            detail = str((status or {}).get("error") or "").strip()
            raise RuntimeError(
                detail
                or "MCP server could not be connected; its configuration was not saved"
            )

    async def _install_mcp(self, task_id: str, extension_id: str, request: dict[str, Any], actor: str) -> dict[str, Any]:
        config = self._mcp_install_config(extension_id, request)
        package = dict(request.get("package") or {})
        installed_pypi_identifier = ""
        activated_mise_target = ""
        if not config and package:
            (
                ...
                installed_pypi_identifier,
                activated_mise_target,
            ) = await self._install_mcp_registry_package(
                task_id,
                extension_id,
                package,
                request,
        ...
        try:
            mcp_service, previous_servers, servers = self._prepare_mcp_config(
                task_id,
                extension_id,
                config,
                request,
            ...
            await self._rollback_mcp_managed_package(
                pypi_identifier=installed_pypi_identifier,
                mise_target=activated_mise_target,
            )
            raise
        try:
            await self._connect_mcp_config(mcp_service, servers, config)
        except BaseException:
    ...
    async def uninstall(self, kind: str, extension_id: str, *, version: str = "", actor: str = "user") -> dict[str, Any]:
        kind = str(kind).lower()
        if kind == "skill":
            ok = _active_skills_service(required=True).uninstall_skill(extension_id)
            _forget_extension_enabled(kind, extension_id)
            _audit(actor, "uninstall", f"skill:{extension_id}", {}, "ok" if ok else "missing")
            return {"ok": ok}
        if kind == "mcp":
            mcp_service = _active_mcp_service(required=True)
            existing = await mcp_service.remove(extension_id)
            if existing is None:
                return {
                    "ok": False,
        ...
        if kind == "agent":
            ...
            if not record:
                return {
                    "ok": False,
                    "code": "installed_agent_not_found",
                    "error": localized(
                        "Installed agent not found.",
                        "未找到已安装的智能体。",
                    ),
                }
            await get_process_manager().release(str(record.get("installation_id") or ""))
        ...
        if not record:
            return {
                "ok": False,
                "code": "managed_extension_not_found",
                "error": localized(
                    "Cyrene-managed extension not found.",
                    "未找到由 Cyrene 管理的扩展。",
                ),
            }
        if kind == "toolchain" and extension_id == "python":
