...
from agent.plugin.plugin_impl.cyrene_extensions.extension_catalog import AGENT_MANIFEST_API, RECOMMENDED_AGENTS, RECOMMENDED_AGENT_ORDER
...
def list_agent_installations() -> list[dict[str, Any]]:
    ...
    for record in value:
        if not isinstance(record, dict):
            continue
        if record.get("runtime_state") in {None, "", "pending_transport"}:
            record["runtime_state"] = "not_started"
            changed = True
        if "runtime_dependencies" not in record:
            profile = (
                RECOMMENDED_AGENTS.get(str(record.get("agent_id") or ""))
            ...
            dependency = (
                ...
                and isinstance(profile.get("dependency"), dict)
                else {}
            )
            dependency_bin = str(dependency.get("bin") or "").strip()
            record["runtime_dependencies"] = [dependency_bin] if dependency_bin else []
            changed = True
    if changed:
...
def register_agent_installation(
    *,
    ...
    profile = RECOMMENDED_AGENTS.get(agent_id) if recommended else None
    dependency = (
        profile.get("dependency")
        if isinstance(profile, dict) and isinstance(profile.get("dependency"), dict)
        else {}
    )
    dependency_bin = str(dependency.get("bin") or "").strip()
    runtime_dependencies = [dependency_bin] if dependency_bin else []
    ...
    for agent_id in RECOMMENDED_AGENT_ORDER:
        ...
        cards.append({
            ...
            "description": localized(
                ...
                str(spec.get("description_zh") or spec.get("description") or ""),
            ),
            "recommended": True,
            "driver": spec["driver"],
...
def get_agent_detail(installation_id: str) -> dict[str, Any]:
    ...
    if record is None:
        raise KeyError("agent_installation_not_found")
    return {
        **agent_card(record),
...
def update_agent_settings(installation_id: str, changes: dict[str, Any], *, actor: str = "user") -> dict[str, Any]:
    ...
    if "enabled" in changes:
        if not isinstance(changes["enabled"], bool):
            raise ValueError("agent_settings_invalid")
        ...
        if action == "logout":
            ...
            if not isinstance(auth_caps, dict) or "logout" not in auth_caps:
                raise AgentRuntimeError(
                    "capability_missing",
                    localized(
                        "The agent does not support ACP logout.",
                        "该智能体不支持 ACP 登出。",
                    ),
                )
            ...
            if not method_id:
                raise AgentRuntimeError(
                    "capability_missing",
                    localized(
                        "The agent does not provide an in-protocol login method.",
                        "该智能体未提供协议内登录方式。",
                    ),
                )
...
async def probe_agent(installation_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        safe_error = localized(
            "The agent health check failed.",
            "智能体健康检查失败。",
        )
        logger.warning(
            "Agent health check failed [installation=%s kind=%s]",
            installation_id,
            kind,
            exc_info=True,
        )
        ...
        record["last_error_code"] = "agent_probe_failed"
        record["updated_at"] = _now()
        update_installation_record(record)
        return {
            "ok": False,
...
def audit(actor: str, action: str, target: str, detail: dict[str, Any]) -> None:
    from agent.plugin.plugin_impl.cyrene_extensions.extension_service import audit_extension_event
