"""Application services for Skills-owned behavior-learning HTTP use cases."""
...
class ProjectResolver:
    ...
    def project_ids(self, project: str = "") -> list[str]:
        ...
        return list(dict.fromkeys((resolved, raw)))
...
class LearningApplicationService:
    ...
    async def media_file(self, path: str) -> tuple[Path, str]:
        ...
        if not target.exists() or not target.is_file():
            raise LearningApplicationError(
                localized("Media not found.", "未找到媒体文件。"),
                404,
                "media_not_found",
            )
    ...
    async def run(self, skill_id: str) -> dict:
        return await learning.run_learned_skill_result(skill_id)
