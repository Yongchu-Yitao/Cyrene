...
class KnowledgeService:
    ...
    async def archive_run(
        ...
        del now
        if str(session.get("status") or "").strip().casefold() not in {
            "review",
            "completed",
            "done",
        }:
            return []
        workspace = self.resolve_workspace(str(project.get("id") or ""))
        ...
        for artifact in artifacts:
            if not isinstance(artifact, Mapping) or artifact.get("type") != "file_change":
                continue
            raw_path = str(artifact.get("path") or artifact.get("name") or "").strip()
            status = str(artifact.get("status") or "").strip().casefold()
            if (
                not raw_path
            ...
            attachment = dict(artifact.get("attachment")) if isinstance(artifact.get("attachment"), Mapping) else {}
            ...
            if pinned_id:
                ...
                try:
                    pinned_candidate.relative_to(EXPORTS_DIR.resolve())
                except ValueError:
                    pinned_candidate = None
                if pinned_candidate is not None and pinned_candidate.is_file():
            ...
            if source is None:
                ...
                if not candidate.is_file():
                    continue
                source = candidate
            filename = str(attachment.get("name") or source.name)
            content_type = str(attachment.get("content_type") or mimetypes.guess_type(filename)[0] or "application/octet-stream")
            content = await asyncio.to_thread(source.read_bytes)
            item = await self._write_upload(
                workspace,
                filename,
                content,
                content_type,
                provider="workbench_artifact",
                provider_library_id=session_id,
                provider_key=raw_path,
            )
            item = (
                await asyncio.to_thread(
                    ...
                    {
                        "title": Path(filename).stem or filename,
                        "abstract": str(session.get("goal") or ""),
        ...
        document_ids = [str(item["id"]) for item in archived]
        if not document_ids:
            return []
        run["knowledgeDocumentIds"] = document_ids
        project_context = dict(project.get("context")) if isinstance(project.get("context"), dict) else {}
        known_ids = list(project_context.get("knowledgeDocumentIds") or [])
        for document_id in document_ids:
            if document_id not in known_ids:
                known_ids.append(document_id)
        project_context["knowledgeDocumentIds"] = known_ids
