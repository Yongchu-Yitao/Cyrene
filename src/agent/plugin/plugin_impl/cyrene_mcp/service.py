"""Editable MCP configuration, lifecycle, and dynamic Plugin registration."""
...
from agent.plugin.validation import validate_plugin_arguments
...
from .transport import BLOCKED_EXECUTABLES, MCPServerConnection
...
_CONFIGURATION_FIELDS = frozenset(
    {"transport", "command", "args", "url", "env", "headers", "enabled"}
)
...
class MCPServerNotFoundError(ValueError):
    """Raised when an immutable MCP server identity cannot be resolved."""
    ...
    for raw in configs:
        ...
        if transport == "stdio":
            ...
            if parsed.query or parsed.fragment:
                raise ValueError(
                    "Remote MCP URL must not contain query parameters or fragments; use headers for authentication"
                )
...
def _configuration_update(value: Any) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError("MCP server configuration must be an object")
    unsupported = sorted(set(value) - _CONFIGURATION_FIELDS)
    if unsupported:
        raise ValueError(
            "Unsupported MCP configuration fields: " + ", ".join(unsupported)
        )
    update = dict(value)
...
def _merge_server_configuration(
    existing: Mapping[str, Any],
    incoming: Any,
) -> dict[str, Any]:
    update = _configuration_update(incoming)
    server = dict(existing)
    server.update(update)
    server["name"] = str(existing.get("name") or "").strip()
    server = merge_redacted_mcp_configs([dict(existing)], [server])[0]
    transport = _canonical_transport(server.get("transport"))
    server["transport"] = transport
    if transport == "stdio":
        server.pop("url", None)
...
class MCPPluginService:
    ...
    def __init__(self, *, data_directory: str | Path = DATA_DIR) -> None:
        ...
        self._authoritative_registry: PluginRegistry | None = None
        self._registries: weakref.WeakKeyDictionary[
            PluginRegistry, set[str]
        ] = weakref.WeakKeyDictionary()
        self._registry_errors: weakref.WeakKeyDictionary[
            PluginRegistry, dict[str, str]
        ] = weakref.WeakKeyDictionary()
    ...
    def attach_registry(
        ...
        *,
        ...
        with self._state_lock:
            if authoritative or self._authoritative_registry is None:
                self._authoritative_registry = registry
    ...
    def _build_packs(
        ...
        for server_name in sorted(connections):
            ...
            for tool in sorted(
                ...
                external_description = str(tool.get("description") or tool_name)
            ...
            packs[server_name] = PluginPack(
                ...
                metadata={
                    "i18n": {
                        ...
                        "zh": {
                            "name": server_name,
                            "description": f"MCP 服务器 {server_name} 提供的工具。",
                        },
                    }
                },
            )
    ...
    def _sync_registry(
        ...
        with self._state_lock:
            ...
            registry_errors = dict(self._registry_errors.get(registry, {}))
            for pack_id in previous | desired:
                registry_errors.pop(pack_id, None)
            registry_errors.update(errors)
            if registry_errors:
                self._registry_errors[registry] = registry_errors
            else:
                self._registry_errors.pop(registry, None)
    ...
    async def _invoke_raw_owned(
        ...
        with self._state_lock:
            ...
            connection = self._connections.get(normalized_server)
            pack = self._packs.get(normalized_server)
            registry = self._authoritative_registry
        if connection is None:
            raise ValueError(f"MCP server '{server_name}' is not connected")
        if pack is None:
            raise RuntimeError(
                f"MCP server '{server_name}' has no active Plugin pack"
            )
        canonical_name = next(
            (
                plugin.canonical_name
                for plugin in pack.plugins
                if str(plugin.metadata.get("mcp_tool") or "") == normalized_tool
            ),
            "",
        )
        ...
        if registry is None:
            raise RuntimeError("MCP Plugin registry is not attached")

        # Native integrations must cross the same activation/customization
        # boundary as Agent calls. Resolve by the stable canonical identity so
        # renamed tools still work, while deleted, disabled, unregistered, or
        # failed-to-publish tools cannot be invoked through this raw adapter.
        registered = registry.registered_by_canonical(canonical_name)
        resolved = registry.resolve(registered.plugin.name)
        if (
            registered.pack_id != pack.id
            or registered.source != f"mcp:{normalized_server}"
            or str(resolved.metadata.get("mcp_server") or "") != normalized_server
            or str(resolved.metadata.get("mcp_tool") or "") != normalized_tool
        ):
            raise RuntimeError(
        ...
        return await connection.call_tool(normalized_tool, normalized_arguments)
    ...
    def status(self) -> list[dict[str, Any]]:
        with self._state_lock:
            ...
            for attached_errors in self._registry_errors.values():
                for pack_id, error in attached_errors.items():
                    registry_errors.setdefault(pack_id, error)
        result: list[dict[str, Any]] = []
        ...
        return [
            {
                ...
                "i18n": dict(plugin.metadata.get("i18n") or {}),
                "mcp_server": target,
    ...
    def build_observation_content(
        self,
        value: Any,
        *,
        tool_name: str = "",
    ) -> list[dict[str, Any]] | None:
        """Translate this Plugin's persisted result into model observation parts."""

        return build_mcp_observation_content(value, tool_name=tool_name)

    def materialize_content_block(self, block: Mapping[str, Any]) -> dict[str, Any]:
        """Materialize an MCP artifact immediately before provider submission."""

        return materialize_model_content_block(block)
    ...
    async def update_configuration(
        ...
        """Update one server without allowing its persisted identity to change."""
        ...
        index = next(
            (
                position
                for position, item in enumerate(configs)
        ...
        configs[index] = _merge_server_configuration(
            configs[index],
            configuration,
        )
        ...
        safe_config = next(
            (
                item
                for item in self.configs(redacted=True)
                if str(item.get("name") or "").strip() == target
            ),
            {"name": target},
        )
        live = self.server_status(target) or {}
        ...
        return {
            ...
            "tools": tools,
            "tool_count": len(tools),
            "pack_id": str(live.get("pack_id") or self.pack_id_for_server(target)),
        }
