...
if __package__ in {None, ""}:  # direct subprocess launch from an editable pack
    ...
    from terminal.manager import TerminalInputBusyError, TerminalManager
else:
    from .client import (
        LIFECYCLE_VERSION,
        MAX_MESSAGE_BYTES,
        ...
        try:
            ...
            if request.get("token") != self.token or int(request.get("version") or 0) != PROTOCOL_VERSION:
                await self._send(writer, {
                    ...
                    "code": "unauthorized",
                    "error": localized(
                        "The terminal daemon request is unauthorized.",
                        "终端守护进程请求未获授权。",
                    ),
                })
                return
            ...
            with contextlib.suppress(Exception):
                await self._send(writer, {
                    ...
                    "code": "bad_request",
                    "error": localized(
                        "The terminal daemon request is invalid.",
                        "终端守护进程请求无效。",
                    ),
                })
    ...
    async def _dispatch(self, writer: asyncio.StreamWriter, request: dict[str, Any]) -> None:
        ...
        try:
            ...
            await self._send(writer, {
                ...
                "code": "input_busy",
                "error": localized(
                    "User input has priority. Retry terminal input after {retry_after_ms} ms.",
                    "用户输入优先，请在 {retry_after_ms} 毫秒后重试终端输入。",
            ...
            await self._send(writer, {
                "ok": False,
                "code": "not_found",
                "error": localized("Terminal not found.", "未找到终端。"),
            })
            return
