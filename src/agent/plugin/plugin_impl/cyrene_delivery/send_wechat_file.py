...
from agent.plugin.native_runtime import plugin_localized, publish_runtime_event, resolve_exportable_path, run_context_value
...
async def _tool_send_wechat_file(args: dict[str, Any], context: PluginContext) -> str:
    ...
    channels = active_plugin_service("channels")
    if channels is None:
