...
//
// Exposes `window.WorkbenchWelcomePage`.
(function () {
  ...
  // (/api/onboarding/*) and refreshes global DATA via window.bumpData /
  // window.reloadUiData — it does not depend on any legacy-UI component.
  function OnboardingFlow(props) {
    ...
    function applyResponse(payload) {
      if (payload && payload.onboarding) {
        try { window.DATA.onboarding = payload.onboarding; } catch (e) {}
        if (window.bumpData) window.bumpData();
      }
      if (window.reloadUiData) { try { return window.reloadUiData(); } catch (e) {} }
  ...
  function WorkbenchWelcomePage(props) {
    ...
    var name = (window.DATA && window.DATA.user && window.DATA.user.name) || "";
  ...
  window.WorkbenchWelcomePage = WorkbenchWelcomePage;
})();
