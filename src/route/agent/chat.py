...
def register_chat_routes(router: APIRouter, bot: Any, db_path: str) -> None:
    ...
    async def api_chat_export_file(export_id: str):
        # Export keys are generated by Cyrene and may include a legacy Unicode
        # display-name suffix. Do not run them through the upload sanitizer:
        # it changes CJK names and makes an existing export resolve to a 404.
        # Validate the single path component directly, then enforce containment.
        if (
            not export_id
            or export_id in {".", ".."}
            or "/" in export_id
        ...
        target = (_EXPORTS_DIR / export_id).resolve()
