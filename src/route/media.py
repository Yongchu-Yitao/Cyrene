...
def register_media_routes(router: APIRouter) -> None:
    manager = get_media_job_manager()

    @router.get("/api/media/status")
    async def api_media_status():
        status = await asyncio.to_thread(get_media_daemon().status)
    ...
    async def api_media_job(job_id: str):
        ...
        if not job:
            return JSONResponse({"error": "media job not found"}, status_code=404)
    ...
    async def api_media_batch(batch_id: str):
        ...
        if not batch:
            return JSONResponse({"error": "media batch not found"}, status_code=404)
